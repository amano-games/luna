...
animator_update(struct animator *animator, f32 timestamp)
{
	...
	struct animation *animation        = &animator->animation;
	struct animation_clips_slice *bank = animation_db_get_data_slice(animator->assets_db, animator->clips_handle);
...
animator_set_animation(struct animator *animator, usize index)
{
	...
	animator->animation.clip = assets_db_get_animation_clip(animator->assets_db, animator->clips_handle, index);
