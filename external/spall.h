...
SPALL_FN SPALL_FORCEINLINE bool
spall__file_write(SpallProfile *ctx, const void *p, size_t n)
{
...
SPALL_FN bool
spall__file_flush(SpallProfile *ctx)
{
	if(!ctx->data) return false;
...
SPALL_FN void
spall__file_close(SpallProfile *ctx)
{
	if(!ctx->data) return;
...
SPALL_FN SPALL_FORCEINLINE bool
spall__buffer_flush(SpallProfile *ctx, SpallBuffer *wb)
{
	// precon: wb
	// precon: wb->data
...
SPALL_FN SPALL_FORCEINLINE bool
spall__buffer_write(SpallProfile *ctx, SpallBuffer *wb, void *p, size_t n)
{
...
SPALL_FN bool
spall_buffer_flush(SpallProfile *ctx, SpallBuffer *wb)
{
#ifdef SPALL_DEBUG
	if(!wb) return false;
...
SPALL_FN bool
spall_buffer_init(SpallProfile *ctx, SpallBuffer *wb)
{
...
SPALL_FN bool
spall_buffer_quit(SpallProfile *ctx, SpallBuffer *wb)
{
...
SPALL_FN bool
spall_buffer_abort(SpallBuffer *wb)
{
	if(!wb) return false;
	wb->ctx = NULL;
...
SPALL_FN size_t
spall_build_header(void *buffer, size_t rem_size, double timestamp_unit)
{
...
SPALL_FN SPALL_FORCEINLINE size_t
spall_build_begin(void *buffer, size_t rem_size, const char *name, signed long name_len, const char *args, signed long args_len, double when, uint32_t tid, uint32_t pid)
{
...
SPALL_FN SPALL_FORCEINLINE size_t
spall_build_end(void *buffer, size_t rem_size, double when, uint32_t tid, uint32_t pid)
{
...
SPALL_FN SpallProfile
spall_init_callbacks(double timestamp_unit,
	SpallWriteCallback write,
	SpallFlushCallback flush,
	SpallCloseCallback close,
	void *userdata,
...
SPALL_FN SpallProfile
spall_init_file_ex(const char *filename, double timestamp_unit, bool is_json)
{
	SpallProfile ctx;
	memset(&ctx, 0, sizeof(ctx));
...
SPALL_FN SpallProfile
spall_init_file(const char *filename, double timestamp_unit)
{
	return spall_init_file_ex(filename, timestamp_unit, false);
}
SPALL_FN SpallProfile
spall_init_file_json(const char *filename, double timestamp_unit)
{
	return spall_init_file_ex(filename, timestamp_unit, true);
}

SPALL_FN bool
spall_flush(SpallProfile *ctx)
{
#ifdef SPALL_DEBUG
	if(!ctx) return false;
#endif
...
SPALL_FN SPALL_FORCEINLINE bool
spall_buffer_begin_args(SpallProfile *ctx, SpallBuffer *wb, const char *name, signed long name_len, const char *args, signed long args_len, double when, uint32_t tid, uint32_t pid)
{
#ifdef SPALL_DEBUG
	if(!ctx) return false;
...
SPALL_FN SPALL_FORCEINLINE bool
spall_buffer_begin_ex(SpallProfile *ctx, SpallBuffer *wb, const char *name, signed long name_len, double when, uint32_t tid, uint32_t pid)
{
	return spall_buffer_begin_args(ctx, wb, name, name_len, "", 0, when, tid, pid);
}

SPALL_FN bool
spall_buffer_begin(SpallProfile *ctx, SpallBuffer *wb, const char *name, signed long name_len, double when)
{
	return spall_buffer_begin_args(ctx, wb, name, name_len, "", 0, when, 0, 0);
}

SPALL_FN SPALL_FORCEINLINE bool
spall_buffer_end_ex(SpallProfile *ctx, SpallBuffer *wb, double when, uint32_t tid, uint32_t pid)
{
#ifdef SPALL_DEBUG
	if(!ctx) return false;
	if(!wb) return false;
	...
	if(ctx->is_json) {
		char buf[512];
		int buf_len = snprintf(buf, sizeof(buf), "{\"ph\":\"E\",\"ts\":%f,\"pid\":%u,\"tid\":%u},\n", when * ctx->timestamp_unit, pid, tid);
		if(buf_len <= 0) return false;
		if(buf_len >= sizeof(buf)) return false;
		if(!spall__buffer_write(ctx, wb, buf, buf_len)) return false;
	} else {
...
SPALL_FN bool
spall_buffer_end(SpallProfile *ctx, SpallBuffer *wb, double when)
{
	return spall_buffer_end_ex(ctx, wb, when, 0, 0);
}

SPALL_FN SPALL_FORCEINLINE void
spall__buffer_profile(SpallProfile *ctx, SpallBuffer *wb, double spall_time_begin, double spall_time_end, const char *name, int name_len)
{
	// precon: ctx
	// precon: ctx->write
