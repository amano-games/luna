...
/*
    ...
    SOKOL_dbg_assert(c)             - your own assert macro (default: dbg_assert(c))
    SOKOL_UNREACHABLE()         - a guard macro for unreachable code (default: dbg_assert(false))
...
inline sg_image
sg_make_image(const sg_image_desc &desc)
{
	return sg_make_image(&desc);
}
inline sg_sampler
sg_make_sampler(const sg_sampler_desc &desc)
{
...
inline void
sg_init_buffer(sg_buffer buf, const sg_buffer_desc &desc)
{
	return sg_init_buffer(buf, &desc);
}
inline void
sg_init_image(sg_image img, const sg_image_desc &desc)
{
	return sg_init_image(img, &desc);
}
inline void
sg_init_sampler(sg_sampler smp, const sg_sampler_desc &desc)
{
	return sg_init_sampler(smp, &desc);
}
inline void
sg_init_shader(sg_shader shd, const sg_shader_desc &desc)
{
	return sg_init_shader(shd, &desc);
}
inline void
sg_init_pipeline(sg_pipeline pip, const sg_pipeline_desc &desc)
{
	return sg_init_pipeline(pip, &desc);
}
inline void
sg_init_attachments(sg_attachments atts, const sg_attachments_desc &desc)
{
	return sg_init_attachments(atts, &desc);
}

inline void
sg_update_buffer(sg_buffer buf_id, const sg_range &data)
{
	return sg_update_buffer(buf_id, &data);
}
inline int
sg_append_buffer(sg_buffer buf_id, const sg_range &data)
{
...
#define SOKOL_UNREACHABLE SOKOL_dbg_assert(false)
#endif
...
#define _SG_TRACE_ARGS(fn, ...) \
	if(_sg.hooks.fn) { _sg.hooks.fn(__VA_ARGS__, _sg.hooks.user_data); }
#define _SG_TRACE_NOARGS(fn) \
	if(_sg.hooks.fn) { _sg.hooks.fn(_sg.hooks.user_data); }
#else
#define _SG_TRACE_ARGS(fn, ...)
#define _SG_TRACE_NOARGS(fn)
#endif
...
#define _SG_GL_CHECK_ERROR() \
	{ \
		SOKOL_dbg_assert(glGetError() == GL_NO_ERROR); \
	}
#endif
#endif
...
#define _sg_stats_add(key, val) \
	{ \
		if(_sg.stats_enabled) { _sg.stats.key += val; } \
	}
...
_SOKOL_PRIVATE void
_sg_buffer_common_init(_sg_buffer_common_t *cmn, const sg_buffer_desc *desc)
{
...
_SOKOL_PRIVATE void
_sg_image_common_init(_sg_image_common_t *cmn, const sg_image_desc *desc)
{
...
_SOKOL_PRIVATE void
_sg_sampler_common_init(_sg_sampler_common_t *cmn, const sg_sampler_desc *desc)
{
...
_SOKOL_PRIVATE void
_sg_shader_common_init(_sg_shader_common_t *cmn, const sg_shader_desc *desc)
{
	...
	for(size_t i = 0; i < SG_MAX_IMAGE_SAMPLER_PAIRS; i++) {
		...
		if(src->stage != SG_SHADERSTAGE_NONE) {
			dst->stage = src->stage;
			SOKOL_dbg_assert((src->image_slot >= 0) && (src->image_slot < SG_MAX_IMAGE_BINDSLOTS));
			SOKOL_dbg_assert(desc->images[src->image_slot].stage == src->stage);
			dst->image_slot = src->image_slot;
			SOKOL_dbg_assert((src->sampler_slot >= 0) && (src->sampler_slot < SG_MAX_SAMPLER_BINDSLOTS));
			SOKOL_dbg_assert(desc->samplers[src->sampler_slot].stage == src->stage);
...
_SOKOL_PRIVATE void
_sg_pipeline_common_init(_sg_pipeline_common_t *cmn, const sg_pipeline_desc *desc)
{
	SOKOL_dbg_assert((desc->color_count >= 0) && (desc->color_count <= SG_MAX_COLOR_ATTACHMENTS));
	...
	for(int i = 0; i < SG_MAX_VERTEXBUFFER_BINDSLOTS; i++) {
		...
		if(a_state->format != SG_VERTEXFORMAT_INVALID) {
			SOKOL_dbg_assert(a_state->buffer_index < SG_MAX_VERTEXBUFFER_BINDSLOTS);
...
_SOKOL_PRIVATE void
_sg_attachment_common_init(_sg_attachment_common_t *cmn, const sg_attachment_desc *desc)
{
...
{
	SOKOL_dbg_assert((width > 0) && (height > 0));
	cmn->width  = width;
	cmn->height = height;
...
static void
_sg_log(sg_log_item log_item, uint32_t log_level, const char *msg, uint32_t line_nr)
{
...
_SOKOL_PRIVATE void
_sg_clear(void *ptr, size_t size)
{
	SOKOL_dbg_assert(ptr && (size > 0));
	memset(ptr, 0, size);
}

_SOKOL_PRIVATE void *
_sg_malloc(size_t size)
{
	SOKOL_dbg_assert(size > 0);
	void *ptr;
...
_SOKOL_PRIVATE bool
_sg_strempty(const _sg_str_t *str)
{
	return 0 == str->buf[0];
}

_SOKOL_PRIVATE const char *
_sg_strptr(const _sg_str_t *str)
{
	return &str->buf[0];
}

_SOKOL_PRIVATE void
_sg_strcpy(_sg_str_t *dst, const char *src)
{
	SOKOL_dbg_assert(dst);
	if(src) {
#if defined(_MSC_VER)
...
_sg_align_u32(uint32_t val, uint32_t align)
{
	SOKOL_dbg_assert((align > 0) && ((align & (align - 1)) == 0));
...
_SOKOL_PRIVATE int
_sg_vertexformat_bytesize(sg_vertex_format fmt)
{
	switch(fmt) {
...
_SOKOL_PRIVATE const char *
_sg_vertexformat_to_string(sg_vertex_format fmt)
{
	switch(fmt) {
...
_SOKOL_PRIVATE const char *
_sg_shaderattrbasetype_to_string(sg_shader_attr_base_type b)
{
	switch(b) {
...
_SOKOL_PRIVATE uint32_t
_sg_uniform_alignment(sg_uniform_type type, int array_count, sg_uniform_layout ub_layout)
{
	if(ub_layout == SG_UNIFORMLAYOUT_NATIVE) {
		return 1;
	} else {
		SOKOL_dbg_assert(array_count > 0);
		if(array_count == 1) {
			switch(type) {
...
_SOKOL_PRIVATE uint32_t
_sg_uniform_size(sg_uniform_type type, int array_count, sg_uniform_layout ub_layout)
{
	SOKOL_dbg_assert(array_count > 0);
	if(array_count == 1) {
		switch(type) {
...
_SOKOL_PRIVATE bool
_sg_is_compressed_pixel_format(sg_pixel_format fmt)
{
	switch(fmt) {
...
_SOKOL_PRIVATE bool
_sg_is_valid_rendertarget_color_format(sg_pixel_format fmt)
{
	const int fmt_index = (int)fmt;
	SOKOL_dbg_assert((fmt_index >= 0) && (fmt_index < _SG_PIXELFORMAT_NUM));
	return _sg.formats[fmt_index].render && !_sg.formats[fmt_index].depth;
}

_SOKOL_PRIVATE bool
_sg_is_valid_rendertarget_depth_format(sg_pixel_format fmt)
{
	const int fmt_index = (int)fmt;
	SOKOL_dbg_assert((fmt_index >= 0) && (fmt_index < _SG_PIXELFORMAT_NUM));
	return _sg.formats[fmt_index].render && _sg.formats[fmt_index].depth;
...
_SOKOL_PRIVATE bool
_sg_is_depth_or_depth_stencil_format(sg_pixel_format fmt)
{
	return (SG_PIXELFORMAT_DEPTH == fmt) || (SG_PIXELFORMAT_DEPTH_STENCIL == fmt);
}

_SOKOL_PRIVATE bool
_sg_is_depth_stencil_format(sg_pixel_format fmt)
{
	return (SG_PIXELFORMAT_DEPTH_STENCIL == fmt);
}

_SOKOL_PRIVATE int
_sg_pixelformat_bytesize(sg_pixel_format fmt)
{
	switch(fmt) {
...
_SOKOL_PRIVATE int
_sg_roundup(int val, int round_to)
{
	return (val + (round_to - 1)) & ~(round_to - 1);
}

_SOKOL_PRIVATE uint32_t
_sg_roundup_u32(uint32_t val, uint32_t round_to)
{
	return (val + (round_to - 1)) & ~(round_to - 1);
}

_SOKOL_PRIVATE uint64_t
_sg_roundup_u64(uint64_t val, uint64_t round_to)
{
	return (val + (round_to - 1)) & ~(round_to - 1);
}

_SOKOL_PRIVATE bool
_sg_multiple_u64(uint64_t val, uint64_t of)
{
	return (val & (of - 1)) == 0;
}
...
*/
_SOKOL_PRIVATE int
_sg_row_pitch(sg_pixel_format fmt, int width, int row_align)
{
	int pitch;
	switch(fmt) {
	case SG_PIXELFORMAT_BC1_RGBA:
...
_SOKOL_PRIVATE int
_sg_num_rows(sg_pixel_format fmt, int height)
{
	int num_rows;
	switch(fmt) {
	case SG_PIXELFORMAT_BC1_RGBA:
...
_SOKOL_PRIVATE int
_sg_miplevel_dim(int base_dim, int mip_level)
{
...
*/
_SOKOL_PRIVATE int
_sg_surface_pitch(sg_pixel_format fmt, int width, int height, int row_align)
{
...
_SOKOL_PRIVATE void
_sg_pixelformat_all(_sg_pixelformat_info_t *pfi)
{
	pfi->sample = true;
	pfi->filter = true;
	pfi->blend  = true;
...
_SOKOL_PRIVATE void
_sg_pixelformat_s(_sg_pixelformat_info_t *pfi)
{
	pfi->sample = true;
}

_SOKOL_PRIVATE void
_sg_pixelformat_sf(_sg_pixelformat_info_t *pfi)
{
	pfi->sample = true;
	pfi->filter = true;
}
...
_SOKOL_PRIVATE void
_sg_pixelformat_sr(_sg_pixelformat_info_t *pfi)
{
	pfi->sample = true;
	pfi->render = true;
}
...
_SOKOL_PRIVATE void
_sg_pixelformat_sfr(_sg_pixelformat_info_t *pfi)
{
	pfi->sample = true;
	pfi->filter = true;
	pfi->render = true;
...
_SOKOL_PRIVATE void
_sg_pixelformat_srmd(_sg_pixelformat_info_t *pfi)
{
	pfi->sample = true;
	pfi->render = true;
	pfi->msaa   = true;
...
_SOKOL_PRIVATE void
_sg_pixelformat_srm(_sg_pixelformat_info_t *pfi)
{
	pfi->sample = true;
	pfi->render = true;
	pfi->msaa   = true;
...
_SOKOL_PRIVATE void
_sg_pixelformat_sfrm(_sg_pixelformat_info_t *pfi)
{
	pfi->sample = true;
	pfi->filter = true;
	pfi->render = true;
...
_SOKOL_PRIVATE void
_sg_pixelformat_sbrm(_sg_pixelformat_info_t *pfi)
{
	pfi->sample = true;
	pfi->blend  = true;
	pfi->render = true;
...
_SOKOL_PRIVATE void
_sg_pixelformat_sbr(_sg_pixelformat_info_t *pfi)
{
	pfi->sample = true;
	pfi->blend  = true;
	pfi->render = true;
...
_SOKOL_PRIVATE void
_sg_pixelformat_sfbr(_sg_pixelformat_info_t *pfi)
{
	pfi->sample = true;
	pfi->filter = true;
	pfi->blend  = true;
...
_SOKOL_PRIVATE void
_sg_dummy_setup_backend(const sg_desc *desc)
{
	SOKOL_dbg_assert(desc);
	_SOKOL_UNUSED(desc);
...
_SOKOL_PRIVATE void
_sg_dummy_discard_backend(void)
{
	// empty
}

_SOKOL_PRIVATE void
_sg_dummy_reset_state_cache(void)
{
	// empty
}
...
_SOKOL_PRIVATE void
_sg_dummy_discard_buffer(_sg_buffer_t *buf)
{
	SOKOL_dbg_assert(buf);
	_SOKOL_UNUSED(buf);
}
...
_SOKOL_PRIVATE void
_sg_dummy_discard_image(_sg_image_t *img)
{
	SOKOL_dbg_assert(img);
	_SOKOL_UNUSED(img);
}
...
_SOKOL_PRIVATE void
_sg_dummy_discard_sampler(_sg_sampler_t *smp)
{
	SOKOL_dbg_assert(smp);
	_SOKOL_UNUSED(smp);
}
...
_SOKOL_PRIVATE void
_sg_dummy_discard_shader(_sg_shader_t *shd)
{
	SOKOL_dbg_assert(shd);
	_SOKOL_UNUSED(shd);
}
...
_SOKOL_PRIVATE void
_sg_dummy_discard_pipeline(_sg_pipeline_t *pip)
{
	SOKOL_dbg_assert(pip);
	_SOKOL_UNUSED(pip);
}

_SOKOL_PRIVATE sg_resource_state
_sg_dummy_create_attachments(_sg_attachments_t *atts, _sg_image_t **color_images, _sg_image_t **resolve_images, _sg_image_t *ds_img, const sg_attachments_desc *desc)
{
	SOKOL_dbg_assert(atts && desc);
	SOKOL_dbg_assert(color_images && resolve_images);

	for(int i = 0; i < atts->cmn.num_colors; i++) {
		const sg_attachment_desc *color_desc = &desc->colors[i];
		_SOKOL_UNUSED(color_desc);
		SOKOL_dbg_assert(color_desc->image.id != SG_INVALID_ID);
		SOKOL_dbg_assert(0 == atts->dmy.colors[i].image);
		SOKOL_dbg_assert(color_images[i] && (color_images[i]->slot.id == color_desc->image.id));
		SOKOL_dbg_assert(_sg_is_valid_rendertarget_color_format(color_images[i]->cmn.pixel_format));
		...
		if(resolve_desc->image.id != SG_INVALID_ID) {
			SOKOL_dbg_assert(0 == atts->dmy.resolves[i].image);
			SOKOL_dbg_assert(resolve_images[i] && (resolve_images[i]->slot.id == resolve_desc->image.id));
			SOKOL_dbg_assert(color_images[i] && (color_images[i]->cmn.pixel_format == resolve_images[i]->cmn.pixel_format));
			atts->dmy.resolves[i].image = resolve_images[i];
		}
	}

	SOKOL_dbg_assert(0 == atts->dmy.depth_stencil.image);
	const sg_attachment_desc *ds_desc = &desc->depth_stencil;
	if(ds_desc->image.id != SG_INVALID_ID) {
		SOKOL_dbg_assert(ds_img && (ds_img->slot.id == ds_desc->image.id));
		SOKOL_dbg_assert(_sg_is_valid_rendertarget_depth_format(ds_img->cmn.pixel_format));
...
_SOKOL_PRIVATE void
_sg_dummy_discard_attachments(_sg_attachments_t *atts)
{
	SOKOL_dbg_assert(atts);
	_SOKOL_UNUSED(atts);
}

_SOKOL_PRIVATE _sg_image_t *
_sg_dummy_attachments_color_image(const _sg_attachments_t *atts, int index)
{
	SOKOL_dbg_assert(atts && (index >= 0) && (index < SG_MAX_COLOR_ATTACHMENTS));
	return atts->dmy.colors[index].image;
}

_SOKOL_PRIVATE _sg_image_t *
_sg_dummy_attachments_resolve_image(const _sg_attachments_t *atts, int index)
{
	SOKOL_dbg_assert(atts && (index >= 0) && (index < SG_MAX_COLOR_ATTACHMENTS));
...
_SOKOL_PRIVATE _sg_image_t *
_sg_dummy_attachments_ds_image(const _sg_attachments_t *atts)
{
	SOKOL_dbg_assert(atts);
...
_SOKOL_PRIVATE void
_sg_dummy_begin_pass(const sg_pass *pass)
{
	SOKOL_dbg_assert(pass);
	_SOKOL_UNUSED(pass);
}

_SOKOL_PRIVATE void
_sg_dummy_end_pass(void)
{
	// empty
}

_SOKOL_PRIVATE void
_sg_dummy_commit(void)
{
	// empty
}

_SOKOL_PRIVATE void
_sg_dummy_apply_viewport(int x, int y, int w, int h, bool origin_top_left)
{
	_SOKOL_UNUSED(x);
	_SOKOL_UNUSED(y);
	_SOKOL_UNUSED(w);
...
_SOKOL_PRIVATE void
_sg_dummy_apply_scissor_rect(int x, int y, int w, int h, bool origin_top_left)
{
	_SOKOL_UNUSED(x);
	_SOKOL_UNUSED(y);
	_SOKOL_UNUSED(w);
...
_SOKOL_PRIVATE void
_sg_dummy_apply_pipeline(_sg_pipeline_t *pip)
{
	SOKOL_dbg_assert(pip);
	_SOKOL_UNUSED(pip);
}
...
_SOKOL_PRIVATE bool
_sg_dummy_apply_bindings(_sg_bindings_t *bnd)
{
	SOKOL_dbg_assert(bnd);
	SOKOL_dbg_assert(bnd->pip);
	_SOKOL_UNUSED(bnd);
	return true;
}

_SOKOL_PRIVATE void
_sg_dummy_apply_uniforms(int ub_slot, const sg_range *data)
{
...
_SOKOL_PRIVATE void
_sg_dummy_draw(int base_element, int num_elements, int num_instances)
{
...
_SOKOL_PRIVATE void
_sg_dummy_dispatch(int num_groups_x, int num_groups_y, int num_groups_z)
{
...
_SOKOL_PRIVATE void
_sg_dummy_update_buffer(_sg_buffer_t *buf, const sg_range *data)
{
	SOKOL_dbg_assert(buf && data && data->ptr && (data->size > 0));
	_SOKOL_UNUSED(data);
...
_SOKOL_PRIVATE bool
_sg_dummy_append_buffer(_sg_buffer_t *buf, const sg_range *data, bool new_frame)
{
	SOKOL_dbg_assert(buf && data && data->ptr && (data->size > 0));
	_SOKOL_UNUSED(data);
	if(new_frame) {
...
_SOKOL_PRIVATE void
_sg_dummy_update_image(_sg_image_t *img, const sg_image_data *data)
{
	SOKOL_dbg_assert(img && data);
	_SOKOL_UNUSED(data);
...
_sg_gl_buffer_target(sg_buffer_type t)
{
	switch(t) {
...
_SOKOL_PRIVATE GLenum
_sg_gl_texture_target(sg_image_type t, int sample_count)
{
#if defined(SOKOL_GLCORE)
...
#else
	SOKOL_dbg_assert(sample_count == 1);
...
_SOKOL_PRIVATE GLenum
_sg_gl_shader_stage(sg_shader_stage stage)
{
	switch(stage) {
...
_SOKOL_PRIVATE GLint
_sg_gl_vertexformat_size(sg_vertex_format fmt)
{
	switch(fmt) {
...
_SOKOL_PRIVATE GLenum
_sg_gl_vertexformat_type(sg_vertex_format fmt)
{
	switch(fmt) {
	case SG_VERTEXFORMAT_FLOAT:
	case SG_VERTEXFORMAT_FLOAT2:
...
_SOKOL_PRIVATE GLboolean
_sg_gl_vertexformat_normalized(sg_vertex_format fmt)
{
	switch(fmt) {
	case SG_VERTEXFORMAT_BYTE4N:
	case SG_VERTEXFORMAT_UBYTE4N:
...
_SOKOL_PRIVATE GLenum
_sg_gl_primitive_type(sg_primitive_type t)
{
	switch(t) {
...
_SOKOL_PRIVATE GLenum
_sg_gl_index_type(sg_index_type t)
{
	switch(t) {
...
_SOKOL_PRIVATE GLenum
_sg_gl_compare_func(sg_compare_func cmp)
{
	switch(cmp) {
...
_SOKOL_PRIVATE GLenum
_sg_gl_stencil_op(sg_stencil_op op)
{
	switch(op) {
...
_SOKOL_PRIVATE GLenum
_sg_gl_blend_factor(sg_blend_factor f)
{
	switch(f) {
...
_SOKOL_PRIVATE GLenum
_sg_gl_blend_op(sg_blend_op op)
{
	switch(op) {
...
_SOKOL_PRIVATE GLenum
_sg_gl_min_filter(sg_filter min_f, sg_filter mipmap_f)
{
...
_SOKOL_PRIVATE GLenum
_sg_gl_mag_filter(sg_filter mag_f)
{
...
_SOKOL_PRIVATE GLenum
_sg_gl_teximage_type(sg_pixel_format fmt)
{
	switch(fmt) {
	case SG_PIXELFORMAT_R8:
	case SG_PIXELFORMAT_R8UI:
...
_SOKOL_PRIVATE GLenum
_sg_gl_teximage_format(sg_pixel_format fmt)
{
	switch(fmt) {
	case SG_PIXELFORMAT_R8:
	case SG_PIXELFORMAT_R8SN:
...
_SOKOL_PRIVATE GLenum
_sg_gl_teximage_internal_format(sg_pixel_format fmt)
{
	switch(fmt) {
...
_SOKOL_PRIVATE GLenum
_sg_gl_cubeface_target(int face_index)
{
	switch(face_index) {
...
_SOKOL_PRIVATE void
_sg_gl_init_pixelformats(bool has_bgra)
{
...
_SOKOL_PRIVATE void
_sg_gl_init_pixelformats_half_float(bool has_colorbuffer_half_float)
{
...
_SOKOL_PRIVATE void
_sg_gl_init_pixelformats_s3tc(void)
{
...
_SOKOL_PRIVATE void
_sg_gl_init_pixelformats_rgtc(void)
{
...
_SOKOL_PRIVATE void
_sg_gl_init_pixelformats_bptc(void)
{
...
_SOKOL_PRIVATE void
_sg_gl_init_pixelformats_etc2(void)
{
...
_SOKOL_PRIVATE void
_sg_gl_init_pixelformats_astc(void)
{
...
_SOKOL_PRIVATE void
_sg_gl_cache_clear_buffer_bindings(bool force)
{
...
_SOKOL_PRIVATE void
_sg_gl_cache_bind_buffer(GLenum target, GLuint buffer)
{
	SOKOL_dbg_assert((GL_ARRAY_BUFFER == target) || (GL_ELEMENT_ARRAY_BUFFER == target) || (GL_SHADER_STORAGE_BUFFER == target));
...
_SOKOL_PRIVATE void
_sg_gl_cache_bind_storage_buffer(uint8_t glsl_binding_n, GLuint buffer)
{
	SOKOL_dbg_assert(glsl_binding_n < _SG_GL_MAX_SBUF_BINDINGS);
...
_SOKOL_PRIVATE void
_sg_gl_cache_store_buffer_binding(GLenum target)
{
	if(target == GL_ARRAY_BUFFER) {
...
_SOKOL_PRIVATE void
_sg_gl_cache_restore_buffer_binding(GLenum target)
{
	if(target == GL_ARRAY_BUFFER) {
...
_SOKOL_PRIVATE void
_sg_gl_cache_invalidate_buffer(GLuint buf)
{
...
_SOKOL_PRIVATE void
_sg_gl_cache_active_texture(GLenum texture)
{
	_SG_GL_CHECK_ERROR();
...
_SOKOL_PRIVATE void
_sg_gl_cache_clear_texture_sampler_bindings(bool force)
{
	_SG_GL_CHECK_ERROR();
...
_SOKOL_PRIVATE void
_sg_gl_cache_bind_texture_sampler(int8_t gl_tex_slot, GLenum target, GLuint texture, GLuint sampler)
{
	/* it's valid to call this function with target=0 and/or texture=0
       ...
       the new binding
    */
	SOKOL_dbg_assert((gl_tex_slot >= 0) && (gl_tex_slot < _SG_GL_MAX_IMG_SMP_BINDINGS));
...
_SOKOL_PRIVATE void
_sg_gl_cache_store_texture_sampler_binding(int8_t gl_tex_slot)
{
	SOKOL_dbg_assert((gl_tex_slot >= 0) && (gl_tex_slot < _SG_GL_MAX_IMG_SMP_BINDINGS));
...
_SOKOL_PRIVATE void
_sg_gl_cache_restore_texture_sampler_binding(int8_t gl_tex_slot)
{
	SOKOL_dbg_assert((gl_tex_slot >= 0) && (gl_tex_slot < _SG_GL_MAX_IMG_SMP_BINDINGS));
	_sg_gl_cache_texture_sampler_bind_slot *slot = &_sg.gl.cache.stored_texture_sampler;
	if(slot->texture != 0) {
		// we only care about restoring valid ids
		SOKOL_dbg_assert(slot->target != 0);
...
_SOKOL_PRIVATE void
_sg_gl_cache_invalidate_texture_sampler(GLuint tex, GLuint smp)
{
	_SG_GL_CHECK_ERROR();
...
_SOKOL_PRIVATE void
_sg_gl_cache_invalidate_program(GLuint prog)
{
...
_SOKOL_PRIVATE void
_sg_gl_cache_invalidate_pipeline(_sg_pipeline_t *pip)
{
...
_SOKOL_PRIVATE void
_sg_gl_setup_backend(const sg_desc *desc)
{
	_SOKOL_UNUSED(desc);
...
_SOKOL_PRIVATE void
_sg_gl_discard_backend(void)
{
	SOKOL_dbg_assert(_sg.gl.valid);
	if(_sg.gl.vao) {
...
_sg_gl_create_buffer(_sg_buffer_t *buf, const sg_buffer_desc *desc)
{
	...
	for(int slot = 0; slot < buf->cmn.num_slots; slot++) {
		GLuint gl_buf = 0;
		if(buf->gl.injected) {
			SOKOL_dbg_assert(desc->gl_buffers[slot]);
			gl_buf = desc->gl_buffers[slot];
		} else {
			glGenBuffers(1, &gl_buf);
			SOKOL_dbg_assert(gl_buf);
...
_SOKOL_PRIVATE void
_sg_gl_discard_buffer(_sg_buffer_t *buf)
{
	SOKOL_dbg_assert(buf);
	_SG_GL_CHECK_ERROR();
	for(int slot = 0; slot < buf->cmn.num_slots; slot++) {
...
_SOKOL_PRIVATE bool
_sg_gl_supported_texture_format(sg_pixel_format fmt)
{
	const int fmt_index = (int)fmt;
	SOKOL_dbg_assert((fmt_index > SG_PIXELFORMAT_NONE) && (fmt_index < _SG_PIXELFORMAT_NUM));
...
_sg_gl_create_image(_sg_image_t *img, const sg_image_desc *desc)
{
	...
	if(!_sg.features.msaa_image_bindings && img->cmn.render_target && msaa) {
		...
		for(int slot = 0; slot < img->cmn.num_slots; slot++) {
			SOKOL_dbg_assert(desc->gl_textures[slot]);
		...
		for(int slot = 0; slot < img->cmn.num_slots; slot++) {
			...
			SOKOL_dbg_assert(img->gl.tex[slot]);
					...
					for(int mip_index = 0; mip_index < img->cmn.num_mipmaps; mip_index++, data_index++) {
						...
						if((SG_IMAGETYPE_2D == img->cmn.type) || (SG_IMAGETYPE_CUBE == img->cmn.type)) {
							if(is_compressed) {
								...
								glCompressedTexImage2D(gl_img_target, mip_index, gl_internal_format, mip_width, mip_height, 0, data_size, data_ptr);
							} else {
								const GLenum gl_type = _sg_gl_teximage_type(img->cmn.pixel_format);
#if defined(SOKOL_GLCORE) && !defined(__APPLE__)
								if(msaa) {
									glTexImage2DMultisample(gl_img_target, img->cmn.sample_count, gl_internal_format, mip_width, mip_height, GL_TRUE);
								} else {
									glTexImage2D(gl_img_target, mip_index, (GLint)gl_internal_format, mip_width, mip_height, 0, gl_format, gl_type, data_ptr);
								}
#else
								SOKOL_dbg_assert(!msaa);
								glTexImage2D(gl_img_target, mip_index, (GLint)gl_internal_format, mip_width, mip_height, 0, gl_format, gl_type, data_ptr);
							...
							if(is_compressed) {
								SOKOL_dbg_assert(!msaa);
								const GLsizei data_size = (GLsizei)desc->data.subimage[face_index][mip_index].size;
								glCompressedTexImage3D(gl_img_target, mip_index, gl_internal_format, mip_width, mip_height, mip_depth, 0, data_size, data_ptr);
							} else {
								const GLenum gl_type = _sg_gl_teximage_type(img->cmn.pixel_format);
#if defined(SOKOL_GLCORE) && !defined(__APPLE__)
								if(msaa) {
									// NOTE: only for array textures, not actual 3D textures!
									glTexImage3DMultisample(gl_img_target, img->cmn.sample_count, gl_internal_format, mip_width, mip_height, mip_depth, GL_TRUE);
								} else {
									glTexImage3D(gl_img_target, mip_index, (GLint)gl_internal_format, mip_width, mip_height, mip_depth, 0, gl_format, gl_type, data_ptr);
								}
#else
								SOKOL_dbg_assert(!msaa);
								glTexImage3D(gl_img_target, mip_index, (GLint)gl_internal_format, mip_width, mip_height, mip_depth, 0, gl_format, gl_type, data_ptr);
...
_SOKOL_PRIVATE void
_sg_gl_discard_image(_sg_image_t *img)
{
	SOKOL_dbg_assert(img);
	_SG_GL_CHECK_ERROR();
	for(int slot = 0; slot < img->cmn.num_slots; slot++) {
...
_SOKOL_PRIVATE void
_sg_gl_discard_sampler(_sg_sampler_t *smp)
{
	SOKOL_dbg_assert(smp);
	_SG_GL_CHECK_ERROR();
...
_SOKOL_PRIVATE GLuint
_sg_gl_compile_shader(sg_shader_stage stage, const char *src)
{
	SOKOL_dbg_assert(src);
	_SG_GL_CHECK_ERROR();
...
_SOKOL_PRIVATE bool
_sg_gl_ensure_glsl_bindslot_ranges(const sg_shader_desc *desc)
{
	SOKOL_dbg_assert(desc);
	for(size_t i = 0; i < SG_MAX_STORAGEBUFFER_BINDSLOTS; i++) {
...
_sg_gl_create_shader(_sg_shader_t *shd, const sg_shader_desc *desc)
{
	...
	SOKOL_dbg_assert((has_vs && has_fs) || has_cs);
	...
	for(size_t ub_index = 0; ub_index < SG_MAX_UNIFORMBLOCK_BINDSLOTS; ub_index++) {
		...
		SOKOL_dbg_assert(ub_desc->size > 0);
		_sg_gl_uniform_block_t *ub = &shd->gl.uniform_blocks[ub_index];
		SOKOL_dbg_assert(ub->num_uniforms == 0);
		...
		for(int u_index = 0; u_index < SG_MAX_UNIFORMBLOCK_MEMBERS; u_index++) {
			...
			SOKOL_dbg_assert(u_desc->glsl_name);
		...
		SOKOL_dbg_assert(ub_desc->size == (size_t)cur_uniform_offset);
	...
	for(size_t sbuf_index = 0; sbuf_index < SG_MAX_STORAGEBUFFER_BINDSLOTS; sbuf_index++) {
		...
		SOKOL_dbg_assert(sbuf_desc->glsl_binding_n < _SG_GL_MAX_SBUF_BINDINGS);
	...
	for(size_t img_smp_index = 0; img_smp_index < SG_MAX_IMAGE_SAMPLER_PAIRS; img_smp_index++) {
		...
		SOKOL_dbg_assert(img_smp_desc->glsl_name);
...
_SOKOL_PRIVATE void
_sg_gl_discard_shader(_sg_shader_t *shd)
{
	SOKOL_dbg_assert(shd);
	_SG_GL_CHECK_ERROR();
	if(shd->gl.prog) {
...
{
	SOKOL_dbg_assert(pip && shd && desc);
	SOKOL_dbg_assert((pip->shader == 0) && (pip->cmn.shader_id.id != SG_INVALID_ID));
	SOKOL_dbg_assert(desc->shader.id == shd->slot.id);
	SOKOL_dbg_assert(shd->gl.prog);
	SOKOL_dbg_assert(_sg.limits.max_vertex_attrs <= SG_MAX_VERTEX_ATTRIBUTES);
	pip->shader = shd;
	...
	for(int attr_index = 0; attr_index < _sg.limits.max_vertex_attrs; attr_index++) {
		...
		if(attr_loc != -1) {
			SOKOL_dbg_assert(attr_loc < (GLint)_sg.limits.max_vertex_attrs);
			_sg_gl_attr_t *gl_attr = &pip->gl.attrs[attr_loc];
			SOKOL_dbg_assert(gl_attr->vb_index == -1);
			...
			SOKOL_dbg_assert(l_state->stride > 0);
...
_SOKOL_PRIVATE void
_sg_gl_discard_pipeline(_sg_pipeline_t *pip)
{
	SOKOL_dbg_assert(pip);
...
_sg_gl_fb_attach_texture(const _sg_gl_attachment_t *gl_att, const _sg_attachment_common_t *cmn_att, GLenum gl_att_type)
{
	...
	SOKOL_dbg_assert(gl_tex);
	const GLuint gl_target = img->gl.target;
	SOKOL_dbg_assert(gl_target);
...
_SOKOL_PRIVATE sg_resource_state
_sg_gl_create_attachments(_sg_attachments_t *atts, _sg_image_t **color_images, _sg_image_t **resolve_images, _sg_image_t *ds_image, const sg_attachments_desc *desc)
{
	SOKOL_dbg_assert(atts && desc);
	SOKOL_dbg_assert(color_images && resolve_images);
	_SG_GL_CHECK_ERROR();
	...
	for(int i = 0; i < atts->cmn.num_colors; i++) {
		...
		_SOKOL_UNUSED(color_desc);
		SOKOL_dbg_assert(color_desc->image.id != SG_INVALID_ID);
		SOKOL_dbg_assert(0 == atts->gl.colors[i].image);
		SOKOL_dbg_assert(color_images[i] && (color_images[i]->slot.id == color_desc->image.id));
		SOKOL_dbg_assert(_sg_is_valid_rendertarget_color_format(color_images[i]->cmn.pixel_format));
		...
		const sg_attachment_desc *resolve_desc = &desc->resolves[i];
		if(resolve_desc->image.id != SG_INVALID_ID) {
			SOKOL_dbg_assert(0 == atts->gl.resolves[i].image);
			SOKOL_dbg_assert(resolve_images[i] && (resolve_images[i]->slot.id == resolve_desc->image.id));
			SOKOL_dbg_assert(color_images[i] && (color_images[i]->cmn.pixel_format == resolve_images[i]->cmn.pixel_format));
			atts->gl.resolves[i].image = resolve_images[i];
		}
	}
	SOKOL_dbg_assert(0 == atts->gl.depth_stencil.image);
	const sg_attachment_desc *ds_desc = &desc->depth_stencil;
	if(ds_desc->image.id != SG_INVALID_ID) {
		SOKOL_dbg_assert(ds_image && (ds_image->slot.id == ds_desc->image.id));
		SOKOL_dbg_assert(_sg_is_valid_rendertarget_depth_format(ds_image->cmn.pixel_format));
	...
	for(int i = 0; i < atts->cmn.num_colors; i++) {
		...
		if(gl_resolve_att->image) {
			...
			SOKOL_dbg_assert(0 == atts->gl.msaa_resolve_framebuffer[i]);
...
_SOKOL_PRIVATE void
_sg_gl_discard_attachments(_sg_attachments_t *atts)
{
	SOKOL_dbg_assert(atts);
	_SG_GL_CHECK_ERROR();
	if(0 != atts->gl.fb) {
...
_SOKOL_PRIVATE _sg_image_t *
_sg_gl_attachments_color_image(const _sg_attachments_t *atts, int index)
{
	SOKOL_dbg_assert(atts && (index >= 0) && (index < SG_MAX_COLOR_ATTACHMENTS));
...
_SOKOL_PRIVATE _sg_image_t *
_sg_gl_attachments_resolve_image(const _sg_attachments_t *atts, int index)
{
	SOKOL_dbg_assert(atts && (index >= 0) && (index < SG_MAX_COLOR_ATTACHMENTS));
...
_SOKOL_PRIVATE _sg_image_t *
_sg_gl_attachments_ds_image(const _sg_attachments_t *atts)
{
	SOKOL_dbg_assert(atts);
...
_SOKOL_PRIVATE void
_sg_gl_begin_pass(const sg_pass *pass)
{
...
_SOKOL_PRIVATE void
_sg_gl_end_pass(void)
{
	_SG_GL_CHECK_ERROR();

	if(_sg.cur_pass.atts) {
		const _sg_attachments_t *atts = _sg.cur_pass.atts;
		SOKOL_dbg_assert(atts->slot.id == _sg.cur_pass.atts_id.id);
...
_SOKOL_PRIVATE void
_sg_gl_apply_viewport(int x, int y, int w, int h, bool origin_top_left)
{
...
_SOKOL_PRIVATE void
_sg_gl_apply_scissor_rect(int x, int y, int w, int h, bool origin_top_left)
{
	y = origin_top_left ? (_sg.cur_pass.height - (y + h)) : y;
	glScissor(x, y, w, h);
}
...
_SOKOL_PRIVATE void
_sg_gl_apply_pipeline(_sg_pipeline_t *pip)
{
	SOKOL_dbg_assert(pip);
	SOKOL_dbg_assert(pip->shader && (pip->cmn.shader_id.id == pip->shader->slot.id));
	_SG_GL_CHECK_ERROR();
...
_SOKOL_PRIVATE void
_sg_gl_handle_memory_barriers(const _sg_shader_t *shd, const _sg_bindings_t *bnd)
{
	if(!_sg.features.compute) {
		return;
	}
...
_SOKOL_PRIVATE bool
_sg_gl_apply_bindings(_sg_bindings_t *bnd)
{
	SOKOL_dbg_assert(bnd);
	SOKOL_dbg_assert(bnd->pip && bnd->pip->shader);
	SOKOL_dbg_assert(bnd->pip->shader->slot.id == bnd->pip->cmn.shader_id.id);
	_SG_GL_CHECK_ERROR();
...
// take care of storage buffer memory barriers
	...
	for(size_t img_smp_index = 0; img_smp_index < SG_MAX_IMAGE_SAMPLER_PAIRS; img_smp_index++) {
		...
		if(gl_tex_slot != -1) {
			SOKOL_dbg_assert(img_smp->image_slot < SG_MAX_IMAGE_BINDSLOTS);
			SOKOL_dbg_assert(img_smp->sampler_slot < SG_MAX_SAMPLER_BINDSLOTS);
	...
	for(GLuint attr_index = 0; attr_index < (GLuint)_sg.limits.max_vertex_attrs; attr_index++) {
		...
		if(attr->vb_index >= 0) {
			...
			SOKOL_dbg_assert(attr->vb_index < SG_MAX_VERTEXBUFFER_BINDSLOTS);
...
_SOKOL_PRIVATE void
_sg_gl_apply_uniforms(int ub_slot, const sg_range *data)
{
	SOKOL_dbg_assert(_sg.gl.cache.cur_pipeline);
	SOKOL_dbg_assert((ub_slot >= 0) && (ub_slot < SG_MAX_UNIFORMBLOCK_BINDSLOTS));
	const _sg_pipeline_t *pip = _sg.gl.cache.cur_pipeline;
	SOKOL_dbg_assert(pip && pip->shader);
	SOKOL_dbg_assert(pip->slot.id == _sg.gl.cache.cur_pipeline_id.id);
	const _sg_shader_t *shd = pip->shader;
	SOKOL_dbg_assert(shd->slot.id == pip->cmn.shader_id.id);
	SOKOL_dbg_assert(SG_SHADERSTAGE_NONE != shd->cmn.uniform_blocks[ub_slot].stage);
	SOKOL_dbg_assert(data->size == shd->cmn.uniform_blocks[ub_slot].size);
	...
	for(int u_index = 0; u_index < gl_ub->num_uniforms; u_index++) {
		...
		SOKOL_dbg_assert(u->type != SG_UNIFORMTYPE_INVALID);
		if(u->gl_loc == -1) {
			continue;
		}
...
_SOKOL_PRIVATE void
_sg_gl_draw(int base_element, int num_elements, int num_instances)
{
	SOKOL_dbg_assert(_sg.gl.cache.cur_pipeline);
...
_SOKOL_PRIVATE void
_sg_gl_dispatch(int num_groups_x, int num_groups_y, int num_groups_z)
{
#if defined(_SOKOL_GL_HAS_COMPUTE)
	if(!_sg.features.compute) {
		return;
...
_SOKOL_PRIVATE void
_sg_gl_update_buffer(_sg_buffer_t *buf, const sg_range *data)
{
	SOKOL_dbg_assert(buf && data && data->ptr && (data->size > 0));
	...
	SOKOL_dbg_assert(buf->cmn.active_slot < SG_NUM_INFLIGHT_FRAMES);
...
_SOKOL_PRIVATE void
_sg_gl_append_buffer(_sg_buffer_t *buf, const sg_range *data, bool new_frame)
{
	SOKOL_dbg_assert(buf && data && data->ptr && (data->size > 0));
	if(new_frame) {
		if(++buf->cmn.active_slot >= buf->cmn.num_slots) {
...
_SOKOL_PRIVATE void
_sg_gl_update_image(_sg_image_t *img, const sg_image_data *data)
{
	SOKOL_dbg_assert(img && data);
	...
	SOKOL_dbg_assert(img->cmn.active_slot < SG_NUM_INFLIGHT_FRAMES);
	SOKOL_dbg_assert(0 != img->gl.tex[img->cmn.active_slot]);
	_sg_gl_cache_store_texture_sampler_binding(0);
	...
	for(int face_index = 0; face_index < num_faces; face_index++) {
		for(int mip_index = 0; mip_index < num_mips; mip_index++) {
			...
			if((SG_IMAGETYPE_2D == img->cmn.type) || (SG_IMAGETYPE_CUBE == img->cmn.type)) {
				glTexSubImage2D(gl_img_target, mip_index, 0, 0, mip_width, mip_height, gl_img_format, gl_img_type, data_ptr);
			} else if((SG_IMAGETYPE_3D == img->cmn.type) || (SG_IMAGETYPE_ARRAY == img->cmn.type)) {
				int mip_depth = img->cmn.num_slices;
				if(SG_IMAGETYPE_3D == img->cmn.type) {
					mip_depth = _sg_miplevel_dim(img->cmn.num_slices, mip_index);
				}
				glTexSubImage3D(gl_img_target, mip_index, 0, 0, 0, mip_width, mip_height, mip_depth, gl_img_format, gl_img_type, data_ptr);
			}
		}
	}
	_sg_gl_cache_restore_texture_sampler_binding(0);
...
_sg_d3d11_CheckFormatSupport(ID3D11Device *self, DXGI_FORMAT Format, UINT *pFormatSupport)
{
#if defined(__cplusplus)
...
static inline void
_sg_d3d11_OMSetRenderTargets(ID3D11DeviceContext *self, UINT NumViews, ID3D11RenderTargetView *const *ppRenderTargetViews, ID3D11DepthStencilView *pDepthStencilView)
{
#if defined(__cplusplus)
...
static inline void
_sg_d3d11_RSSetState(ID3D11DeviceContext *self, ID3D11RasterizerState *pRasterizerState)
{
#if defined(__cplusplus)
...
static inline void
_sg_d3d11_OMSetDepthStencilState(ID3D11DeviceContext *self, ID3D11DepthStencilState *pDepthStencilState, UINT StencilRef)
{
#if defined(__cplusplus)
...
static inline void
_sg_d3d11_OMSetBlendState(ID3D11DeviceContext *self, ID3D11BlendState *pBlendState, const FLOAT BlendFactor[4], UINT SampleMask)
{
#if defined(__cplusplus)
...
static inline void
_sg_d3d11_IASetVertexBuffers(ID3D11DeviceContext *self, UINT StartSlot, UINT NumBuffers, ID3D11Buffer *const *ppVertexBuffers, const UINT *pStrides, const UINT *pOffsets)
{
#if defined(__cplusplus)
...
static inline void
_sg_d3d11_IASetInputLayout(ID3D11DeviceContext *self, ID3D11InputLayout *pInputLayout)
{
#if defined(__cplusplus)
...
static inline void
_sg_d3d11_VSSetShader(ID3D11DeviceContext *self, ID3D11VertexShader *pVertexShader, ID3D11ClassInstance *const *ppClassInstances, UINT NumClassInstances)
{
#if defined(__cplusplus)
...
static inline void
_sg_d3d11_PSSetShader(ID3D11DeviceContext *self, ID3D11PixelShader *pPixelShader, ID3D11ClassInstance *const *ppClassInstances, UINT NumClassInstances)
{
#if defined(__cplusplus)
...
static inline void
_sg_d3d11_CSSetShader(ID3D11DeviceContext *self, ID3D11ComputeShader *pComputeShader, ID3D11ClassInstance *const *ppClassInstances, UINT NumClassInstances)
{
#if defined(__cplusplus)
...
static inline void
_sg_d3d11_VSSetConstantBuffers(ID3D11DeviceContext *self, UINT StartSlot, UINT NumBuffers, ID3D11Buffer *const *ppConstantBuffers)
{
#if defined(__cplusplus)
...
static inline void
_sg_d3d11_PSSetConstantBuffers(ID3D11DeviceContext *self, UINT StartSlot, UINT NumBuffers, ID3D11Buffer *const *ppConstantBuffers)
{
#if defined(__cplusplus)
...
static inline void
_sg_d3d11_CSSetConstantBuffers(ID3D11DeviceContext *self, UINT StartSlot, UINT NumBuffers, ID3D11Buffer *const *ppConstantBuffers)
{
#if defined(__cplusplus)
...
static inline void
_sg_d3d11_VSSetShaderResources(ID3D11DeviceContext *self, UINT StartSlot, UINT NumViews, ID3D11ShaderResourceView *const *ppShaderResourceViews)
{
#if defined(__cplusplus)
...
static inline void
_sg_d3d11_PSSetShaderResources(ID3D11DeviceContext *self, UINT StartSlot, UINT NumViews, ID3D11ShaderResourceView *const *ppShaderResourceViews)
{
#if defined(__cplusplus)
...
static inline void
_sg_d3d11_CSSetShaderResources(ID3D11DeviceContext *self, UINT StartSlot, UINT NumViews, ID3D11ShaderResourceView *const *ppShaderResourceViews)
{
#if defined(__cplusplus)
...
static inline void
_sg_d3d11_VSSetSamplers(ID3D11DeviceContext *self, UINT StartSlot, UINT NumSamplers, ID3D11SamplerState *const *ppSamplers)
{
#if defined(__cplusplus)
...
static inline void
_sg_d3d11_PSSetSamplers(ID3D11DeviceContext *self, UINT StartSlot, UINT NumSamplers, ID3D11SamplerState *const *ppSamplers)
{
#if defined(__cplusplus)
...
static inline void
_sg_d3d11_CSSetSamplers(ID3D11DeviceContext *self, UINT StartSlot, UINT NumSamplers, ID3D11SamplerState *const *ppSamplers)
{
#if defined(__cplusplus)
...
static inline void
_sg_d3d11_CSSetUnorderedAccessViews(ID3D11DeviceContext *self, UINT StartSlot, UINT NumUAVs, ID3D11UnorderedAccessView *const *ppUnorderedAccessViews, const UINT *pUAVInitialCounts)
{
#if defined(__cplusplus)
...
static inline HRESULT
_sg_d3d11_CreateBuffer(ID3D11Device *self, const D3D11_BUFFER_DESC *pDesc, const D3D11_SUBRESOURCE_DATA *pInitialData, ID3D11Buffer **ppBuffer)
{
#if defined(__cplusplus)
...
static inline HRESULT
_sg_d3d11_CreateTexture2D(ID3D11Device *self, const D3D11_TEXTURE2D_DESC *pDesc, const D3D11_SUBRESOURCE_DATA *pInitialData, ID3D11Texture2D **ppTexture2D)
{
#if defined(__cplusplus)
...
static inline HRESULT
_sg_d3d11_CreateShaderResourceView(ID3D11Device *self, ID3D11Resource *pResource, const D3D11_SHADER_RESOURCE_VIEW_DESC *pDesc, ID3D11ShaderResourceView **ppSRView)
{
#if defined(__cplusplus)
...
static inline HRESULT
_sg_d3d11_CreateUnorderedAccessView(ID3D11Device *self, ID3D11Resource *pResource, const D3D11_UNORDERED_ACCESS_VIEW_DESC *pDesc, ID3D11UnorderedAccessView **ppUAVView)
{
#if defined(__cplusplus)
...
static inline void
_sg_d3d11_GetResource(ID3D11View *self, ID3D11Resource **ppResource)
{
#if defined(__cplusplus)
...
static inline HRESULT
_sg_d3d11_CreateTexture3D(ID3D11Device *self, const D3D11_TEXTURE3D_DESC *pDesc, const D3D11_SUBRESOURCE_DATA *pInitialData, ID3D11Texture3D **ppTexture3D)
{
#if defined(__cplusplus)
...
static inline HRESULT
_sg_d3d11_CreateSamplerState(ID3D11Device *self, const D3D11_SAMPLER_DESC *pSamplerDesc, ID3D11SamplerState **ppSamplerState)
{
#if defined(__cplusplus)
...
_sg_d3d11_GetBufferPointer(ID3D10Blob *self)
{
#if defined(__cplusplus)
...
_sg_d3d11_GetBufferSize(ID3D10Blob *self)
{
#if defined(__cplusplus)
...
static inline HRESULT
_sg_d3d11_CreateVertexShader(ID3D11Device *self, const void *pShaderBytecode, SIZE_T BytecodeLength, ID3D11ClassLinkage *pClassLinkage, ID3D11VertexShader **ppVertexShader)
{
#if defined(__cplusplus)
...
static inline HRESULT
_sg_d3d11_CreatePixelShader(ID3D11Device *self, const void *pShaderBytecode, SIZE_T BytecodeLength, ID3D11ClassLinkage *pClassLinkage, ID3D11PixelShader **ppPixelShader)
{
#if defined(__cplusplus)
...
static inline HRESULT
_sg_d3d11_CreateComputeShader(ID3D11Device *self, const void *pShaderBytecode, SIZE_T BytecodeLength, ID3D11ClassLinkage *pClassLinkage, ID3D11ComputeShader **ppComputeShader)
{
#if defined(__cplusplus)
...
static inline HRESULT
_sg_d3d11_CreateInputLayout(ID3D11Device *self, const D3D11_INPUT_ELEMENT_DESC *pInputElementDescs, UINT NumElements, const void *pShaderBytecodeWithInputSignature, SIZE_T BytecodeLength, ID3D11InputLayout **ppInputLayout)
{
#if defined(__cplusplus)
...
static inline HRESULT
_sg_d3d11_CreateRasterizerState(ID3D11Device *self, const D3D11_RASTERIZER_DESC *pRasterizerDesc, ID3D11RasterizerState **ppRasterizerState)
{
#if defined(__cplusplus)
...
static inline HRESULT
_sg_d3d11_CreateDepthStencilState(ID3D11Device *self, const D3D11_DEPTH_STENCIL_DESC *pDepthStencilDesc, ID3D11DepthStencilState **ppDepthStencilState)
{
#if defined(__cplusplus)
...
static inline HRESULT
_sg_d3d11_CreateRenderTargetView(ID3D11Device *self, ID3D11Resource *pResource, const D3D11_RENDER_TARGET_VIEW_DESC *pDesc, ID3D11RenderTargetView **ppRTView)
{
#if defined(__cplusplus)
...
static inline HRESULT
_sg_d3d11_CreateDepthStencilView(ID3D11Device *self, ID3D11Resource *pResource, const D3D11_DEPTH_STENCIL_VIEW_DESC *pDesc, ID3D11DepthStencilView **ppDepthStencilView)
{
#if defined(__cplusplus)
...
static inline void
_sg_d3d11_RSSetScissorRects(ID3D11DeviceContext *self, UINT NumRects, const D3D11_RECT *pRects)
{
#if defined(__cplusplus)
...
static inline void
_sg_d3d11_ClearRenderTargetView(ID3D11DeviceContext *self, ID3D11RenderTargetView *pRenderTargetView, const FLOAT ColorRGBA[4])
{
#if defined(__cplusplus)
...
static inline void
_sg_d3d11_ClearDepthStencilView(ID3D11DeviceContext *self, ID3D11DepthStencilView *pDepthStencilView, UINT ClearFlags, FLOAT Depth, UINT8 Stencil)
{
#if defined(__cplusplus)
...
static inline void
_sg_d3d11_ResolveSubresource(ID3D11DeviceContext *self, ID3D11Resource *pDstResource, UINT DstSubresource, ID3D11Resource *pSrcResource, UINT SrcSubresource, DXGI_FORMAT Format)
{
#if defined(__cplusplus)
...
static inline void
_sg_d3d11_IASetPrimitiveTopology(ID3D11DeviceContext *self, D3D11_PRIMITIVE_TOPOLOGY Topology)
{
#if defined(__cplusplus)
...
static inline void
_sg_d3d11_UpdateSubresource(ID3D11DeviceContext *self, ID3D11Resource *pDstResource, UINT DstSubresource, const D3D11_BOX *pDstBox, const void *pSrcData, UINT SrcRowPitch, UINT SrcDepthPitch)
{
#if defined(__cplusplus)
...
static inline void
_sg_d3d11_DrawIndexedInstanced(ID3D11DeviceContext *self, UINT IndexCountPerInstance, UINT InstanceCount, UINT StartIndexLocation, INT BaseVertexLocation, UINT StartInstanceLocation)
{
#if defined(__cplusplus)
...
static inline void
_sg_d3d11_Draw(ID3D11DeviceContext *self, UINT VertexCount, UINT StartVertexLocation)
{
#if defined(__cplusplus)
...
static inline void
_sg_d3d11_DrawInstanced(ID3D11DeviceContext *self, UINT VertexCountPerInstance, UINT InstanceCount, UINT StartVertexLocation, UINT StartInstanceLocation)
{
#if defined(__cplusplus)
...
static inline void
_sg_d3d11_Dispatch(ID3D11DeviceContext *self, UINT ThreadGroupCountX, UINT ThreadGroupCountY, UINT ThreadGroupCountZ)
{
#if defined(__cplusplus)
...
static inline HRESULT
_sg_d3d11_Map(ID3D11DeviceContext *self, ID3D11Resource *pResource, UINT Subresource, D3D11_MAP MapType, UINT MapFlags, D3D11_MAPPED_SUBRESOURCE *pMappedResource)
{
#if defined(__cplusplus)
...
static inline void
_sg_d3d11_Unmap(ID3D11DeviceContext *self, ID3D11Resource *pResource, UINT Subresource)
{
#if defined(__cplusplus)
...
static inline void
_sg_d3d11_ClearState(ID3D11DeviceContext *self)
{
#if defined(__cplusplus)
	self->ClearState();
#else
...
_SOKOL_PRIVATE D3D11_USAGE
_sg_d3d11_usage(sg_usage usg)
{
	switch(usg) {
...
_SOKOL_PRIVATE D3D11_USAGE
_sg_d3d11_buffer_usage(sg_usage usg, sg_buffer_type type)
{
	switch(usg) {
	case SG_USAGE_IMMUTABLE:
...
_SOKOL_PRIVATE UINT
_sg_d3d11_buffer_bind_flags(sg_usage usg, sg_buffer_type t)
{
	switch(t) {
...
_SOKOL_PRIVATE UINT
_sg_d3d11_buffer_misc_flags(sg_buffer_type t)
{
	switch(t) {
	case SG_BUFFERTYPE_VERTEXBUFFER:
	case SG_BUFFERTYPE_INDEXBUFFER:
...
_SOKOL_PRIVATE UINT
_sg_d3d11_cpu_access_flags(sg_usage usg)
{
	switch(usg) {
	case SG_USAGE_IMMUTABLE:
		return 0;
...
_sg_d3d11_texture_pixel_format(sg_pixel_format fmt)
{
	switch(fmt) {
...
_SOKOL_PRIVATE DXGI_FORMAT
_sg_d3d11_index_format(sg_index_type index_type)
{
	switch(index_type) {
...
_SOKOL_PRIVATE D3D11_TEXTURE_ADDRESS_MODE
_sg_d3d11_address_mode(sg_wrap m)
{
	switch(m) {
...
_SOKOL_PRIVATE DXGI_FORMAT
_sg_d3d11_vertex_format(sg_vertex_format fmt)
{
	switch(fmt) {
...
_SOKOL_PRIVATE D3D11_CULL_MODE
_sg_d3d11_cull_mode(sg_cull_mode m)
{
	switch(m) {
...
_sg_d3d11_compare_func(sg_compare_func f)
{
	switch(f) {
...
_SOKOL_PRIVATE D3D11_STENCIL_OP
_sg_d3d11_stencil_op(sg_stencil_op op)
{
	switch(op) {
...
_SOKOL_PRIVATE D3D11_BLEND
_sg_d3d11_blend_factor(sg_blend_factor f)
{
	switch(f) {
...
_SOKOL_PRIVATE D3D11_BLEND_OP
_sg_d3d11_blend_op(sg_blend_op op)
{
	switch(op) {
...
_SOKOL_PRIVATE UINT8
_sg_d3d11_color_write_mask(sg_color_mask m)
{
	UINT8 res = 0;
	if(m & SG_COLORMASK_R) {
...
_SOKOL_PRIVATE void
_sg_d3d11_setup_backend(const sg_desc *desc)
{
	// assume _sg.d3d11 already is zero-initialized
	SOKOL_dbg_assert(desc);
	SOKOL_dbg_assert(desc->environment.d3d11.device);
	SOKOL_dbg_assert(desc->environment.d3d11.device_context);
...
_SOKOL_PRIVATE void
_sg_d3d11_discard_backend(void)
{
	SOKOL_dbg_assert(_sg.d3d11.valid);
	_sg.d3d11.valid = false;
}

_SOKOL_PRIVATE void
_sg_d3d11_clear_state(void)
{
...
_SOKOL_PRIVATE void
_sg_d3d11_reset_state_cache(void)
{
...
{
	SOKOL_dbg_assert(buf && desc);
	SOKOL_dbg_assert(!buf->d3d11.buf);
	...
	if(injected) {
		...
		if(buf->cmn.type == SG_BUFFERTYPE_STORAGEBUFFER) {
			SOKOL_dbg_assert(_sg_multiple_u64((uint64_t)buf->cmn.size, 4));
...
_SOKOL_PRIVATE void
_sg_d3d11_discard_buffer(_sg_buffer_t *buf)
{
	SOKOL_dbg_assert(buf);
	if(buf->d3d11.buf) {
...
_SOKOL_PRIVATE void
_sg_d3d11_fill_subres_data(const _sg_image_t *img, const sg_image_data *data)
{
	const int num_faces  = (img->cmn.type == SG_IMAGETYPE_CUBE) ? 6 : 1;
	...
	for(int face_index = 0; face_index < num_faces; face_index++) {
		for(int slice_index = 0; slice_index < num_slices; slice_index++) {
			for(int mip_index = 0; mip_index < img->cmn.num_mipmaps; mip_index++, subres_index++) {
				SOKOL_dbg_assert(subres_index < (SG_MAX_MIPMAPS * SG_MAX_TEXTUREARRAY_LAYERS));
...
{
	SOKOL_dbg_assert(img && desc);
	SOKOL_dbg_assert((0 == img->d3d11.tex2d) && (0 == img->d3d11.tex3d) && (0 == img->d3d11.res) && (0 == img->d3d11.srv));
	HRESULT hr;

	const bool injected = (0 != desc->d3d11_texture);
	const bool msaa     = (img->cmn.sample_count > 1);
	SOKOL_dbg_assert(!(msaa && (img->cmn.type == SG_IMAGETYPE_CUBE)));
	...
	if(img->cmn.type != SG_IMAGETYPE_3D) {
		...
		SOKOL_dbg_assert(img->d3d11.tex2d);
		...
		SOKOL_dbg_assert(img->d3d11.tex3d);
...
_SOKOL_PRIVATE void
_sg_d3d11_discard_image(_sg_image_t *img)
{
	SOKOL_dbg_assert(img);
	if(img->d3d11.tex2d) {
...
{
	SOKOL_dbg_assert(smp && desc);
	SOKOL_dbg_assert(0 == smp->d3d11.smp);
...
_SOKOL_PRIVATE void
_sg_d3d11_discard_sampler(_sg_sampler_t *smp)
{
	SOKOL_dbg_assert(smp);
	if(smp->d3d11.smp) {
...
_SOKOL_PRIVATE bool
_sg_d3d11_load_d3dcompiler_dll(void)
{
	if((0 == _sg.d3d11.d3dcompiler_dll) && !_sg.d3d11.d3dcompiler_dll_load_failed) {
		...
		SOKOL_dbg_assert(_sg.d3d11.D3DCompile_func);
	}
	return 0 != _sg.d3d11.d3dcompiler_dll;
}

_SOKOL_PRIVATE ID3DBlob *
_sg_d3d11_compile_shader(const sg_shader_function *shd_func)
{
	if(!_sg_d3d11_load_d3dcompiler_dll()) {
		return NULL;
	}
	SOKOL_dbg_assert(shd_func->d3d11_target);
...
_SOKOL_PRIVATE bool
_sg_d3d11_ensure_hlsl_bindslot_ranges(const sg_shader_desc *desc)
{
	SOKOL_dbg_assert(desc);
	for(size_t i = 0; i < SG_MAX_UNIFORMBLOCK_BINDSLOTS; i++) {
...
{
	SOKOL_dbg_assert(shd && desc);
	SOKOL_dbg_assert(!shd->d3d11.vs && !shd->d3d11.fs && !shd->d3d11.cs && !shd->d3d11.vs_blob);
	HRESULT hr;
	...
	for(size_t ub_index = 0; ub_index < SG_MAX_UNIFORMBLOCK_BINDSLOTS; ub_index++) {
		...
		SOKOL_dbg_assert(d3d11_slot < _SG_D3D11_MAX_STAGE_UB_BINDINGS);
		if(stage == SG_SHADERSTAGE_VERTEX) {
			SOKOL_dbg_assert(0 == shd->d3d11.vs_cbufs[d3d11_slot]);
			shd->d3d11.vs_cbufs[d3d11_slot] = cbuf;
		} else if(stage == SG_SHADERSTAGE_FRAGMENT) {
			SOKOL_dbg_assert(0 == shd->d3d11.fs_cbufs[d3d11_slot]);
			shd->d3d11.fs_cbufs[d3d11_slot] = cbuf;
		} else if(stage == SG_SHADERSTAGE_COMPUTE) {
			SOKOL_dbg_assert(0 == shd->d3d11.cs_cbufs[d3d11_slot]);
	...
	if(has_vs) {
		...
		if(desc->vertex_func.bytecode.ptr) {
			SOKOL_dbg_assert(desc->vertex_func.bytecode.size > 0);
			vs_ptr    = desc->vertex_func.bytecode.ptr;
			vs_length = desc->vertex_func.bytecode.size;
		} else {
			SOKOL_dbg_assert(desc->vertex_func.source);
		...
		if(vs_valid) {
			...
			SOKOL_dbg_assert(shd->d3d11.vs_blob);
	...
	if(has_fs) {
		...
		if(desc->fragment_func.bytecode.ptr) {
			SOKOL_dbg_assert(desc->fragment_func.bytecode.size > 0);
			fs_ptr    = desc->fragment_func.bytecode.ptr;
			fs_length = desc->fragment_func.bytecode.size;
		} else {
			SOKOL_dbg_assert(desc->fragment_func.source);
	...
	if(has_cs) {
		...
		if(desc->compute_func.bytecode.ptr) {
			SOKOL_dbg_assert(desc->compute_func.bytecode.size > 0);
			cs_ptr    = desc->compute_func.bytecode.ptr;
			cs_length = desc->compute_func.bytecode.size;
		} else {
			SOKOL_dbg_assert(desc->compute_func.source);
...
_SOKOL_PRIVATE void
_sg_d3d11_discard_shader(_sg_shader_t *shd)
{
	SOKOL_dbg_assert(shd);
	if(shd->d3d11.vs) {
...
_sg_d3d11_create_pipeline(_sg_pipeline_t *pip, _sg_shader_t *shd, const sg_pipeline_desc *desc)
{
	SOKOL_dbg_assert(pip && shd && desc);
	SOKOL_dbg_assert(desc->shader.id == shd->slot.id);
	SOKOL_dbg_assert(shd->slot.state == SG_RESOURCESTATE_VALID);
	...
	// a render pipeline...
	SOKOL_dbg_assert(shd->d3d11.vs_blob && shd->d3d11.vs_blob_length > 0);
	SOKOL_dbg_assert(!pip->d3d11.il && !pip->d3d11.rs && !pip->d3d11.dss && !pip->d3d11.bs);
	...
	for(; attr_index < SG_MAX_VERTEX_ATTRIBUTES; attr_index++) {
		...
		SOKOL_dbg_assert(a_state->buffer_index < SG_MAX_VERTEXBUFFER_BINDSLOTS);
		SOKOL_dbg_assert(pip->cmn.vertex_buffer_layout_active[a_state->buffer_index]);
		const sg_vertex_buffer_layout_state *l_state = &desc->layout.buffers[a_state->buffer_index];
		const sg_vertex_step step_func               = l_state->step_func;
		const int step_rate                          = l_state->step_rate;
...
_SOKOL_PRIVATE void
_sg_d3d11_discard_pipeline(_sg_pipeline_t *pip)
{
	SOKOL_dbg_assert(pip);
...
_SOKOL_PRIVATE sg_resource_state
_sg_d3d11_create_attachments(_sg_attachments_t *atts, _sg_image_t **color_images, _sg_image_t **resolve_images, _sg_image_t *ds_img, const sg_attachments_desc *desc)
{
	SOKOL_dbg_assert(atts && desc);
	SOKOL_dbg_assert(color_images && resolve_images);
	SOKOL_dbg_assert(_sg.d3d11.dev);
	...
	for(size_t i = 0; i < (size_t)atts->cmn.num_colors; i++) {
		...
		_SOKOL_UNUSED(color_desc);
		SOKOL_dbg_assert(color_desc->image.id != SG_INVALID_ID);
		SOKOL_dbg_assert(0 == atts->d3d11.colors[i].image);
		SOKOL_dbg_assert(color_images[i] && (color_images[i]->slot.id == color_desc->image.id));
		SOKOL_dbg_assert(_sg_is_valid_rendertarget_color_format(color_images[i]->cmn.pixel_format));
		...
		const sg_attachment_desc *resolve_desc = &desc->resolves[i];
		if(resolve_desc->image.id != SG_INVALID_ID) {
			SOKOL_dbg_assert(0 == atts->d3d11.resolves[i].image);
			SOKOL_dbg_assert(resolve_images[i] && (resolve_images[i]->slot.id == resolve_desc->image.id));
			SOKOL_dbg_assert(color_images[i] && (color_images[i]->cmn.pixel_format == resolve_images[i]->cmn.pixel_format));
			atts->d3d11.resolves[i].image = resolve_images[i];
		}
	}
	SOKOL_dbg_assert(0 == atts->d3d11.depth_stencil.image);
	const sg_attachment_desc *ds_desc = &desc->depth_stencil;
	if(ds_desc->image.id != SG_INVALID_ID) {
		SOKOL_dbg_assert(ds_img && (ds_img->slot.id == ds_desc->image.id));
		SOKOL_dbg_assert(_sg_is_valid_rendertarget_depth_format(ds_img->cmn.pixel_format));
	...
	for(size_t i = 0; i < (size_t)atts->cmn.num_colors; i++) {
		...
		SOKOL_dbg_assert(0 == atts->d3d11.colors[i].view.rtv);
		...
		if(color_img->cmn.type == SG_IMAGETYPE_2D) {
			...
			SOKOL_dbg_assert(color_img->cmn.type == SG_IMAGETYPE_3D);
			SOKOL_dbg_assert(!msaa);
		...
		SOKOL_dbg_assert(color_img->d3d11.res);
	...
	SOKOL_dbg_assert(0 == atts->d3d11.depth_stencil.view.dsv);
	if(ds_desc->image.id != SG_INVALID_ID) {
		...
		SOKOL_dbg_assert(ds_img && ds_img->cmn.type != SG_IMAGETYPE_3D);
		...
		SOKOL_dbg_assert(ds_img->d3d11.res);
...
_SOKOL_PRIVATE void
_sg_d3d11_discard_attachments(_sg_attachments_t *atts)
{
	SOKOL_dbg_assert(atts);
...
_SOKOL_PRIVATE _sg_image_t *
_sg_d3d11_attachments_color_image(const _sg_attachments_t *atts, int index)
{
	SOKOL_dbg_assert(atts && (index >= 0) && (index < SG_MAX_COLOR_ATTACHMENTS));
...
_SOKOL_PRIVATE _sg_image_t *
_sg_d3d11_attachments_resolve_image(const _sg_attachments_t *atts, int index)
{
	SOKOL_dbg_assert(atts && (index >= 0) && (index < SG_MAX_COLOR_ATTACHMENTS));
...
_SOKOL_PRIVATE _sg_image_t *
_sg_d3d11_attachments_ds_image(const _sg_attachments_t *atts)
{
	SOKOL_dbg_assert(atts);
...
_SOKOL_PRIVATE void
_sg_d3d11_begin_pass(const sg_pass *pass)
{
	SOKOL_dbg_assert(pass);
	...
	if(atts) {
		...
		SOKOL_dbg_assert(swapchain->d3d11.render_view);
		num_rtvs                        = 1;
...
_SOKOL_PRIVATE UINT
_sg_d3d11_calcsubresource(UINT mip_slice, UINT array_slice, UINT mip_levels)
{
	return mip_slice + array_slice * mip_levels;
}

_SOKOL_PRIVATE void
_sg_d3d11_end_pass(void)
{
	SOKOL_dbg_assert(_sg.d3d11.ctx);

	if(!_sg.cur_pass.is_compute) {
		// need to resolve MSAA render attachments into texture?
		if(_sg.cur_pass.atts_id.id != SG_INVALID_ID) {
			// ...for offscreen pass...
			SOKOL_dbg_assert(_sg.cur_pass.atts && _sg.cur_pass.atts->slot.id == _sg.cur_pass.atts_id.id);
			for(size_t i = 0; i < (size_t)_sg.cur_pass.atts->cmn.num_colors; i++) {
				...
				if(resolve_img) {
					...
					SOKOL_dbg_assert(resolve_img->slot.id == cmn_resolve_att->image_id.id);
					SOKOL_dbg_assert(color_img && (color_img->slot.id == cmn_color_att->image_id.id));
					SOKOL_dbg_assert(color_img->cmn.sample_count > 1);
					SOKOL_dbg_assert(resolve_img->cmn.sample_count == 1);
			...
			if(_sg.d3d11.cur_pass.resolve_view) {
				SOKOL_dbg_assert(_sg.d3d11.cur_pass.render_view);
				SOKOL_dbg_assert(_sg.cur_pass.swapchain.sample_count > 1);
				SOKOL_dbg_assert(_sg.cur_pass.swapchain.color_fmt > SG_PIXELFORMAT_NONE);
				...
				SOKOL_dbg_assert(d3d11_render_res);
				SOKOL_dbg_assert(d3d11_resolve_res);
...
_SOKOL_PRIVATE void
_sg_d3d11_apply_viewport(int x, int y, int w, int h, bool origin_top_left)
{
	SOKOL_dbg_assert(_sg.d3d11.ctx);
	D3D11_VIEWPORT vp;
	vp.TopLeftX = (FLOAT)x;
...
_SOKOL_PRIVATE void
_sg_d3d11_apply_scissor_rect(int x, int y, int w, int h, bool origin_top_left)
{
	SOKOL_dbg_assert(_sg.d3d11.ctx);
	D3D11_RECT rect;
	rect.left   = x;
...
_SOKOL_PRIVATE void
_sg_d3d11_apply_pipeline(_sg_pipeline_t *pip)
{
	SOKOL_dbg_assert(pip);
	SOKOL_dbg_assert(pip->shader && (pip->cmn.shader_id.id == pip->shader->slot.id));
	SOKOL_dbg_assert(_sg.d3d11.ctx);
	...
	if(pip->cmn.is_compute) {
		// a compute pipeline
		SOKOL_dbg_assert(pip->shader->d3d11.cs);
		...
		// a render pipeline
		SOKOL_dbg_assert(pip->d3d11.rs && pip->d3d11.bs && pip->d3d11.dss);
		SOKOL_dbg_assert(pip->shader->d3d11.vs);
		SOKOL_dbg_assert(pip->shader->d3d11.fs);
...
_SOKOL_PRIVATE bool
_sg_d3d11_apply_bindings(_sg_bindings_t *bnd)
{
	SOKOL_dbg_assert(bnd);
	SOKOL_dbg_assert(bnd->pip && bnd->pip->shader);
	SOKOL_dbg_assert(bnd->pip->shader->slot.id == bnd->pip->cmn.shader_id.id);
	...
	for(size_t i = 0; i < SG_MAX_IMAGE_BINDSLOTS; i++) {
		...
		SOKOL_dbg_assert(stage != SG_SHADERSTAGE_NONE);
		const uint8_t d3d11_slot = shd->d3d11.img_register_t_n[i];
		SOKOL_dbg_assert(d3d11_slot < _SG_D3D11_MAX_STAGE_SRV_BINDINGS);
		SOKOL_dbg_assert(img->d3d11.srv);
	...
	for(size_t i = 0; i < SG_MAX_STORAGEBUFFER_BINDSLOTS; i++) {
		...
		SOKOL_dbg_assert(stage != SG_SHADERSTAGE_NONE);
		if(shd->cmn.storage_buffers[i].readonly) {
			SOKOL_dbg_assert(sbuf->d3d11.srv);
			...
			SOKOL_dbg_assert(sbuf->d3d11.uav);
			SOKOL_dbg_assert(stage == SG_SHADERSTAGE_COMPUTE);
			const uint8_t d3d11_slot = shd->d3d11.sbuf_register_u_n[i];
			SOKOL_dbg_assert(d3d11_slot < _SG_D3D11_MAX_STAGE_UAV_BINDINGS);
	...
	for(size_t i = 0; i < SG_MAX_SAMPLER_BINDSLOTS; i++) {
		...
		SOKOL_dbg_assert(d3d11_slot < _SG_D3D11_MAX_STAGE_SMP_BINDINGS);
		SOKOL_dbg_assert(smp->d3d11.smp);
...
_SOKOL_PRIVATE void
_sg_d3d11_apply_uniforms(int ub_slot, const sg_range *data)
{
	SOKOL_dbg_assert(_sg.d3d11.ctx);
	SOKOL_dbg_assert((ub_slot >= 0) && (ub_slot < SG_MAX_UNIFORMBLOCK_BINDSLOTS));
	SOKOL_dbg_assert(_sg.d3d11.cur_pipeline && _sg.d3d11.cur_pipeline->slot.id == _sg.d3d11.cur_pipeline_id.id);
	const _sg_shader_t *shd = _sg.d3d11.cur_pipeline->shader;
	SOKOL_dbg_assert(shd && (shd->slot.id == _sg.d3d11.cur_pipeline->cmn.shader_id.id));
	SOKOL_dbg_assert(data->size == shd->cmn.uniform_blocks[ub_slot].size);

	ID3D11Buffer *cbuf = shd->d3d11.all_cbufs[ub_slot];
	SOKOL_dbg_assert(cbuf);
...
_SOKOL_PRIVATE void
_sg_d3d11_draw(int base_element, int num_elements, int num_instances)
{
...
_SOKOL_PRIVATE void
_sg_d3d11_dispatch(int num_groups_x, int num_groups_y, int num_groups_z)
{
	_sg_d3d11_Dispatch(_sg.d3d11.ctx, (UINT)num_groups_x, (UINT)num_groups_y, (UINT)num_groups_z);
}

_SOKOL_PRIVATE void
_sg_d3d11_commit(void)
{
	// empty
}

_SOKOL_PRIVATE void
_sg_d3d11_update_buffer(_sg_buffer_t *buf, const sg_range *data)
{
	SOKOL_dbg_assert(buf && data && data->ptr && (data->size > 0));
	SOKOL_dbg_assert(_sg.d3d11.ctx);
	SOKOL_dbg_assert(buf->d3d11.buf);
...
_SOKOL_PRIVATE void
_sg_d3d11_append_buffer(_sg_buffer_t *buf, const sg_range *data, bool new_frame)
{
	SOKOL_dbg_assert(buf && data && data->ptr && (data->size > 0));
	SOKOL_dbg_assert(_sg.d3d11.ctx);
	SOKOL_dbg_assert(buf->d3d11.buf);
...
_SOKOL_PRIVATE void
_sg_d3d11_update_image(_sg_image_t *img, const sg_image_data *data)
{
	SOKOL_dbg_assert(img && data);
	SOKOL_dbg_assert(_sg.d3d11.ctx);
	SOKOL_dbg_assert(img->d3d11.res);
	const int num_faces        = (img->cmn.type == SG_IMAGETYPE_CUBE) ? 6 : 1;
	const int num_slices       = (img->cmn.type == SG_IMAGETYPE_ARRAY) ? img->cmn.num_slices : 1;
	...
	for(int face_index = 0; face_index < num_faces; face_index++) {
		for(int slice_index = 0; slice_index < num_slices; slice_index++) {
			for(int mip_index = 0; mip_index < img->cmn.num_mipmaps; mip_index++, subres_index++) {
				...
				const sg_range *subimg_data = &(data->subimage[face_index][mip_index]);
				const size_t slice_size     = subimg_data->size / (size_t)num_slices;
				SOKOL_dbg_assert(slice_size == (size_t)(src_depth_pitch * num_depth_slices));
				const size_t slice_offset = slice_size * (size_t)slice_index;
				...
				if(SUCCEEDED(hr)) {
					...
					for(int depth_index = 0; depth_index < num_depth_slices; depth_index++) {
						if(src_row_pitch == (int)d3d11_msr.RowPitch) {
							...
							SOKOL_dbg_assert((copy_size * (size_t)num_depth_slices) == slice_size);
							memcpy(dst_ptr, src_ptr, copy_size);
						} else {
							SOKOL_dbg_assert(src_row_pitch < (int)d3d11_msr.RowPitch);
...
//-- enum translation functions ------------------------------------------------
_SOKOL_PRIVATE MTLLoadAction
_sg_mtl_load_action(sg_load_action a)
{
	switch(a) {
...
_SOKOL_PRIVATE MTLStoreAction
_sg_mtl_store_action(sg_store_action a, bool resolve)
{
	switch(a) {
...
_SOKOL_PRIVATE MTLVertexFormat
_sg_mtl_vertex_format(sg_vertex_format fmt)
{
	switch(fmt) {
...
_SOKOL_PRIVATE MTLPrimitiveType
_sg_mtl_primitive_type(sg_primitive_type t)
{
	switch(t) {
...
_SOKOL_PRIVATE MTLPixelFormat
_sg_mtl_pixel_format(sg_pixel_format fmt)
{
	switch(fmt) {
...
_SOKOL_PRIVATE MTLColorWriteMask
_sg_mtl_color_write_mask(sg_color_mask m)
{
...
_SOKOL_PRIVATE MTLBlendOperation
_sg_mtl_blend_op(sg_blend_op op)
{
	switch(op) {
...
_SOKOL_PRIVATE MTLBlendFactor
_sg_mtl_blend_factor(sg_blend_factor f)
{
	switch(f) {
...
_SOKOL_PRIVATE MTLCompareFunction
_sg_mtl_compare_func(sg_compare_func f)
{
	switch(f) {
...
_SOKOL_PRIVATE MTLStencilOperation
_sg_mtl_stencil_op(sg_stencil_op op)
{
	switch(op) {
...
_SOKOL_PRIVATE MTLCullMode
_sg_mtl_cull_mode(sg_cull_mode m)
{
	switch(m) {
...
_SOKOL_PRIVATE MTLWinding
_sg_mtl_winding(sg_face_winding w)
{
	switch(w) {
...
_SOKOL_PRIVATE MTLIndexType
_sg_mtl_index_type(sg_index_type t)
{
	switch(t) {
...
_SOKOL_PRIVATE int
_sg_mtl_index_size(sg_index_type t)
{
	switch(t) {
	case SG_INDEXTYPE_NONE: return 0;
...
_SOKOL_PRIVATE MTLTextureType
_sg_mtl_texture_type(sg_image_type t)
{
	switch(t) {
...
_SOKOL_PRIVATE MTLSamplerAddressMode
_sg_mtl_address_mode(sg_wrap w)
{
...
_SOKOL_PRIVATE
API_AVAILABLE(ios(14.0), macos(12.0)) MTLSamplerBorderColor _sg_mtl_border_color(sg_border_color c)
{
	switch(c) {
...
_SOKOL_PRIVATE MTLSamplerMinMagFilter
_sg_mtl_minmag_filter(sg_filter f)
{
	switch(f) {
...
_SOKOL_PRIVATE MTLSamplerMipFilter
_sg_mtl_mipmap_filter(sg_filter f)
{
	switch(f) {
	case SG_FILTER_NEAREST:
...
_SOKOL_PRIVATE size_t
_sg_mtl_vertexbuffer_bindslot(size_t sokol_bindslot)
{
...
_SOKOL_PRIVATE void
_sg_mtl_init_pool(const sg_desc *desc)
{
	...
	SOKOL_dbg_assert([_sg.mtl.idpool.pool count] == (NSUInteger)_sg.mtl.idpool.num_slots);
...
_SOKOL_PRIVATE int
_sg_mtl_alloc_pool_slot(void)
{
	SOKOL_dbg_assert(_sg.mtl.idpool.free_queue_top > 0);
	const int slot_index = _sg.mtl.idpool.free_queue[--_sg.mtl.idpool.free_queue_top];
	SOKOL_dbg_assert((slot_index > 0) && (slot_index < _sg.mtl.idpool.num_slots));
	return slot_index;
}

// put a free resource pool slot back into the free-queue
_SOKOL_PRIVATE void
_sg_mtl_free_pool_slot(int slot_index)
{
	SOKOL_dbg_assert(_sg.mtl.idpool.free_queue_top < _sg.mtl.idpool.num_slots);
	SOKOL_dbg_assert((slot_index > 0) && (slot_index < _sg.mtl.idpool.num_slots));
...
_sg_mtl_add_resource(id res)
{
	...
	SOKOL_dbg_assert([NSNull null] == _sg.mtl.idpool.pool[(NSUInteger)slot_index]);
...
*/
_SOKOL_PRIVATE void
_sg_mtl_release_resource(uint32_t frame_index, int slot_index)
{
	...
	// release queue full?
	SOKOL_dbg_assert(_sg.mtl.idpool.release_queue_front != _sg.mtl.idpool.release_queue_back);
	SOKOL_dbg_assert(0 == _sg.mtl.idpool.release_queue[release_index].frame_index);
...
_SOKOL_PRIVATE void
_sg_mtl_garbage_collect(uint32_t frame_index)
{
	while(_sg.mtl.idpool.release_queue_back != _sg.mtl.idpool.release_queue_front) {
		...
		SOKOL_dbg_assert(_sg.mtl.idpool.pool[(NSUInteger)slot_index] != [NSNull null]);
...
_SOKOL_PRIVATE id
_sg_mtl_id(int slot_index)
{
	return _sg.mtl.idpool.pool[(NSUInteger)slot_index];
}

_SOKOL_PRIVATE void
_sg_mtl_clear_state_cache(void)
{
...
_sg_mtl_init_caps(void)
{
	...
	if(@available(macOS 12.0, iOS 14.0, *)) {
		_sg.features.image_clamp_to_border = [_sg.mtl.device supportsFamily:MTLGPUFamilyApple7] || [_sg.mtl.device supportsFamily:MTLGPUFamilyMac2];
...
_SOKOL_PRIVATE void
_sg_mtl_setup_backend(const sg_desc *desc)
{
	// assume already zero-initialized
	SOKOL_dbg_assert(desc);
	SOKOL_dbg_assert(desc->environment.metal.device);
	SOKOL_dbg_assert(desc->uniform_buffer_size > 0);
...
_SOKOL_PRIVATE void
_sg_mtl_discard_backend(void)
{
	SOKOL_dbg_assert(_sg.mtl.valid);
...
_SOKOL_PRIVATE void
_sg_mtl_reset_state_cache(void)
{
	_sg_mtl_clear_state_cache();
}
...
_SOKOL_PRIVATE void
_sg_mtl_discard_buffer(_sg_buffer_t *buf)
{
	SOKOL_dbg_assert(buf);
	for(int slot = 0; slot < buf->cmn.num_slots; slot++) {
...
_sg_mtl_copy_image_data(const _sg_image_t *img, __unsafe_unretained id<MTLTexture> mtl_tex, const sg_image_data *data)
{
	...
	for(int face_index = 0; face_index < num_faces; face_index++) {
		for(int mip_index = 0; mip_index < img->cmn.num_mipmaps; mip_index++) {
			SOKOL_dbg_assert(data->subimage[face_index][mip_index].ptr);
			SOKOL_dbg_assert(data->subimage[face_index][mip_index].size > 0);
            ...
            */
			...
			for(int slice_index = 0; slice_index < num_slices; slice_index++) {
				...
				SOKOL_dbg_assert((slice_offset + bytes_per_slice) <= (int)data->subimage[face_index][mip_index].size);
...
_SOKOL_PRIVATE bool
_sg_mtl_init_texdesc_common(MTLTextureDescriptor *mtl_desc, _sg_image_t *img)
{
...
_SOKOL_PRIVATE void
_sg_mtl_init_texdesc_rt(MTLTextureDescriptor *mtl_desc, _sg_image_t *img)
{
	SOKOL_dbg_assert(img->cmn.render_target);
	_SOKOL_UNUSED(img);
...
_SOKOL_PRIVATE void
_sg_mtl_init_texdesc_rt_msaa(MTLTextureDescriptor *mtl_desc, _sg_image_t *img)
{
	SOKOL_dbg_assert(img->cmn.sample_count > 1);
	mtl_desc.usage           = MTLTextureUsageShaderRead | MTLTextureUsageRenderTarget;
	mtl_desc.resourceOptions = MTLResourceStorageModePrivate;
...
_sg_mtl_create_image(_sg_image_t *img, const sg_image_desc *desc)
{
	...
	for(int slot = 0; slot < img->cmn.num_slots; slot++) {
		...
		if(injected) {
			SOKOL_dbg_assert(desc->mtl_textures[slot]);
...
_SOKOL_PRIVATE void
_sg_mtl_discard_image(_sg_image_t *img)
{
	SOKOL_dbg_assert(img);
...
_sg_mtl_create_sampler(_sg_sampler_t *smp, const sg_sampler_desc *desc)
{
	...
	if(injected) {
		SOKOL_dbg_assert(desc->mtl_sampler);
...
_SOKOL_PRIVATE void
_sg_mtl_discard_sampler(_sg_sampler_t *smp)
{
	SOKOL_dbg_assert(smp);
	// it's valid to call release resource with a 'null resource'
...
_SOKOL_PRIVATE id<MTLLibrary>
_sg_mtl_compile_library(const char *src)
{
	NSError *err       = NULL;
...
_SOKOL_PRIVATE id<MTLLibrary>
_sg_mtl_library_from_bytecode(const void *ptr, size_t num_bytes)
{
	NSError *err             = NULL;
...
_SOKOL_PRIVATE bool
_sg_mtl_create_shader_func(const sg_shader_function *func, const char *label, const char *label_ext, _sg_mtl_shader_func_t *res)
{
	SOKOL_dbg_assert(res->mtl_lib == _SG_MTL_INVALID_SLOT_INDEX);
	SOKOL_dbg_assert(res->mtl_func == _SG_MTL_INVALID_SLOT_INDEX);
	id<MTLLibrary> mtl_lib = nil;
	if(func->bytecode.ptr) {
		SOKOL_dbg_assert(func->bytecode.size > 0);
...
_SOKOL_PRIVATE void
_sg_mtl_discard_shader_func(const _sg_mtl_shader_func_t *func)
{
...
_SOKOL_PRIVATE bool
_sg_mtl_ensure_msl_bindslot_ranges(const sg_shader_desc *desc)
{
	SOKOL_dbg_assert(desc);
	for(size_t i = 0; i < SG_MAX_UNIFORMBLOCK_BINDSLOTS; i++) {
...
_SOKOL_PRIVATE void
_sg_mtl_discard_shader(_sg_shader_t *shd)
{
	SOKOL_dbg_assert(shd);
	_sg_mtl_discard_shader_func(&shd->mtl.vertex_func);
	_sg_mtl_discard_shader_func(&shd->mtl.fragment_func);
...
_sg_mtl_create_pipeline(_sg_pipeline_t *pip, _sg_shader_t *shd, const sg_pipeline_desc *desc)
{
	...
	if(pip->cmn.is_compute) {
		...
		for(size_t i = 0; i < SG_MAX_STORAGEBUFFER_BINDSLOTS; i++) {
			const sg_shader_stage stage = shd->cmn.storage_buffers[i].stage;
			SOKOL_dbg_assert((stage != SG_SHADERSTAGE_VERTEX) && (stage != SG_SHADERSTAGE_FRAGMENT));
		...
		SOKOL_dbg_assert(shd->mtl.vertex_func.mtl_func != _SG_MTL_INVALID_SLOT_INDEX);
		rp_desc.vertexFunction = _sg_mtl_id(shd->mtl.vertex_func.mtl_func);
		SOKOL_dbg_assert(shd->mtl.fragment_func.mtl_func != _SG_MTL_INVALID_SLOT_INDEX);
		...
		for(NSUInteger i = 0; i < (NSUInteger)desc->color_count; i++) {
			SOKOL_dbg_assert(i < SG_MAX_COLOR_ATTACHMENTS);
		...
		for(size_t i = 0; i < SG_MAX_STORAGEBUFFER_BINDSLOTS; i++) {
			...
			SOKOL_dbg_assert(stage != SG_SHADERSTAGE_COMPUTE);
			if(stage == SG_SHADERSTAGE_VERTEX) {
				SOKOL_dbg_assert(shd->cmn.storage_buffers[i].readonly);
				rp_desc.vertexBuffers[mtl_slot].mutability = MTLMutabilityImmutable;
			} else if(stage == SG_SHADERSTAGE_FRAGMENT) {
				SOKOL_dbg_assert(shd->cmn.storage_buffers[i].readonly);
...
_SOKOL_PRIVATE void
_sg_mtl_discard_pipeline(_sg_pipeline_t *pip)
{
	SOKOL_dbg_assert(pip);
	// it's valid to call release resource with a 'null resource'
...
_SOKOL_PRIVATE sg_resource_state
_sg_mtl_create_attachments(_sg_attachments_t *atts, _sg_image_t **color_images, _sg_image_t **resolve_images, _sg_image_t *ds_img, const sg_attachments_desc *desc)
{
	SOKOL_dbg_assert(atts && desc);
	SOKOL_dbg_assert(color_images && resolve_images);
	...
	for(int i = 0; i < atts->cmn.num_colors; i++) {
		...
		_SOKOL_UNUSED(color_desc);
		SOKOL_dbg_assert(color_desc->image.id != SG_INVALID_ID);
		SOKOL_dbg_assert(0 == atts->mtl.colors[i].image);
		SOKOL_dbg_assert(color_images[i] && (color_images[i]->slot.id == color_desc->image.id));
		SOKOL_dbg_assert(_sg_is_valid_rendertarget_color_format(color_images[i]->cmn.pixel_format));
		...
		const sg_attachment_desc *resolve_desc = &desc->resolves[i];
		if(resolve_desc->image.id != SG_INVALID_ID) {
			SOKOL_dbg_assert(0 == atts->mtl.resolves[i].image);
			SOKOL_dbg_assert(resolve_images[i] && (resolve_images[i]->slot.id == resolve_desc->image.id));
			SOKOL_dbg_assert(color_images[i] && (color_images[i]->cmn.pixel_format == resolve_images[i]->cmn.pixel_format));
			atts->mtl.resolves[i].image = resolve_images[i];
		}
	}
	SOKOL_dbg_assert(0 == atts->mtl.depth_stencil.image);
	const sg_attachment_desc *ds_desc = &desc->depth_stencil;
	if(ds_desc->image.id != SG_INVALID_ID) {
		SOKOL_dbg_assert(ds_img && (ds_img->slot.id == ds_desc->image.id));
		SOKOL_dbg_assert(_sg_is_valid_rendertarget_depth_format(ds_img->cmn.pixel_format));
...
_SOKOL_PRIVATE void
_sg_mtl_discard_attachments(_sg_attachments_t *atts)
{
	SOKOL_dbg_assert(atts);
	_SOKOL_UNUSED(atts);
}
...
_SOKOL_PRIVATE _sg_image_t *
_sg_mtl_attachments_color_image(const _sg_attachments_t *atts, int index)
{
...
_SOKOL_PRIVATE _sg_image_t *
_sg_mtl_attachments_resolve_image(const _sg_attachments_t *atts, int index)
{
	// NOTE: may return null
	SOKOL_dbg_assert(atts && (index >= 0) && (index < SG_MAX_COLOR_ATTACHMENTS));
...
_SOKOL_PRIVATE _sg_image_t *
_sg_mtl_attachments_ds_image(const _sg_attachments_t *atts)
{
	// NOTE: may return null
	SOKOL_dbg_assert(atts);
...
_SOKOL_PRIVATE void
_sg_mtl_bind_uniform_buffers(void)
{
	...
	if(_sg.cur_pass.is_compute) {
		SOKOL_dbg_assert(nil != _sg.mtl.compute_cmd_encoder);
		...
		SOKOL_dbg_assert(nil != _sg.mtl.render_cmd_encoder);
		for(size_t slot = 0; slot < SG_MAX_UNIFORMBLOCK_BINDSLOTS; slot++) {
...
_SOKOL_PRIVATE void
_sg_mtl_begin_compute_pass(const sg_pass *pass)
{
	SOKOL_dbg_assert(pass);
	(void)pass;
	SOKOL_dbg_assert(nil != _sg.mtl.cmd_buffer);
	SOKOL_dbg_assert(nil == _sg.mtl.compute_cmd_encoder);
	SOKOL_dbg_assert(nil == _sg.mtl.render_cmd_encoder);
...
_SOKOL_PRIVATE void
_sg_mtl_begin_render_pass(const sg_pass *pass)
{
	SOKOL_dbg_assert(pass);
	SOKOL_dbg_assert(nil != _sg.mtl.cmd_buffer);
	SOKOL_dbg_assert(nil == _sg.mtl.render_cmd_encoder);
	...
	SOKOL_dbg_assert(pass_desc);
	if(atts) {
		// setup pass descriptor for offscreen rendering
		SOKOL_dbg_assert(atts->slot.state == SG_RESOURCESTATE_VALID);
		for(NSUInteger i = 0; i < (NSUInteger)atts->cmn.num_colors; i++) {
			...
			SOKOL_dbg_assert(color_att_img->slot.state == SG_RESOURCESTATE_VALID);
			SOKOL_dbg_assert(color_att_img->slot.id == cmn_color_att->image_id.id);
			SOKOL_dbg_assert(color_att_img->mtl.tex[color_att_img->cmn.active_slot] != _SG_MTL_INVALID_SLOT_INDEX);
			...
			if(resolve_att_img) {
				SOKOL_dbg_assert(resolve_att_img->slot.state == SG_RESOURCESTATE_VALID);
				SOKOL_dbg_assert(resolve_att_img->slot.id == cmn_resolve_att->image_id.id);
				SOKOL_dbg_assert(resolve_att_img->mtl.tex[resolve_att_img->cmn.active_slot] != _SG_MTL_INVALID_SLOT_INDEX);
		...
		if(0 != ds_att_img) {
			SOKOL_dbg_assert(ds_att_img->slot.state == SG_RESOURCESTATE_VALID);
			SOKOL_dbg_assert(ds_att_img->slot.id == atts->cmn.depth_stencil.image_id.id);
			SOKOL_dbg_assert(ds_att_img->mtl.tex[ds_att_img->cmn.active_slot] != _SG_MTL_INVALID_SLOT_INDEX);
...
_SOKOL_PRIVATE void
_sg_mtl_begin_pass(const sg_pass *pass)
{
	SOKOL_dbg_assert(pass);
	SOKOL_dbg_assert(_sg.mtl.cmd_queue);
	SOKOL_dbg_assert(nil == _sg.mtl.compute_cmd_encoder);
	SOKOL_dbg_assert(nil == _sg.mtl.render_cmd_encoder);
	SOKOL_dbg_assert(nil == _sg.mtl.cur_drawable);
	_sg_mtl_clear_state_cache();
...
_SOKOL_PRIVATE void
_sg_mtl_apply_viewport(int x, int y, int w, int h, bool origin_top_left)
{
	SOKOL_dbg_assert(nil != _sg.mtl.render_cmd_encoder);
	SOKOL_dbg_assert(_sg.cur_pass.height > 0);
	MTLViewport vp;
	vp.originX = (double)x;
...
_SOKOL_PRIVATE void
_sg_mtl_apply_scissor_rect(int x, int y, int w, int h, bool origin_top_left)
{
	SOKOL_dbg_assert(nil != _sg.mtl.render_cmd_encoder);
	SOKOL_dbg_assert(_sg.cur_pass.width > 0);
	SOKOL_dbg_assert(_sg.cur_pass.height > 0);
...
_SOKOL_PRIVATE void
_sg_mtl_apply_pipeline(_sg_pipeline_t *pip)
{
	SOKOL_dbg_assert(pip);
	SOKOL_dbg_assert(pip->shader && (pip->cmn.shader_id.id == pip->shader->slot.id));
	if(_sg.mtl.state_cache.cur_pipeline_id.id != pip->slot.id) {
		...
		if(pip->cmn.is_compute) {
			SOKOL_dbg_assert(_sg.cur_pass.is_compute);
			SOKOL_dbg_assert(nil != _sg.mtl.compute_cmd_encoder);
			SOKOL_dbg_assert(pip->mtl.cps != _SG_MTL_INVALID_SLOT_INDEX);
			...
			SOKOL_dbg_assert(pip->mtl.rps != _SG_MTL_INVALID_SLOT_INDEX);
			...
			SOKOL_dbg_assert(pip->mtl.dss != _SG_MTL_INVALID_SLOT_INDEX);
...
_SOKOL_PRIVATE bool
_sg_mtl_apply_bindings(_sg_bindings_t *bnd)
{
	SOKOL_dbg_assert(bnd);
	SOKOL_dbg_assert(bnd->pip);
	SOKOL_dbg_assert(bnd->pip && bnd->pip->shader);
	...
	if(!_sg.cur_pass.is_compute) {
		...
		if(bnd->ib) {
			SOKOL_dbg_assert(bnd->pip->cmn.index_type != SG_INDEXTYPE_NONE);
		...
		for(size_t i = 0; i < SG_MAX_VERTEXBUFFER_BINDSLOTS; i++) {
			...
			const NSUInteger mtl_slot = _sg_mtl_vertexbuffer_bindslot(i);
			SOKOL_dbg_assert(mtl_slot < _SG_MTL_MAX_STAGE_BUFFER_BINDINGS);
			...
			if((_sg.mtl.state_cache.cur_vs_buffer_ids[mtl_slot].id != vb->slot.id) ||
				...
				if(_sg.mtl.state_cache.cur_vs_buffer_ids[mtl_slot].id != vb->slot.id) {
					...
					SOKOL_dbg_assert(vb->mtl.buf[vb->cmn.active_slot] != _SG_MTL_INVALID_SLOT_INDEX);
	...
	for(size_t i = 0; i < SG_MAX_IMAGE_BINDSLOTS; i++) {
		...
		SOKOL_dbg_assert(img->mtl.tex[img->cmn.active_slot] != _SG_MTL_INVALID_SLOT_INDEX);
		const sg_shader_stage stage = shd->cmn.images[i].stage;
		SOKOL_dbg_assert((stage == SG_SHADERSTAGE_VERTEX) || (stage == SG_SHADERSTAGE_FRAGMENT) || (stage == SG_SHADERSTAGE_COMPUTE));
		const NSUInteger mtl_slot = shd->mtl.img_texture_n[i];
		SOKOL_dbg_assert(mtl_slot < _SG_MTL_MAX_STAGE_IMAGE_BINDINGS);
		if(stage == SG_SHADERSTAGE_VERTEX) {
			SOKOL_dbg_assert(nil != _sg.mtl.render_cmd_encoder);
	...
	for(size_t i = 0; i < SG_MAX_SAMPLER_BINDSLOTS; i++) {
		...
		SOKOL_dbg_assert(smp->mtl.sampler_state != _SG_MTL_INVALID_SLOT_INDEX);
		const sg_shader_stage stage = shd->cmn.samplers[i].stage;
		SOKOL_dbg_assert((stage == SG_SHADERSTAGE_VERTEX) || (stage == SG_SHADERSTAGE_FRAGMENT) || (stage == SG_SHADERSTAGE_COMPUTE));
		const NSUInteger mtl_slot = shd->mtl.smp_sampler_n[i];
		SOKOL_dbg_assert(mtl_slot < _SG_MTL_MAX_STAGE_SAMPLER_BINDINGS);
		if(stage == SG_SHADERSTAGE_VERTEX) {
			SOKOL_dbg_assert(nil != _sg.mtl.render_cmd_encoder);
	...
	for(size_t i = 0; i < SG_MAX_STORAGEBUFFER_BINDSLOTS; i++) {
		...
		SOKOL_dbg_assert(sbuf->mtl.buf[sbuf->cmn.active_slot] != _SG_MTL_INVALID_SLOT_INDEX);
		const sg_shader_stage stage = shd->cmn.storage_buffers[i].stage;
		SOKOL_dbg_assert((stage == SG_SHADERSTAGE_VERTEX) || (stage == SG_SHADERSTAGE_FRAGMENT) || (stage == SG_SHADERSTAGE_COMPUTE));
		const NSUInteger mtl_slot = shd->mtl.sbuf_buffer_n[i];
		SOKOL_dbg_assert(mtl_slot < _SG_MTL_MAX_STAGE_UB_SBUF_BINDINGS);
		if(stage == SG_SHADERSTAGE_VERTEX) {
			SOKOL_dbg_assert(nil != _sg.mtl.render_cmd_encoder);
...
_SOKOL_PRIVATE void
_sg_mtl_apply_uniforms(int ub_slot, const sg_range *data)
{
	SOKOL_dbg_assert((ub_slot >= 0) && (ub_slot < SG_MAX_UNIFORMBLOCK_BINDSLOTS));
	SOKOL_dbg_assert(((size_t)_sg.mtl.cur_ub_offset + data->size) <= (size_t)_sg.mtl.ub_size);
	SOKOL_dbg_assert((_sg.mtl.cur_ub_offset & (_SG_MTL_UB_ALIGN - 1)) == 0);
	const _sg_pipeline_t *pip = _sg.mtl.state_cache.cur_pipeline;
	SOKOL_dbg_assert(pip && pip->shader);
	SOKOL_dbg_assert(pip->slot.id == _sg.mtl.state_cache.cur_pipeline_id.id);
	const _sg_shader_t *shd = pip->shader;
	SOKOL_dbg_assert(shd->slot.id == pip->cmn.shader_id.id);
	SOKOL_dbg_assert(data->size == shd->cmn.uniform_blocks[ub_slot].size);
...
_SOKOL_PRIVATE void
_sg_mtl_draw(int base_element, int num_elements, int num_instances)
{
	SOKOL_dbg_assert(nil != _sg.mtl.render_cmd_encoder);
	SOKOL_dbg_assert(_sg.mtl.state_cache.cur_pipeline && (_sg.mtl.state_cache.cur_pipeline->slot.id == _sg.mtl.state_cache.cur_pipeline_id.id));
	if(SG_INDEXTYPE_NONE != _sg.mtl.state_cache.cur_pipeline->cmn.index_type) {
		// indexed rendering
		SOKOL_dbg_assert(_sg.mtl.state_cache.cur_indexbuffer && (_sg.mtl.state_cache.cur_indexbuffer->slot.id == _sg.mtl.state_cache.cur_indexbuffer_id.id));
		const _sg_buffer_t *ib = _sg.mtl.state_cache.cur_indexbuffer;
		SOKOL_dbg_assert(ib->mtl.buf[ib->cmn.active_slot] != _SG_MTL_INVALID_SLOT_INDEX);
...
_SOKOL_PRIVATE void
_sg_mtl_dispatch(int num_groups_x, int num_groups_y, int num_groups_z)
{
	SOKOL_dbg_assert(nil != _sg.mtl.compute_cmd_encoder);
	SOKOL_dbg_assert(_sg.mtl.state_cache.cur_pipeline && (_sg.mtl.state_cache.cur_pipeline->slot.id == _sg.mtl.state_cache.cur_pipeline_id.id));
...
_SOKOL_PRIVATE void
_sg_mtl_update_buffer(_sg_buffer_t *buf, const sg_range *data)
{
	SOKOL_dbg_assert(buf && data && data->ptr && (data->size > 0));
	if(++buf->cmn.active_slot >= buf->cmn.num_slots) {
		buf->cmn.active_slot = 0;
...
_SOKOL_PRIVATE void
_sg_mtl_append_buffer(_sg_buffer_t *buf, const sg_range *data, bool new_frame)
{
	SOKOL_dbg_assert(buf && data && data->ptr && (data->size > 0));
	if(new_frame) {
		if(++buf->cmn.active_slot >= buf->cmn.num_slots) {
...
_SOKOL_PRIVATE void
_sg_mtl_update_image(_sg_image_t *img, const sg_image_data *data)
{
	SOKOL_dbg_assert(img && data);
	if(++img->cmn.active_slot >= img->cmn.num_slots) {
		img->cmn.active_slot = 0;
...
_SOKOL_PRIVATE void
_sg_mtl_push_debug_group(const char *name)
{
	SOKOL_dbg_assert(name);
...
_SOKOL_PRIVATE WGPUStringView
_sg_wgpu_stringview(const char *str)
{
	WGPUStringView res;
	if(str) {
		res.data   = str;
...
_SOKOL_PRIVATE WGPUOptionalBool
_sg_wgpu_optional_bool(bool b)
{
...
_SOKOL_PRIVATE WGPUBufferUsage
_sg_wgpu_buffer_usage(sg_buffer_type t, sg_usage u)
{
...
_SOKOL_PRIVATE WGPULoadOp
_sg_wgpu_load_op(WGPUTextureView view, sg_load_action a)
{
	if(0 == view) {
...
_SOKOL_PRIVATE WGPUStoreOp
_sg_wgpu_store_op(WGPUTextureView view, sg_store_action a)
{
	if(0 == view) {
...
_SOKOL_PRIVATE WGPUAddressMode
_sg_wgpu_sampler_address_mode(sg_wrap m)
{
	switch(m) {
	case SG_WRAP_REPEAT:
...
_SOKOL_PRIVATE WGPUFilterMode
_sg_wgpu_sampler_minmag_filter(sg_filter f)
{
	switch(f) {
	case SG_FILTER_NEAREST:
...
_sg_wgpu_indexformat(sg_index_type t)
{
...
_SOKOL_PRIVATE WGPUVertexStepMode
_sg_wgpu_stepmode(sg_vertex_step s)
{
	return (s == SG_VERTEXSTEP_PER_VERTEX) ? WGPUVertexStepMode_Vertex : WGPUVertexStepMode_Instance;
}

_SOKOL_PRIVATE WGPUVertexFormat
_sg_wgpu_vertexformat(sg_vertex_format f)
{
	switch(f) {
...
_SOKOL_PRIVATE WGPUPrimitiveTopology
_sg_wgpu_topology(sg_primitive_type t)
{
	switch(t) {
...
_SOKOL_PRIVATE WGPUFrontFace
_sg_wgpu_frontface(sg_face_winding fw)
{
	return (fw == SG_FACEWINDING_CCW) ? WGPUFrontFace_CCW : WGPUFrontFace_CW;
}

_SOKOL_PRIVATE WGPUCullMode
_sg_wgpu_cullmode(sg_cull_mode cm)
{
	switch(cm) {
...
_sg_wgpu_textureformat(sg_pixel_format p)
{
	switch(p) {
...
_SOKOL_PRIVATE WGPUCompareFunction
_sg_wgpu_comparefunc(sg_compare_func f)
{
	switch(f) {
...
_SOKOL_PRIVATE WGPUStencilOperation
_sg_wgpu_stencilop(sg_stencil_op op)
{
	switch(op) {
...
_SOKOL_PRIVATE WGPUBlendOperation
_sg_wgpu_blendop(sg_blend_op op)
{
	switch(op) {
...
_SOKOL_PRIVATE WGPUBlendFactor
_sg_wgpu_blendfactor(sg_blend_factor f)
{
	switch(f) {
...
_SOKOL_PRIVATE WGPUColorWriteMask
_sg_wgpu_colorwritemask(uint8_t m)
{
...
_SOKOL_PRIVATE WGPUShaderStage
_sg_wgpu_shader_stage(sg_shader_stage stage)
{
	switch(stage) {
...
_SOKOL_PRIVATE void
_sg_wgpu_uniform_buffer_init(const sg_desc *desc)
{
	SOKOL_dbg_assert(0 == _sg.wgpu.uniform.staging);
	SOKOL_dbg_assert(0 == _sg.wgpu.uniform.buf);
	...
	SOKOL_dbg_assert(_sg.wgpu.uniform.buf);
}

_SOKOL_PRIVATE void
_sg_wgpu_uniform_buffer_discard(void)
{
...
_SOKOL_PRIVATE void
_sg_wgpu_uniform_buffer_on_commit(void)
{
...
_SOKOL_PRIVATE void
_sg_wgpu_bindgroups_pool_init(const sg_desc *desc)
{
	SOKOL_dbg_assert((desc->wgpu_bindgroups_cache_size > 0) && (desc->wgpu_bindgroups_cache_size < _SG_MAX_POOL_SIZE));
	_sg_wgpu_bindgroups_pool_t *p = &_sg.wgpu.bindgroups_pool;
	SOKOL_dbg_assert(0 == p->bindgroups);
...
_SOKOL_PRIVATE void
_sg_wgpu_bindgroups_pool_discard(void)
{
	_sg_wgpu_bindgroups_pool_t *p = &_sg.wgpu.bindgroups_pool;
	SOKOL_dbg_assert(p->bindgroups);
	_sg_free(p->bindgroups);
	p->bindgroups = 0;
	_sg_pool_discard(&p->pool);
}

_SOKOL_PRIVATE _sg_wgpu_bindgroup_t *
_sg_wgpu_bindgroup_at(uint32_t bg_id)
{
	SOKOL_dbg_assert(SG_INVALID_ID != bg_id);
	_sg_wgpu_bindgroups_pool_t *p = &_sg.wgpu.bindgroups_pool;
	int slot_index                = _sg_slot_index(bg_id);
	SOKOL_dbg_assert((slot_index > _SG_INVALID_SLOT_INDEX) && (slot_index < p->pool.size));
	return &p->bindgroups[slot_index];
}

_SOKOL_PRIVATE _sg_wgpu_bindgroup_t *
_sg_wgpu_lookup_bindgroup(uint32_t bg_id)
{
...
_SOKOL_PRIVATE _sg_wgpu_bindgroup_handle_t
_sg_wgpu_alloc_bindgroup(void)
{
	_sg_wgpu_bindgroups_pool_t *p = &_sg.wgpu.bindgroups_pool;
...
_SOKOL_PRIVATE void
_sg_wgpu_dealloc_bindgroup(_sg_wgpu_bindgroup_t *bg)
{
	SOKOL_dbg_assert(bg && (bg->slot.state == SG_RESOURCESTATE_ALLOC) && (bg->slot.id != SG_INVALID_ID));
	_sg_wgpu_bindgroups_pool_t *p = &_sg.wgpu.bindgroups_pool;
...
_SOKOL_PRIVATE void
_sg_wgpu_reset_bindgroup_to_alloc_state(_sg_wgpu_bindgroup_t *bg)
{
	SOKOL_dbg_assert(bg);
...
_SOKOL_PRIVATE uint64_t
_sg_wgpu_hash(const void *key, int len, uint64_t seed)
{
...
_SOKOL_PRIVATE uint64_t
_sg_wgpu_bindgroups_cache_item(_sg_wgpu_bindgroups_cache_item_type_t type, uint8_t wgpu_binding, uint32_t id)
{
...
_SOKOL_PRIVATE uint64_t
_sg_wgpu_bindgroups_cache_pip_item(uint32_t id)
{
...
_SOKOL_PRIVATE void
_sg_wgpu_init_bindgroups_cache_key(_sg_wgpu_bindgroups_cache_key_t *key, const _sg_bindings_t *bnd)
{
	SOKOL_dbg_assert(bnd);
	SOKOL_dbg_assert(bnd->pip);
	const _sg_shader_t *shd = bnd->pip->shader;
	SOKOL_dbg_assert(shd && shd->slot.id == bnd->pip->cmn.shader_id.id);
	...
	for(size_t i = 0; i < SG_MAX_IMAGE_BINDSLOTS; i++) {
		...
		SOKOL_dbg_assert(bnd->imgs[i]);
		const size_t item_idx = i + 1;
		SOKOL_dbg_assert(item_idx < _SG_WGPU_BINDGROUPSCACHEKEY_NUM_ITEMS);
		SOKOL_dbg_assert(0 == key->items[item_idx]);
...
_SOKOL_PRIVATE _sg_wgpu_bindgroup_t *
_sg_wgpu_create_bindgroup(_sg_bindings_t *bnd)
{
	SOKOL_dbg_assert(_sg.wgpu.dev);
	SOKOL_dbg_assert(bnd->pip);
	const _sg_shader_t *shd = bnd->pip->shader;
	SOKOL_dbg_assert(shd && (shd->slot.id == bnd->pip->cmn.shader_id.id));
	...
	for(size_t i = 0; i < SG_MAX_IMAGE_BINDSLOTS; i++) {
		...
		SOKOL_dbg_assert(bnd->imgs[i]);
		SOKOL_dbg_assert(bgl_index < _SG_WGPU_MAX_IMG_SMP_SBUF_BINDGROUP_ENTRIES);
...
_SOKOL_PRIVATE void
_sg_wgpu_discard_bindgroup(_sg_wgpu_bindgroup_t *bg)
{
	SOKOL_dbg_assert(bg);
	...
	if(bg->slot.state == SG_RESOURCESTATE_VALID) {
		...
		SOKOL_dbg_assert(bg->slot.state == SG_RESOURCESTATE_ALLOC);
	}
	if(bg->slot.state == SG_RESOURCESTATE_ALLOC) {
		_sg_wgpu_dealloc_bindgroup(bg);
		SOKOL_dbg_assert(bg->slot.state == SG_RESOURCESTATE_INITIAL);
	}
}

_SOKOL_PRIVATE void
_sg_wgpu_discard_all_bindgroups(void)
{
	_sg_wgpu_bindgroups_pool_t *p = &_sg.wgpu.bindgroups_pool;
...
_SOKOL_PRIVATE void
_sg_wgpu_bindgroups_cache_init(const sg_desc *desc)
{
	SOKOL_dbg_assert(desc);
	SOKOL_dbg_assert(_sg.wgpu.bindgroups_cache.num == 0);
	SOKOL_dbg_assert(_sg.wgpu.bindgroups_cache.index_mask == 0);
	SOKOL_dbg_assert(_sg.wgpu.bindgroups_cache.items == 0);
...
_SOKOL_PRIVATE void
_sg_wgpu_bindgroups_cache_discard(void)
{
...
_SOKOL_PRIVATE void
_sg_wgpu_bindgroups_cache_set(uint64_t hash, uint32_t bg_id)
{
	uint32_t index = hash & _sg.wgpu.bindgroups_cache.index_mask;
	SOKOL_dbg_assert(index < _sg.wgpu.bindgroups_cache.num);
	SOKOL_dbg_assert(_sg.wgpu.bindgroups_cache.items);
	_sg.wgpu.bindgroups_cache.items[index].id = bg_id;
}

_SOKOL_PRIVATE uint32_t
_sg_wgpu_bindgroups_cache_get(uint64_t hash)
{
	uint32_t index = hash & _sg.wgpu.bindgroups_cache.index_mask;
	SOKOL_dbg_assert(index < _sg.wgpu.bindgroups_cache.num);
	SOKOL_dbg_assert(_sg.wgpu.bindgroups_cache.items);
...
_SOKOL_PRIVATE void
_sg_wgpu_bindgroups_cache_invalidate(_sg_wgpu_bindgroups_cache_item_type_t type, uint32_t id)
{
	...
	for(uint32_t cache_item_idx = 0; cache_item_idx < _sg.wgpu.bindgroups_cache.num; cache_item_idx++) {
		...
		if(bg_id != SG_INVALID_ID) {
			...
			SOKOL_dbg_assert(bg && (bg->slot.state == SG_RESOURCESTATE_VALID));
...
_SOKOL_PRIVATE void
_sg_wgpu_bindings_cache_clear(void)
{
	memset(&_sg.wgpu.bindings_cache, 0, sizeof(_sg.wgpu.bindings_cache));
}

_SOKOL_PRIVATE bool
_sg_wgpu_bindings_cache_vb_dirty(size_t index, const _sg_buffer_t *vb, uint64_t offset)
{
	SOKOL_dbg_assert((index >= 0) && (index < SG_MAX_VERTEXBUFFER_BINDSLOTS));
	if(vb) {
		return (_sg.wgpu.bindings_cache.vbs[index].buffer.id != vb->slot.id) || (_sg.wgpu.bindings_cache.vbs[index].offset != offset);
	} else {
...
_SOKOL_PRIVATE void
_sg_wgpu_bindings_cache_vb_update(size_t index, const _sg_buffer_t *vb, uint64_t offset)
{
	SOKOL_dbg_assert((index >= 0) && (index < SG_MAX_VERTEXBUFFER_BINDSLOTS));
	if(vb) {
...
_SOKOL_PRIVATE bool
_sg_wgpu_bindings_cache_ib_dirty(const _sg_buffer_t *ib, uint64_t offset)
{
	if(ib) {
...
_SOKOL_PRIVATE void
_sg_wgpu_bindings_cache_ib_update(const _sg_buffer_t *ib, uint64_t offset)
{
	if(ib) {
...
_SOKOL_PRIVATE bool
_sg_wgpu_bindings_cache_bg_dirty(const _sg_wgpu_bindgroup_t *bg)
{
	if(bg) {
...
_SOKOL_PRIVATE void
_sg_wgpu_bindings_cache_bg_update(const _sg_wgpu_bindgroup_t *bg)
{
	if(bg) {
...
_SOKOL_PRIVATE void
_sg_wgpu_set_img_smp_sbuf_bindgroup(_sg_wgpu_bindgroup_t *bg)
{
	if(_sg_wgpu_bindings_cache_bg_dirty(bg)) {
		...
		if(_sg.cur_pass.is_compute) {
			SOKOL_dbg_assert(_sg.wgpu.cpass_enc);
			if(bg) {
				SOKOL_dbg_assert(bg->slot.state == SG_RESOURCESTATE_VALID);
				SOKOL_dbg_assert(bg->bindgroup);
			...
			SOKOL_dbg_assert(_sg.wgpu.rpass_enc);
			if(bg) {
				SOKOL_dbg_assert(bg->slot.state == SG_RESOURCESTATE_VALID);
				SOKOL_dbg_assert(bg->bindgroup);
...
_SOKOL_PRIVATE bool
_sg_wgpu_apply_bindgroup(_sg_bindings_t *bnd)
{
...
_SOKOL_PRIVATE bool
_sg_wgpu_apply_index_buffer(_sg_bindings_t *bnd)
{
	SOKOL_dbg_assert(_sg.wgpu.rpass_enc);
	...
	if(_sg_wgpu_bindings_cache_ib_dirty(ib, offset)) {
		...
		if(ib) {
			...
			SOKOL_dbg_assert(buf_size > offset);
...
_SOKOL_PRIVATE bool
_sg_wgpu_apply_vertex_buffers(_sg_bindings_t *bnd)
{
	SOKOL_dbg_assert(_sg.wgpu.rpass_enc);
...
_SOKOL_PRIVATE void
_sg_wgpu_setup_backend(const sg_desc *desc)
{
	SOKOL_dbg_assert(desc);
	SOKOL_dbg_assert(desc->environment.wgpu.device);
	SOKOL_dbg_assert(desc->uniform_buffer_size > 0);
	_sg.backend    = SG_BACKEND_WGPU;
	_sg.wgpu.valid = true;
	_sg.wgpu.dev   = (WGPUDevice)desc->environment.wgpu.device;
	_sg.wgpu.queue = wgpuDeviceGetQueue(_sg.wgpu.dev);
	SOKOL_dbg_assert(_sg.wgpu.queue);
	...
	SOKOL_dbg_assert(_sg.wgpu.cmd_enc);
}

_SOKOL_PRIVATE void
_sg_wgpu_discard_backend(void)
{
	SOKOL_dbg_assert(_sg.wgpu.valid);
	SOKOL_dbg_assert(_sg.wgpu.cmd_enc);
...
_SOKOL_PRIVATE void
_sg_wgpu_reset_state_cache(void)
{
	_sg_wgpu_bindings_cache_clear();
}
...
_sg_wgpu_create_buffer(_sg_buffer_t *buf, const sg_buffer_desc *desc)
{
	...
	if(injected) {
		...
		if(map_at_creation) {
			SOKOL_dbg_assert(desc->data.ptr && (desc->data.size > 0));
			SOKOL_dbg_assert(desc->data.size <= (size_t)buf->cmn.size);
			// FIXME: inefficient on WASM
			void *ptr = wgpuBufferGetMappedRange(buf->wgpu.buf, 0, wgpu_buf_size);
			SOKOL_dbg_assert(ptr);
...
_SOKOL_PRIVATE void
_sg_wgpu_discard_buffer(_sg_buffer_t *buf)
{
	SOKOL_dbg_assert(buf);
	if(buf->cmn.type == SG_BUFFERTYPE_STORAGEBUFFER) {
...
_SOKOL_PRIVATE void
_sg_wgpu_copy_buffer_data(const _sg_buffer_t *buf, uint64_t offset, const sg_range *data)
{
	SOKOL_dbg_assert((offset + data->size) <= (size_t)buf->cmn.size);
...
_SOKOL_PRIVATE void
_sg_wgpu_copy_image_data(const _sg_image_t *img, WGPUTexture wgpu_tex, const sg_image_data *data)
{
...
_SOKOL_PRIVATE void
_sg_wgpu_discard_image(_sg_image_t *img)
{
	SOKOL_dbg_assert(img);
...
_SOKOL_PRIVATE void
_sg_wgpu_discard_sampler(_sg_sampler_t *smp)
{
	SOKOL_dbg_assert(smp);
...
{
	SOKOL_dbg_assert(func);
	SOKOL_dbg_assert(func->source);
	SOKOL_dbg_assert(func->entry);
...
_SOKOL_PRIVATE void
_sg_wgpu_discard_shader_func(_sg_wgpu_shader_func_t *func)
{
	if(func->module) {
...
_SOKOL_PRIVATE int
_sg_wgpu_dynoffset_cmp(const void *a, const void *b)
{
...
_SOKOL_PRIVATE bool
_sg_wgpu_ensure_wgsl_bindslot_ranges(const sg_shader_desc *desc)
{
	SOKOL_dbg_assert(desc);
	for(size_t i = 0; i < SG_MAX_UNIFORMBLOCK_BINDSLOTS; i++) {
...
{
	SOKOL_dbg_assert(shd && desc);
	SOKOL_dbg_assert(shd->wgpu.vertex_func.module == 0);
	SOKOL_dbg_assert(shd->wgpu.fragment_func.module == 0);
	SOKOL_dbg_assert(shd->wgpu.compute_func.module == 0);
	SOKOL_dbg_assert(shd->wgpu.bgl_ub == 0);
	SOKOL_dbg_assert(shd->wgpu.bg_ub == 0);
	SOKOL_dbg_assert(shd->wgpu.bgl_img_smp_sbuf == 0);
	...
	SOKOL_dbg_assert(_SG_WGPU_MAX_UB_BINDGROUP_ENTRIES <= _SG_WGPU_MAX_IMG_SMP_SBUF_BINDGROUP_ENTRIES);
...
_SOKOL_PRIVATE void
_sg_wgpu_discard_shader(_sg_shader_t *shd)
{
	SOKOL_dbg_assert(shd);
	_sg_wgpu_discard_shader_func(&shd->wgpu.vertex_func);
	_sg_wgpu_discard_shader_func(&shd->wgpu.fragment_func);
...
_sg_wgpu_create_pipeline(_sg_pipeline_t *pip, _sg_shader_t *shd, const sg_pipeline_desc *desc)
{
	...
	SOKOL_dbg_assert(desc->shader.id == shd->slot.id);
	SOKOL_dbg_assert(shd->wgpu.bgl_ub);
	SOKOL_dbg_assert(shd->wgpu.bgl_img_smp_sbuf);
	pip->shader = shd;
	...
	SOKOL_dbg_assert(wgpu_pip_layout);
	...
	if(pip->cmn.is_compute) {
		...
		for(int va_idx = 0; va_idx < SG_MAX_VERTEX_ATTRIBUTES; va_idx++) {
			...
			SOKOL_dbg_assert(vb_idx < SG_MAX_VERTEXBUFFER_BINDSLOTS);
			SOKOL_dbg_assert(pip->cmn.vertex_buffer_layout_active[vb_idx]);
...
_SOKOL_PRIVATE void
_sg_wgpu_discard_pipeline(_sg_pipeline_t *pip)
{
	SOKOL_dbg_assert(pip);
...
_SOKOL_PRIVATE sg_resource_state
_sg_wgpu_create_attachments(_sg_attachments_t *atts, _sg_image_t **color_images, _sg_image_t **resolve_images, _sg_image_t *ds_img, const sg_attachments_desc *desc)
{
	SOKOL_dbg_assert(atts && desc);
	SOKOL_dbg_assert(color_images && resolve_images);
	...
	for(int i = 0; i < atts->cmn.num_colors; i++) {
		...
		_SOKOL_UNUSED(color_desc);
		SOKOL_dbg_assert(color_desc->image.id != SG_INVALID_ID);
		SOKOL_dbg_assert(0 == atts->wgpu.colors[i].image);
		SOKOL_dbg_assert(color_images[i] && (color_images[i]->slot.id == color_desc->image.id));
		SOKOL_dbg_assert(_sg_is_valid_rendertarget_color_format(color_images[i]->cmn.pixel_format));
		SOKOL_dbg_assert(color_images[i]->wgpu.tex);
		...
		const sg_attachment_desc *resolve_desc = &desc->resolves[i];
		if(resolve_desc->image.id != SG_INVALID_ID) {
			SOKOL_dbg_assert(0 == atts->wgpu.resolves[i].image);
			SOKOL_dbg_assert(resolve_images[i] && (resolve_images[i]->slot.id == resolve_desc->image.id));
			SOKOL_dbg_assert(color_images[i] && (color_images[i]->cmn.pixel_format == resolve_images[i]->cmn.pixel_format));
			SOKOL_dbg_assert(resolve_images[i]->wgpu.tex);
	...
	SOKOL_dbg_assert(0 == atts->wgpu.depth_stencil.image);
	const sg_attachment_desc *ds_desc = &desc->depth_stencil;
	if(ds_desc->image.id != SG_INVALID_ID) {
		SOKOL_dbg_assert(ds_img && (ds_img->slot.id == ds_desc->image.id));
		SOKOL_dbg_assert(_sg_is_valid_rendertarget_depth_format(ds_img->cmn.pixel_format));
		SOKOL_dbg_assert(ds_img->wgpu.tex);
...
_SOKOL_PRIVATE void
_sg_wgpu_discard_attachments(_sg_attachments_t *atts)
{
	SOKOL_dbg_assert(atts);
	for(int i = 0; i < atts->cmn.num_colors; i++) {
...
_SOKOL_PRIVATE _sg_image_t *
_sg_wgpu_attachments_color_image(const _sg_attachments_t *atts, int index)
{
	SOKOL_dbg_assert(atts && (index >= 0) && (index < SG_MAX_COLOR_ATTACHMENTS));
	// NOTE: may return null
...
_SOKOL_PRIVATE _sg_image_t *
_sg_wgpu_attachments_resolve_image(const _sg_attachments_t *atts, int index)
{
	SOKOL_dbg_assert(atts && (index >= 0) && (index < SG_MAX_COLOR_ATTACHMENTS));
	// NOTE: may return null
...
_SOKOL_PRIVATE _sg_image_t *
_sg_wgpu_attachments_ds_image(const _sg_attachments_t *atts)
{
	// NOTE: may return null
	SOKOL_dbg_assert(atts);
...
_SOKOL_PRIVATE void
_sg_wgpu_init_color_att(WGPURenderPassColorAttachment *wgpu_att, const sg_color_attachment_action *action, WGPUTextureView color_view, WGPUTextureView resolve_view)
{
...
_SOKOL_PRIVATE void
_sg_wgpu_init_ds_att(WGPURenderPassDepthStencilAttachment *wgpu_att, const sg_pass_action *action, sg_pixel_format fmt, WGPUTextureView view)
{
	wgpu_att->view            = view;
...
_SOKOL_PRIVATE void
_sg_wgpu_begin_compute_pass(const sg_pass *pass)
{
...
_SOKOL_PRIVATE void
_sg_wgpu_begin_render_pass(const sg_pass *pass)
{
	const _sg_attachments_t *atts = _sg.cur_pass.atts;
	const sg_swapchain *swapchain = &pass->swapchain;
	const sg_pass_action *action  = &pass->action;
	...
	if(atts) {
		...
		if(wgpu_depth_stencil_view) {
			SOKOL_dbg_assert(swapchain->depth_format > SG_PIXELFORMAT_NONE);
...
_SOKOL_PRIVATE void
_sg_wgpu_begin_pass(const sg_pass *pass)
{
	SOKOL_dbg_assert(pass);
	SOKOL_dbg_assert(_sg.wgpu.dev);
	SOKOL_dbg_assert(_sg.wgpu.cmd_enc);
	SOKOL_dbg_assert(0 == _sg.wgpu.rpass_enc);
	SOKOL_dbg_assert(0 == _sg.wgpu.cpass_enc);
...
_SOKOL_PRIVATE void
_sg_wgpu_apply_viewport(int x, int y, int w, int h, bool origin_top_left)
{
	SOKOL_dbg_assert(_sg.wgpu.rpass_enc);
...
_SOKOL_PRIVATE void
_sg_wgpu_apply_scissor_rect(int x, int y, int w, int h, bool origin_top_left)
{
	SOKOL_dbg_assert(_sg.wgpu.rpass_enc);
	const _sg_recti_t clip = _sg_clipi(x, y, w, h, _sg.cur_pass.width, _sg.cur_pass.height);
...
_SOKOL_PRIVATE void
_sg_wgpu_set_ub_bindgroup(const _sg_shader_t *shd)
{
	// NOTE: dynamic offsets must be in binding order, not in BindGroupEntry order
	SOKOL_dbg_assert(shd->wgpu.ub_num_dynoffsets < SG_MAX_UNIFORMBLOCK_BINDSLOTS);
	...
	for(size_t i = 0; i < SG_MAX_UNIFORMBLOCK_BINDSLOTS; i++) {
		...
		SOKOL_dbg_assert(dynoffset_index < shd->wgpu.ub_num_dynoffsets);
...
_SOKOL_PRIVATE void
_sg_wgpu_apply_pipeline(_sg_pipeline_t *pip)
{
	SOKOL_dbg_assert(pip);
	SOKOL_dbg_assert(pip->shader && (pip->shader->slot.id == pip->cmn.shader_id.id));
	_sg.wgpu.cur_pipeline       = pip;
	_sg.wgpu.cur_pipeline_id.id = pip->slot.id;
	if(pip->cmn.is_compute) {
		SOKOL_dbg_assert(_sg.cur_pass.is_compute);
		SOKOL_dbg_assert(pip->wgpu.cpip);
		SOKOL_dbg_assert(_sg.wgpu.cpass_enc);
		wgpuComputePassEncoderSetPipeline(_sg.wgpu.cpass_enc, pip->wgpu.cpip);
	} else {
		SOKOL_dbg_assert(!_sg.cur_pass.is_compute);
		SOKOL_dbg_assert(pip->wgpu.rpip);
		SOKOL_dbg_assert(_sg.wgpu.rpass_enc);
...
_SOKOL_PRIVATE bool
_sg_wgpu_apply_bindings(_sg_bindings_t *bnd)
{
	SOKOL_dbg_assert(bnd);
	SOKOL_dbg_assert(bnd->pip->shader && (bnd->pip->cmn.shader_id.id == bnd->pip->shader->slot.id));
	bool retval = true;
	if(!_sg.cur_pass.is_compute) {
...
_SOKOL_PRIVATE void
_sg_wgpu_apply_uniforms(int ub_slot, const sg_range *data)
{
	const uint32_t alignment = _sg.wgpu.limits.limits.minUniformBufferOffsetAlignment;
	SOKOL_dbg_assert(_sg.wgpu.uniform.staging);
	SOKOL_dbg_assert((ub_slot >= 0) && (ub_slot < SG_MAX_UNIFORMBLOCK_BINDSLOTS));
	SOKOL_dbg_assert((_sg.wgpu.uniform.offset + data->size) <= _sg.wgpu.uniform.num_bytes);
	SOKOL_dbg_assert((_sg.wgpu.uniform.offset & (alignment - 1)) == 0);
	const _sg_pipeline_t *pip = _sg.wgpu.cur_pipeline;
	SOKOL_dbg_assert(pip && pip->shader);
	SOKOL_dbg_assert(pip->slot.id == _sg.wgpu.cur_pipeline_id.id);
	const _sg_shader_t *shd = pip->shader;
	SOKOL_dbg_assert(shd->slot.id == pip->cmn.shader_id.id);
	SOKOL_dbg_assert(data->size == shd->cmn.uniform_blocks[ub_slot].size);
	SOKOL_dbg_assert(data->size <= _SG_WGPU_MAX_UNIFORM_UPDATE_SIZE);
...
_SOKOL_PRIVATE void
_sg_wgpu_draw(int base_element, int num_elements, int num_instances)
{
	SOKOL_dbg_assert(_sg.wgpu.rpass_enc);
	SOKOL_dbg_assert(_sg.wgpu.cur_pipeline && (_sg.wgpu.cur_pipeline->slot.id == _sg.wgpu.cur_pipeline_id.id));
...
_SOKOL_PRIVATE void
_sg_wgpu_dispatch(int num_groups_x, int num_groups_y, int num_groups_z)
{
	SOKOL_dbg_assert(_sg.wgpu.cpass_enc);
...
_SOKOL_PRIVATE void
_sg_wgpu_update_buffer(_sg_buffer_t *buf, const sg_range *data)
{
	SOKOL_dbg_assert(data && data->ptr && (data->size > 0));
	SOKOL_dbg_assert(buf);
	_sg_wgpu_copy_buffer_data(buf, 0, data);
}

_SOKOL_PRIVATE void
_sg_wgpu_append_buffer(_sg_buffer_t *buf, const sg_range *data, bool new_frame)
{
	SOKOL_dbg_assert(data && data->ptr && (data->size > 0));
...
_SOKOL_PRIVATE void
_sg_wgpu_update_image(_sg_image_t *img, const sg_image_data *data)
{
	SOKOL_dbg_assert(img && data);
...
// >>generic backend
static inline void
_sg_setup_backend(const sg_desc *desc)
{
#if defined(_SOKOL_ANY_GL)
...
_sg_create_buffer(_sg_buffer_t *buf, const sg_buffer_desc *desc)
{
#if defined(_SOKOL_ANY_GL)
...
static inline void
_sg_discard_buffer(_sg_buffer_t *buf)
{
#if defined(_SOKOL_ANY_GL)
...
static inline sg_resource_state
_sg_create_image(_sg_image_t *img, const sg_image_desc *desc)
{
#if defined(_SOKOL_ANY_GL)
...
static inline void
_sg_discard_image(_sg_image_t *img)
{
#if defined(_SOKOL_ANY_GL)
...
static inline sg_resource_state
_sg_create_sampler(_sg_sampler_t *smp, const sg_sampler_desc *desc)
{
#if defined(_SOKOL_ANY_GL)
...
static inline void
_sg_discard_sampler(_sg_sampler_t *smp)
{
#if defined(_SOKOL_ANY_GL)
...
static inline sg_resource_state
_sg_create_shader(_sg_shader_t *shd, const sg_shader_desc *desc)
{
#if defined(_SOKOL_ANY_GL)
...
static inline void
_sg_discard_shader(_sg_shader_t *shd)
{
#if defined(_SOKOL_ANY_GL)
...
static inline void
_sg_discard_pipeline(_sg_pipeline_t *pip)
{
#if defined(_SOKOL_ANY_GL)
...
static inline sg_resource_state
_sg_create_attachments(_sg_attachments_t *atts, _sg_image_t **color_images, _sg_image_t **resolve_images, _sg_image_t *ds_image, const sg_attachments_desc *desc)
{
#if defined(_SOKOL_ANY_GL)
...
static inline void
_sg_discard_attachments(_sg_attachments_t *atts)
{
#if defined(_SOKOL_ANY_GL)
...
static inline _sg_image_t *
_sg_attachments_color_image(const _sg_attachments_t *atts, int index)
{
#if defined(_SOKOL_ANY_GL)
...
static inline _sg_image_t *
_sg_attachments_resolve_image(const _sg_attachments_t *atts, int index)
{
#if defined(_SOKOL_ANY_GL)
...
static inline _sg_image_t *
_sg_attachments_ds_image(const _sg_attachments_t *atts)
{
#if defined(_SOKOL_ANY_GL)
...
static inline void
_sg_begin_pass(const sg_pass *pass)
{
#if defined(_SOKOL_ANY_GL)
...
static inline void
_sg_apply_viewport(int x, int y, int w, int h, bool origin_top_left)
{
#if defined(_SOKOL_ANY_GL)
...
static inline void
_sg_apply_scissor_rect(int x, int y, int w, int h, bool origin_top_left)
{
#if defined(_SOKOL_ANY_GL)
...
static inline void
_sg_apply_pipeline(_sg_pipeline_t *pip)
{
#if defined(_SOKOL_ANY_GL)
...
static inline bool
_sg_apply_bindings(_sg_bindings_t *bnd)
{
#if defined(_SOKOL_ANY_GL)
...
static inline void
_sg_apply_uniforms(int ub_slot, const sg_range *data)
{
#if defined(_SOKOL_ANY_GL)
...
static inline void
_sg_draw(int base_element, int num_elements, int num_instances)
{
#if defined(_SOKOL_ANY_GL)
...
static inline void
_sg_dispatch(int num_groups_x, int num_groups_y, int num_groups_z)
{
#if defined(_SOKOL_ANY_GL)
...
static inline void
_sg_update_buffer(_sg_buffer_t *buf, const sg_range *data)
{
#if defined(_SOKOL_ANY_GL)
...
static inline void
_sg_append_buffer(_sg_buffer_t *buf, const sg_range *data, bool new_frame)
{
#if defined(_SOKOL_ANY_GL)
...
static inline void
_sg_update_image(_sg_image_t *img, const sg_image_data *data)
{
#if defined(_SOKOL_ANY_GL)
...
static inline void
_sg_push_debug_group(const char *name)
{
#if defined(SOKOL_METAL)
...
// >>pool
_SOKOL_PRIVATE void
_sg_pool_init(_sg_pool_t *pool, int num)
{
	SOKOL_dbg_assert(pool && (num >= 1));
...
_SOKOL_PRIVATE void
_sg_pool_discard(_sg_pool_t *pool)
{
	SOKOL_dbg_assert(pool);
	SOKOL_dbg_assert(pool->free_queue);
	_sg_free(pool->free_queue);
	pool->free_queue = 0;
	SOKOL_dbg_assert(pool->gen_ctrs);
...
_SOKOL_PRIVATE int
_sg_pool_alloc_index(_sg_pool_t *pool)
{
	SOKOL_dbg_assert(pool);
	SOKOL_dbg_assert(pool->free_queue);
	if(pool->queue_top > 0) {
		int slot_index = pool->free_queue[--pool->queue_top];
		SOKOL_dbg_assert((slot_index > 0) && (slot_index < pool->size));
		return slot_index;
	} else {
		// pool exhausted
...
_SOKOL_PRIVATE void
_sg_pool_free_index(_sg_pool_t *pool, int slot_index)
{
	SOKOL_dbg_assert((slot_index > _SG_INVALID_SLOT_INDEX) && (slot_index < pool->size));
	SOKOL_dbg_assert(pool);
	SOKOL_dbg_assert(pool->free_queue);
	SOKOL_dbg_assert(pool->queue_top < pool->size);
#ifdef SOKOL_DEBUG
	// debug check against double-free
	for(int i = 0; i < pool->queue_top; i++) {
		SOKOL_dbg_assert(pool->free_queue[i] != slot_index);
	}
#endif
	pool->free_queue[pool->queue_top++] = slot_index;
	SOKOL_dbg_assert(pool->queue_top <= (pool->size - 1));
}

_SOKOL_PRIVATE void
_sg_slot_reset(_sg_slot_t *slot)
{
	SOKOL_dbg_assert(slot);
	_sg_clear(slot, sizeof(_sg_slot_t));
}

_SOKOL_PRIVATE void
_sg_reset_buffer_to_alloc_state(_sg_buffer_t *buf)
{
	SOKOL_dbg_assert(buf);
...
_SOKOL_PRIVATE void
_sg_reset_image_to_alloc_state(_sg_image_t *img)
{
	SOKOL_dbg_assert(img);
...
_SOKOL_PRIVATE void
_sg_reset_sampler_to_alloc_state(_sg_sampler_t *smp)
{
	SOKOL_dbg_assert(smp);
...
_SOKOL_PRIVATE void
_sg_reset_shader_to_alloc_state(_sg_shader_t *shd)
{
	SOKOL_dbg_assert(shd);
...
_SOKOL_PRIVATE void
_sg_reset_pipeline_to_alloc_state(_sg_pipeline_t *pip)
{
	SOKOL_dbg_assert(pip);
...
_SOKOL_PRIVATE void
_sg_reset_attachments_to_alloc_state(_sg_attachments_t *atts)
{
	SOKOL_dbg_assert(atts);
...
_SOKOL_PRIVATE void
_sg_setup_pools(_sg_pools_t *p, const sg_desc *desc)
{
	SOKOL_dbg_assert(p);
	SOKOL_dbg_assert(desc);
	// note: the pools here will have an additional item, since slot 0 is reserved
	SOKOL_dbg_assert((desc->buffer_pool_size > 0) && (desc->buffer_pool_size < _SG_MAX_POOL_SIZE));
	...
	SOKOL_dbg_assert((desc->image_pool_size > 0) && (desc->image_pool_size < _SG_MAX_POOL_SIZE));
	...
	SOKOL_dbg_assert((desc->sampler_pool_size > 0) && (desc->sampler_pool_size < _SG_MAX_POOL_SIZE));
	...
	SOKOL_dbg_assert((desc->shader_pool_size > 0) && (desc->shader_pool_size < _SG_MAX_POOL_SIZE));
	...
	SOKOL_dbg_assert((desc->pipeline_pool_size > 0) && (desc->pipeline_pool_size < _SG_MAX_POOL_SIZE));
	...
	SOKOL_dbg_assert((desc->attachments_pool_size > 0) && (desc->attachments_pool_size < _SG_MAX_POOL_SIZE));
...
_SOKOL_PRIVATE void
_sg_discard_pools(_sg_pools_t *p)
{
	SOKOL_dbg_assert(p);
...
*/
_SOKOL_PRIVATE uint32_t
_sg_slot_alloc(_sg_pool_t *pool, _sg_slot_t *slot, int slot_index)
{
	/* FIXME: add handling for an overflowing generation counter,
       for now, just overflow (another option is to disable
       the slot)
    */
	SOKOL_dbg_assert(pool && pool->gen_ctrs);
	SOKOL_dbg_assert((slot_index > _SG_INVALID_SLOT_INDEX) && (slot_index < pool->size));
	SOKOL_dbg_assert(slot->id == SG_INVALID_ID);
	SOKOL_dbg_assert(slot->state == SG_RESOURCESTATE_INITIAL);
...
_SOKOL_PRIVATE int
_sg_slot_index(uint32_t id)
{
	int slot_index = (int)(id & _SG_SLOT_MASK);
	SOKOL_dbg_assert(_SG_INVALID_SLOT_INDEX != slot_index);
	return slot_index;
}

// returns pointer to resource by id without matching id check
_SOKOL_PRIVATE _sg_buffer_t *
_sg_buffer_at(const _sg_pools_t *p, uint32_t buf_id)
{
	SOKOL_dbg_assert(p && (SG_INVALID_ID != buf_id));
	int slot_index = _sg_slot_index(buf_id);
	SOKOL_dbg_assert((slot_index > _SG_INVALID_SLOT_INDEX) && (slot_index < p->buffer_pool.size));
	return &p->buffers[slot_index];
}

_SOKOL_PRIVATE _sg_image_t *
_sg_image_at(const _sg_pools_t *p, uint32_t img_id)
{
	SOKOL_dbg_assert(p && (SG_INVALID_ID != img_id));
	int slot_index = _sg_slot_index(img_id);
	SOKOL_dbg_assert((slot_index > _SG_INVALID_SLOT_INDEX) && (slot_index < p->image_pool.size));
	return &p->images[slot_index];
}

_SOKOL_PRIVATE _sg_sampler_t *
_sg_sampler_at(const _sg_pools_t *p, uint32_t smp_id)
{
	SOKOL_dbg_assert(p && (SG_INVALID_ID != smp_id));
	int slot_index = _sg_slot_index(smp_id);
	SOKOL_dbg_assert((slot_index > _SG_INVALID_SLOT_INDEX) && (slot_index < p->sampler_pool.size));
	return &p->samplers[slot_index];
}

_SOKOL_PRIVATE _sg_shader_t *
_sg_shader_at(const _sg_pools_t *p, uint32_t shd_id)
{
	SOKOL_dbg_assert(p && (SG_INVALID_ID != shd_id));
	int slot_index = _sg_slot_index(shd_id);
	SOKOL_dbg_assert((slot_index > _SG_INVALID_SLOT_INDEX) && (slot_index < p->shader_pool.size));
	return &p->shaders[slot_index];
}

_SOKOL_PRIVATE _sg_pipeline_t *
_sg_pipeline_at(const _sg_pools_t *p, uint32_t pip_id)
{
	SOKOL_dbg_assert(p && (SG_INVALID_ID != pip_id));
	int slot_index = _sg_slot_index(pip_id);
	SOKOL_dbg_assert((slot_index > _SG_INVALID_SLOT_INDEX) && (slot_index < p->pipeline_pool.size));
	return &p->pipelines[slot_index];
}

_SOKOL_PRIVATE _sg_attachments_t *
_sg_attachments_at(const _sg_pools_t *p, uint32_t atts_id)
{
	SOKOL_dbg_assert(p && (SG_INVALID_ID != atts_id));
	int slot_index = _sg_slot_index(atts_id);
	SOKOL_dbg_assert((slot_index > _SG_INVALID_SLOT_INDEX) && (slot_index < p->attachments_pool.size));
	return &p->attachments[slot_index];
}

// returns pointer to resource with matching id check, may return 0
_SOKOL_PRIVATE _sg_buffer_t *
_sg_lookup_buffer(const _sg_pools_t *p, uint32_t buf_id)
{
...
_SOKOL_PRIVATE _sg_image_t *
_sg_lookup_image(const _sg_pools_t *p, uint32_t img_id)
{
...
_SOKOL_PRIVATE _sg_sampler_t *
_sg_lookup_sampler(const _sg_pools_t *p, uint32_t smp_id)
{
...
_SOKOL_PRIVATE _sg_shader_t *
_sg_lookup_shader(const _sg_pools_t *p, uint32_t shd_id)
{
	SOKOL_dbg_assert(p);
...
_SOKOL_PRIVATE _sg_pipeline_t *
_sg_lookup_pipeline(const _sg_pools_t *p, uint32_t pip_id)
{
	SOKOL_dbg_assert(p);
...
_SOKOL_PRIVATE _sg_attachments_t *
_sg_lookup_attachments(const _sg_pools_t *p, uint32_t atts_id)
{
	SOKOL_dbg_assert(p);
...
_SOKOL_PRIVATE void
_sg_discard_all_resources(_sg_pools_t *p)
{
...
// >>tracker
_SOKOL_PRIVATE void
_sg_tracker_init(_sg_tracker_t *tracker, uint32_t num)
{
	SOKOL_dbg_assert(tracker);
	SOKOL_dbg_assert(num > 0);
	SOKOL_dbg_assert(0 == tracker->size);
	SOKOL_dbg_assert(0 == tracker->cur);
	SOKOL_dbg_assert(0 == tracker->items);
	tracker->size  = (uint32_t)num;
	tracker->items = (uint32_t *)_sg_malloc_clear(num * sizeof(uint32_t));
}

_SOKOL_PRIVATE void
_sg_tracker_discard(_sg_tracker_t *tracker)
{
	SOKOL_dbg_assert(tracker);
	if(tracker->items) {
...
_SOKOL_PRIVATE void
_sg_tracker_reset(_sg_tracker_t *tracker)
{
	SOKOL_dbg_assert(tracker && tracker->items);
	tracker->cur = 0;
}

_SOKOL_PRIVATE bool
_sg_tracker_add(_sg_tracker_t *tracker, uint32_t res_id)
{
	SOKOL_dbg_assert(tracker && tracker->items);
...
#if defined(SOKOL_DEBUG)
_SOKOL_PRIVATE void
_sg_validate_begin(void)
{
	_sg.validate_error = SG_LOGITEM_OK;
}

_SOKOL_PRIVATE bool
_sg_validate_end(void)
{
...
_SOKOL_PRIVATE bool
_sg_validate_buffer_desc(const sg_buffer_desc *desc)
{
#if !defined(SOKOL_DEBUG)
	_SOKOL_UNUSED(desc);
	return true;
...
_SOKOL_PRIVATE void
_sg_validate_image_data(const sg_image_data *data, sg_pixel_format fmt, int width, int height, int num_faces, int num_mips, int num_slices)
{
#if !defined(SOKOL_DEBUG)
	_SOKOL_UNUSED(data);
	_SOKOL_UNUSED(fmt);
...
_SOKOL_PRIVATE bool
_sg_validate_image_desc(const sg_image_desc *desc)
{
#if !defined(SOKOL_DEBUG)
	_SOKOL_UNUSED(desc);
	return true;
	...
	if(desc->render_target) {
		SOKOL_dbg_assert(((int)fmt >= 0) && ((int)fmt < _SG_PIXELFORMAT_NUM));
...
_SOKOL_PRIVATE bool
_sg_validate_sampler_desc(const sg_sampler_desc *desc)
{
#if !defined(SOKOL_DEBUG)
	_SOKOL_UNUSED(desc);
	return true;
	...
	if(desc->max_anisotropy > 1) {
		_SG_VALIDATE((desc->min_filter == SG_FILTER_LINEAR) && (desc->mag_filter == SG_FILTER_LINEAR) && (desc->mipmap_filter == SG_FILTER_LINEAR),
...
_SOKOL_PRIVATE _sg_u128_t
_sg_validate_set_slot_bit(_sg_u128_t bits, sg_shader_stage stage, uint8_t slot)
{
	switch(stage) {
	case SG_SHADERSTAGE_NONE:
		SOKOL_dbg_assert(slot < 128);
		if(slot < 64) {
			bits.lo |= 1ULL << slot;
		} else {
...
_SOKOL_PRIVATE bool
_sg_validate_slot_bits(_sg_u128_t bits, sg_shader_stage stage, uint8_t slot)
{
...
_SOKOL_PRIVATE bool
_sg_validate_shader_desc(const sg_shader_desc *desc)
{
#if !defined(SOKOL_DEBUG)
	_SOKOL_UNUSED(desc);
	return true;
	...
	for(size_t img_smp_idx = 0; img_smp_idx < SG_MAX_IMAGE_SAMPLER_PAIRS; img_smp_idx++) {
		...
		if(img_slot_in_range && smp_slot_in_range) {
			...
			const bool needs_nonfiltering = (img_desc->sample_type == SG_IMAGESAMPLETYPE_UINT) || (img_desc->sample_type == SG_IMAGESAMPLETYPE_SINT) || (img_desc->sample_type == SG_IMAGESAMPLETYPE_UNFILTERABLE_FLOAT);
...
_SOKOL_PRIVATE bool
_sg_validate_pipeline_desc(const sg_pipeline_desc *desc)
{
#if !defined(SOKOL_DEBUG)
	_SOKOL_UNUSED(desc);
	return true;
...
_SOKOL_PRIVATE bool
_sg_validate_attachments_desc(const sg_attachments_desc *desc)
{
#if !defined(SOKOL_DEBUG)
	_SOKOL_UNUSED(desc);
	return true;
...
_SOKOL_PRIVATE bool
_sg_validate_begin_pass(const sg_pass *pass)
{
#if !defined(SOKOL_DEBUG)
	_SOKOL_UNUSED(pass);
	return true;
...
_SOKOL_PRIVATE bool
_sg_validate_apply_viewport(int x, int y, int width, int height, bool origin_top_left)
{
	_SOKOL_UNUSED(x);
	_SOKOL_UNUSED(y);
	_SOKOL_UNUSED(width);
...
_SOKOL_PRIVATE bool
_sg_validate_apply_pipeline(sg_pipeline pip_id)
{
#if !defined(SOKOL_DEBUG)
...
_SOKOL_PRIVATE bool
_sg_validate_apply_bindings(const sg_bindings *bindings)
{
#if !defined(SOKOL_DEBUG)
			...
			if(bindings->samplers[i].id != SG_INVALID_ID) {
				...
				if(smp) {
					...
					if(shd->cmn.samplers[i].sampler_type == SG_SAMPLERTYPE_NONFILTERING) {
						const bool nonfiltering = (smp->cmn.min_filter != SG_FILTER_LINEAR) && (smp->cmn.mag_filter != SG_FILTER_LINEAR) && (smp->cmn.mipmap_filter != SG_FILTER_LINEAR);
...
_SOKOL_PRIVATE bool
_sg_validate_apply_uniforms(int ub_slot, const sg_range *data)
{
#if !defined(SOKOL_DEBUG)
	_SOKOL_UNUSED(ub_slot);
	_SOKOL_UNUSED(data);
	...
	const _sg_pipeline_t *pip = _sg_lookup_pipeline(&_sg.pools, _sg.cur_pipeline.id);
	SOKOL_dbg_assert(pip && (pip->slot.id == _sg.cur_pipeline.id));
	SOKOL_dbg_assert(pip->shader && (pip->shader->slot.id == pip->cmn.shader_id.id));
...
_SOKOL_PRIVATE bool
_sg_validate_draw(int base_element, int num_elements, int num_instances)
{
#if !defined(SOKOL_DEBUG)
	_SOKOL_UNUSED(base_element);
	_SOKOL_UNUSED(num_elements);
...
_SOKOL_PRIVATE bool
_sg_validate_dispatch(int num_groups_x, int num_groups_y, int num_groups_z)
{
#if !defined(SOKOL_DEBUG)
	_SOKOL_UNUSED(num_groups_x);
	_SOKOL_UNUSED(num_groups_y);
...
_SOKOL_PRIVATE bool
_sg_validate_update_buffer(const _sg_buffer_t *buf, const sg_range *data)
{
#if !defined(SOKOL_DEBUG)
	_SOKOL_UNUSED(buf);
	_SOKOL_UNUSED(data);
	...
	SOKOL_dbg_assert(buf && data && data->ptr);
	_sg_validate_begin();
...
_SOKOL_PRIVATE bool
_sg_validate_append_buffer(const _sg_buffer_t *buf, const sg_range *data)
{
#if !defined(SOKOL_DEBUG)
	_SOKOL_UNUSED(buf);
	_SOKOL_UNUSED(data);
...
_SOKOL_PRIVATE bool
_sg_validate_update_image(const _sg_image_t *img, const sg_image_data *data)
{
#if !defined(SOKOL_DEBUG)
	_SOKOL_UNUSED(img);
	_SOKOL_UNUSED(data);
...
_SOKOL_PRIVATE sg_buffer
_sg_alloc_buffer(void)
{
	sg_buffer res;
...
_SOKOL_PRIVATE sg_image
_sg_alloc_image(void)
{
	sg_image res;
...
_SOKOL_PRIVATE sg_sampler
_sg_alloc_sampler(void)
{
	sg_sampler res;
...
_SOKOL_PRIVATE sg_shader
_sg_alloc_shader(void)
{
	sg_shader res;
...
_SOKOL_PRIVATE sg_pipeline
_sg_alloc_pipeline(void)
{
	sg_pipeline res;
...
_SOKOL_PRIVATE sg_attachments
_sg_alloc_attachments(void)
{
	sg_attachments res;
...
_SOKOL_PRIVATE void
_sg_dealloc_buffer(_sg_buffer_t *buf)
{
	SOKOL_dbg_assert(buf && (buf->slot.state == SG_RESOURCESTATE_ALLOC) && (buf->slot.id != SG_INVALID_ID));
	_sg_pool_free_index(&_sg.pools.buffer_pool, _sg_slot_index(buf->slot.id));
	_sg_slot_reset(&buf->slot);
}

_SOKOL_PRIVATE void
_sg_dealloc_image(_sg_image_t *img)
{
	SOKOL_dbg_assert(img && (img->slot.state == SG_RESOURCESTATE_ALLOC) && (img->slot.id != SG_INVALID_ID));
	_sg_pool_free_index(&_sg.pools.image_pool, _sg_slot_index(img->slot.id));
	_sg_slot_reset(&img->slot);
}

_SOKOL_PRIVATE void
_sg_dealloc_sampler(_sg_sampler_t *smp)
{
	SOKOL_dbg_assert(smp && (smp->slot.state == SG_RESOURCESTATE_ALLOC) && (smp->slot.id != SG_INVALID_ID));
	_sg_pool_free_index(&_sg.pools.sampler_pool, _sg_slot_index(smp->slot.id));
	_sg_slot_reset(&smp->slot);
}

_SOKOL_PRIVATE void
_sg_dealloc_shader(_sg_shader_t *shd)
{
	SOKOL_dbg_assert(shd && (shd->slot.state == SG_RESOURCESTATE_ALLOC) && (shd->slot.id != SG_INVALID_ID));
	_sg_pool_free_index(&_sg.pools.shader_pool, _sg_slot_index(shd->slot.id));
	_sg_slot_reset(&shd->slot);
}

_SOKOL_PRIVATE void
_sg_dealloc_pipeline(_sg_pipeline_t *pip)
{
	SOKOL_dbg_assert(pip && (pip->slot.state == SG_RESOURCESTATE_ALLOC) && (pip->slot.id != SG_INVALID_ID));
	_sg_pool_free_index(&_sg.pools.pipeline_pool, _sg_slot_index(pip->slot.id));
	_sg_slot_reset(&pip->slot);
}

_SOKOL_PRIVATE void
_sg_dealloc_attachments(_sg_attachments_t *atts)
{
	SOKOL_dbg_assert(atts && (atts->slot.state == SG_RESOURCESTATE_ALLOC) && (atts->slot.id != SG_INVALID_ID));
	_sg_pool_free_index(&_sg.pools.attachments_pool, _sg_slot_index(atts->slot.id));
	_sg_slot_reset(&atts->slot);
}

_SOKOL_PRIVATE void
_sg_init_buffer(_sg_buffer_t *buf, const sg_buffer_desc *desc)
{
	SOKOL_dbg_assert(buf && (buf->slot.state == SG_RESOURCESTATE_ALLOC));
	SOKOL_dbg_assert(desc);
	...
	SOKOL_dbg_assert((buf->slot.state == SG_RESOURCESTATE_VALID) || (buf->slot.state == SG_RESOURCESTATE_FAILED));
}

_SOKOL_PRIVATE void
_sg_init_image(_sg_image_t *img, const sg_image_desc *desc)
{
	SOKOL_dbg_assert(img && (img->slot.state == SG_RESOURCESTATE_ALLOC));
	SOKOL_dbg_assert(desc);
	...
	SOKOL_dbg_assert((img->slot.state == SG_RESOURCESTATE_VALID) || (img->slot.state == SG_RESOURCESTATE_FAILED));
}

_SOKOL_PRIVATE void
_sg_init_sampler(_sg_sampler_t *smp, const sg_sampler_desc *desc)
{
	SOKOL_dbg_assert(smp && (smp->slot.state == SG_RESOURCESTATE_ALLOC));
	SOKOL_dbg_assert(desc);
	...
	SOKOL_dbg_assert((smp->slot.state == SG_RESOURCESTATE_VALID) || (smp->slot.state == SG_RESOURCESTATE_FAILED));
}

_SOKOL_PRIVATE void
_sg_init_shader(_sg_shader_t *shd, const sg_shader_desc *desc)
{
	SOKOL_dbg_assert(shd && (shd->slot.state == SG_RESOURCESTATE_ALLOC));
	SOKOL_dbg_assert(desc);
	...
	SOKOL_dbg_assert((shd->slot.state == SG_RESOURCESTATE_VALID) || (shd->slot.state == SG_RESOURCESTATE_FAILED));
}

_SOKOL_PRIVATE void
_sg_init_pipeline(_sg_pipeline_t *pip, const sg_pipeline_desc *desc)
{
	SOKOL_dbg_assert(pip && (pip->slot.state == SG_RESOURCESTATE_ALLOC));
	SOKOL_dbg_assert(desc);
	...
	SOKOL_dbg_assert((pip->slot.state == SG_RESOURCESTATE_VALID) || (pip->slot.state == SG_RESOURCESTATE_FAILED));
}

_SOKOL_PRIVATE void
_sg_init_attachments(_sg_attachments_t *atts, const sg_attachments_desc *desc)
{
	SOKOL_dbg_assert(atts && atts->slot.state == SG_RESOURCESTATE_ALLOC);
	SOKOL_dbg_assert(desc);
	...
	SOKOL_dbg_assert((atts->slot.state == SG_RESOURCESTATE_VALID) || (atts->slot.state == SG_RESOURCESTATE_FAILED));
}

_SOKOL_PRIVATE void
_sg_uninit_buffer(_sg_buffer_t *buf)
{
	SOKOL_dbg_assert(buf && ((buf->slot.state == SG_RESOURCESTATE_VALID) || (buf->slot.state == SG_RESOURCESTATE_FAILED)));
	_sg_discard_buffer(buf);
	_sg_reset_buffer_to_alloc_state(buf);
}

_SOKOL_PRIVATE void
_sg_uninit_image(_sg_image_t *img)
{
	SOKOL_dbg_assert(img && ((img->slot.state == SG_RESOURCESTATE_VALID) || (img->slot.state == SG_RESOURCESTATE_FAILED)));
	_sg_discard_image(img);
	_sg_reset_image_to_alloc_state(img);
}

_SOKOL_PRIVATE void
_sg_uninit_sampler(_sg_sampler_t *smp)
{
	SOKOL_dbg_assert(smp && ((smp->slot.state == SG_RESOURCESTATE_VALID) || (smp->slot.state == SG_RESOURCESTATE_FAILED)));
	_sg_discard_sampler(smp);
	_sg_reset_sampler_to_alloc_state(smp);
}

_SOKOL_PRIVATE void
_sg_uninit_shader(_sg_shader_t *shd)
{
	SOKOL_dbg_assert(shd && ((shd->slot.state == SG_RESOURCESTATE_VALID) || (shd->slot.state == SG_RESOURCESTATE_FAILED)));
	_sg_discard_shader(shd);
	_sg_reset_shader_to_alloc_state(shd);
}

_SOKOL_PRIVATE void
_sg_uninit_pipeline(_sg_pipeline_t *pip)
{
	SOKOL_dbg_assert(pip && ((pip->slot.state == SG_RESOURCESTATE_VALID) || (pip->slot.state == SG_RESOURCESTATE_FAILED)));
	_sg_discard_pipeline(pip);
	_sg_reset_pipeline_to_alloc_state(pip);
}

_SOKOL_PRIVATE void
_sg_uninit_attachments(_sg_attachments_t *atts)
{
	SOKOL_dbg_assert(atts && ((atts->slot.state == SG_RESOURCESTATE_VALID) || (atts->slot.state == SG_RESOURCESTATE_FAILED)));
	_sg_discard_attachments(atts);
	_sg_reset_attachments_to_alloc_state(atts);
}

_SOKOL_PRIVATE void
_sg_setup_commit_listeners(const sg_desc *desc)
{
	SOKOL_dbg_assert(desc->max_commit_listeners > 0);
	SOKOL_dbg_assert(0 == _sg.commit_listeners.items);
	SOKOL_dbg_assert(0 == _sg.commit_listeners.num);
	SOKOL_dbg_assert(0 == _sg.commit_listeners.upper);
...
_SOKOL_PRIVATE void
_sg_discard_commit_listeners(void)
{
	SOKOL_dbg_assert(0 != _sg.commit_listeners.items);
...
_SOKOL_PRIVATE void
_sg_notify_commit_listeners(void)
{
	SOKOL_dbg_assert(_sg.commit_listeners.items);
...
_SOKOL_PRIVATE bool
_sg_add_commit_listener(const sg_commit_listener *new_listener)
{
	SOKOL_dbg_assert(new_listener && new_listener->func);
	SOKOL_dbg_assert(_sg.commit_listeners.items);
...
_SOKOL_PRIVATE bool
_sg_remove_commit_listener(const sg_commit_listener *listener)
{
	SOKOL_dbg_assert(listener && listener->func);
	SOKOL_dbg_assert(_sg.commit_listeners.items);
	for(int i = 0; i < _sg.commit_listeners.upper; i++) {
...
_SOKOL_PRIVATE void
_sg_setup_compute(const sg_desc *desc)
{
	SOKOL_dbg_assert(desc && (desc->max_dispatch_calls_per_pass > 0));
...
_SOKOL_PRIVATE void
_sg_discard_compute(void)
{
	_sg_tracker_discard(&_sg.compute.readwrite_sbufs);
}

_SOKOL_PRIVATE void
_sg_compute_pass_track_storage_buffer(_sg_buffer_t *sbuf, bool readonly)
{
	SOKOL_dbg_assert(sbuf);
	if(!readonly) {
...
_SOKOL_PRIVATE void
_sg_compute_on_endpass(void)
{
	SOKOL_dbg_assert(_sg.cur_pass.in_pass);
	SOKOL_dbg_assert(_sg.cur_pass.is_compute);
	_sg_tracker_reset(&_sg.compute.readwrite_sbufs);
}

_SOKOL_PRIVATE sg_desc
_sg_desc_defaults(const sg_desc *desc)
{
	/*
    ...
    */
	...
	SOKOL_dbg_assert(SG_PIXELFORMAT_NONE < res.environment.defaults.color_format);
#elif defined(SOKOL_METAL) || defined(SOKOL_D3D11)
...
_SOKOL_PRIVATE sg_pass
_sg_pass_defaults(const sg_pass *pass)
{
	sg_pass res = *pass;
	if(!res.compute) {
...
{
	SOKOL_dbg_assert(desc);
	SOKOL_dbg_assert((desc->_start_canary == 0) && (desc->_end_canary == 0));
	SOKOL_dbg_assert((desc->allocator.alloc_fn && desc->allocator.free_fn) || (!desc->allocator.alloc_fn && !desc->allocator.free_fn));
...
SOKOL_API_IMPL sg_desc
sg_query_desc(void)
{
	SOKOL_dbg_assert(_sg.valid);
	return _sg.desc;
}

SOKOL_API_IMPL sg_backend
...
SOKOL_API_IMPL int
sg_query_row_pitch(sg_pixel_format fmt, int width, int row_align_bytes)
{
	SOKOL_dbg_assert(_sg.valid);
	SOKOL_dbg_assert(width > 0);
	SOKOL_dbg_assert((row_align_bytes > 0) && _sg_ispow2(row_align_bytes));
	SOKOL_dbg_assert(((int)fmt > SG_PIXELFORMAT_NONE) && ((int)fmt < _SG_PIXELFORMAT_NUM));
	return _sg_row_pitch(fmt, width, row_align_bytes);
}

SOKOL_API_IMPL int
sg_query_surface_pitch(sg_pixel_format fmt, int width, int height, int row_align_bytes)
{
	SOKOL_dbg_assert(_sg.valid);
	SOKOL_dbg_assert((width > 0) && (height > 0));
	SOKOL_dbg_assert((row_align_bytes > 0) && _sg_ispow2(row_align_bytes));
...
SOKOL_API_IMPL sg_frame_stats
sg_query_frame_stats(void)
{
	SOKOL_dbg_assert(_sg.valid);
...
SOKOL_API_IMPL sg_buffer
sg_alloc_buffer(void)
{
	SOKOL_dbg_assert(_sg.valid);
...
SOKOL_API_IMPL sg_sampler
sg_alloc_sampler(void)
{
	SOKOL_dbg_assert(_sg.valid);
...
SOKOL_API_IMPL sg_shader
sg_alloc_shader(void)
{
	SOKOL_dbg_assert(_sg.valid);
...
SOKOL_API_IMPL sg_pipeline
sg_alloc_pipeline(void)
{
	SOKOL_dbg_assert(_sg.valid);
...
SOKOL_API_IMPL void
sg_dealloc_buffer(sg_buffer buf_id)
{
	SOKOL_dbg_assert(_sg.valid);
...
SOKOL_API_IMPL void
sg_dealloc_image(sg_image img_id)
{
	SOKOL_dbg_assert(_sg.valid);
...
SOKOL_API_IMPL void
sg_dealloc_sampler(sg_sampler smp_id)
{
	SOKOL_dbg_assert(_sg.valid);
...
SOKOL_API_IMPL void
sg_dealloc_shader(sg_shader shd_id)
{
	SOKOL_dbg_assert(_sg.valid);
...
SOKOL_API_IMPL void
sg_dealloc_pipeline(sg_pipeline pip_id)
{
	SOKOL_dbg_assert(_sg.valid);
...
SOKOL_API_IMPL void
sg_dealloc_attachments(sg_attachments atts_id)
{
	SOKOL_dbg_assert(_sg.valid);
...
SOKOL_API_IMPL void
sg_init_buffer(sg_buffer buf_id, const sg_buffer_desc *desc)
{
	SOKOL_dbg_assert(_sg.valid);
...
SOKOL_API_IMPL void
sg_init_image(sg_image img_id, const sg_image_desc *desc)
{
	SOKOL_dbg_assert(_sg.valid);
...
SOKOL_API_IMPL void
sg_init_sampler(sg_sampler smp_id, const sg_sampler_desc *desc)
{
	SOKOL_dbg_assert(_sg.valid);
...
SOKOL_API_IMPL void
sg_init_shader(sg_shader shd_id, const sg_shader_desc *desc)
{
	SOKOL_dbg_assert(_sg.valid);
...
SOKOL_API_IMPL void
sg_init_pipeline(sg_pipeline pip_id, const sg_pipeline_desc *desc)
{
	SOKOL_dbg_assert(_sg.valid);
...
SOKOL_API_IMPL void
sg_init_attachments(sg_attachments atts_id, const sg_attachments_desc *desc)
{
	SOKOL_dbg_assert(_sg.valid);
...
SOKOL_API_IMPL void
sg_uninit_buffer(sg_buffer buf_id)
{
	SOKOL_dbg_assert(_sg.valid);
	_sg_buffer_t *buf = _sg_lookup_buffer(&_sg.pools, buf_id.id);
	if(buf) {
		if((buf->slot.state == SG_RESOURCESTATE_VALID) || (buf->slot.state == SG_RESOURCESTATE_FAILED)) {
			...
			SOKOL_dbg_assert(buf->slot.state == SG_RESOURCESTATE_ALLOC);
		} else {
...
SOKOL_API_IMPL void
sg_uninit_image(sg_image img_id)
{
	SOKOL_dbg_assert(_sg.valid);
	_sg_image_t *img = _sg_lookup_image(&_sg.pools, img_id.id);
	if(img) {
		if((img->slot.state == SG_RESOURCESTATE_VALID) || (img->slot.state == SG_RESOURCESTATE_FAILED)) {
			...
			SOKOL_dbg_assert(img->slot.state == SG_RESOURCESTATE_ALLOC);
		} else {
...
SOKOL_API_IMPL void
sg_uninit_sampler(sg_sampler smp_id)
{
	SOKOL_dbg_assert(_sg.valid);
	_sg_sampler_t *smp = _sg_lookup_sampler(&_sg.pools, smp_id.id);
	if(smp) {
		if((smp->slot.state == SG_RESOURCESTATE_VALID) || (smp->slot.state == SG_RESOURCESTATE_FAILED)) {
			...
			SOKOL_dbg_assert(smp->slot.state == SG_RESOURCESTATE_ALLOC);
		} else {
...
SOKOL_API_IMPL void
sg_uninit_shader(sg_shader shd_id)
{
	SOKOL_dbg_assert(_sg.valid);
	_sg_shader_t *shd = _sg_lookup_shader(&_sg.pools, shd_id.id);
	if(shd) {
		if((shd->slot.state == SG_RESOURCESTATE_VALID) || (shd->slot.state == SG_RESOURCESTATE_FAILED)) {
			...
			SOKOL_dbg_assert(shd->slot.state == SG_RESOURCESTATE_ALLOC);
		} else {
...
SOKOL_API_IMPL void
sg_uninit_pipeline(sg_pipeline pip_id)
{
	SOKOL_dbg_assert(_sg.valid);
	_sg_pipeline_t *pip = _sg_lookup_pipeline(&_sg.pools, pip_id.id);
	if(pip) {
		if((pip->slot.state == SG_RESOURCESTATE_VALID) || (pip->slot.state == SG_RESOURCESTATE_FAILED)) {
			...
			SOKOL_dbg_assert(pip->slot.state == SG_RESOURCESTATE_ALLOC);
		} else {
...
SOKOL_API_IMPL void
sg_uninit_attachments(sg_attachments atts_id)
{
	SOKOL_dbg_assert(_sg.valid);
	_sg_attachments_t *atts = _sg_lookup_attachments(&_sg.pools, atts_id.id);
	if(atts) {
		if((atts->slot.state == SG_RESOURCESTATE_VALID) || (atts->slot.state == SG_RESOURCESTATE_FAILED)) {
			...
			SOKOL_dbg_assert(atts->slot.state == SG_RESOURCESTATE_ALLOC);
		} else {
...
SOKOL_API_IMPL void
sg_fail_buffer(sg_buffer buf_id)
{
	SOKOL_dbg_assert(_sg.valid);
	_sg_buffer_t *buf = _sg_lookup_buffer(&_sg.pools, buf_id.id);
	if(buf) {
...
SOKOL_API_IMPL void
sg_fail_sampler(sg_sampler smp_id)
{
	SOKOL_dbg_assert(_sg.valid);
	_sg_sampler_t *smp = _sg_lookup_sampler(&_sg.pools, smp_id.id);
	if(smp) {
...
SOKOL_API_IMPL void
sg_fail_shader(sg_shader shd_id)
{
	SOKOL_dbg_assert(_sg.valid);
	_sg_shader_t *shd = _sg_lookup_shader(&_sg.pools, shd_id.id);
	if(shd) {
...
SOKOL_API_IMPL void
sg_fail_pipeline(sg_pipeline pip_id)
{
	SOKOL_dbg_assert(_sg.valid);
	_sg_pipeline_t *pip = _sg_lookup_pipeline(&_sg.pools, pip_id.id);
	if(pip) {
...
SOKOL_API_IMPL void
sg_fail_attachments(sg_attachments atts_id)
{
	SOKOL_dbg_assert(_sg.valid);
	_sg_attachments_t *atts = _sg_lookup_attachments(&_sg.pools, atts_id.id);
	if(atts) {
...
sg_query_buffer_state(sg_buffer buf_id)
{
	SOKOL_dbg_assert(_sg.valid);
	_sg_buffer_t *buf     = _sg_lookup_buffer(&_sg.pools, buf_id.id);
...
SOKOL_API_IMPL sg_resource_state
sg_query_image_state(sg_image img_id)
{
	SOKOL_dbg_assert(_sg.valid);
	_sg_image_t *img      = _sg_lookup_image(&_sg.pools, img_id.id);
...
SOKOL_API_IMPL sg_resource_state
sg_query_sampler_state(sg_sampler smp_id)
{
	SOKOL_dbg_assert(_sg.valid);
	_sg_sampler_t *smp    = _sg_lookup_sampler(&_sg.pools, smp_id.id);
...
SOKOL_API_IMPL sg_resource_state
sg_query_shader_state(sg_shader shd_id)
{
	SOKOL_dbg_assert(_sg.valid);
	_sg_shader_t *shd     = _sg_lookup_shader(&_sg.pools, shd_id.id);
...
SOKOL_API_IMPL sg_resource_state
sg_query_pipeline_state(sg_pipeline pip_id)
{
	SOKOL_dbg_assert(_sg.valid);
	_sg_pipeline_t *pip   = _sg_lookup_pipeline(&_sg.pools, pip_id.id);
...
SOKOL_API_IMPL void
sg_destroy_buffer(sg_buffer buf_id)
{
	SOKOL_dbg_assert(_sg.valid);
	...
	if(buf) {
		...
		if(buf->slot.state == SG_RESOURCESTATE_ALLOC) {
			_sg_dealloc_buffer(buf);
			SOKOL_dbg_assert(buf->slot.state == SG_RESOURCESTATE_INITIAL);
		}
	}
}

SOKOL_API_IMPL void
sg_destroy_image(sg_image img_id)
{
	SOKOL_dbg_assert(_sg.valid);
	...
	if(img) {
		...
		if(img->slot.state == SG_RESOURCESTATE_ALLOC) {
			_sg_dealloc_image(img);
			SOKOL_dbg_assert(img->slot.state == SG_RESOURCESTATE_INITIAL);
		}
	}
}

SOKOL_API_IMPL void
sg_destroy_sampler(sg_sampler smp_id)
{
	SOKOL_dbg_assert(_sg.valid);
	...
	if(smp) {
		...
		if(smp->slot.state == SG_RESOURCESTATE_ALLOC) {
			_sg_dealloc_sampler(smp);
			SOKOL_dbg_assert(smp->slot.state == SG_RESOURCESTATE_INITIAL);
		}
	}
}

SOKOL_API_IMPL void
sg_destroy_shader(sg_shader shd_id)
{
	SOKOL_dbg_assert(_sg.valid);
	...
	if(shd) {
		...
		if(shd->slot.state == SG_RESOURCESTATE_ALLOC) {
			_sg_dealloc_shader(shd);
			SOKOL_dbg_assert(shd->slot.state == SG_RESOURCESTATE_INITIAL);
		}
	}
}

SOKOL_API_IMPL void
sg_destroy_pipeline(sg_pipeline pip_id)
{
	SOKOL_dbg_assert(_sg.valid);
	...
	if(pip) {
		...
		if(pip->slot.state == SG_RESOURCESTATE_ALLOC) {
			_sg_dealloc_pipeline(pip);
			SOKOL_dbg_assert(pip->slot.state == SG_RESOURCESTATE_INITIAL);
		}
	}
}

SOKOL_API_IMPL void
sg_destroy_attachments(sg_attachments atts_id)
{
	SOKOL_dbg_assert(_sg.valid);
	...
	if(atts) {
		...
		if(atts->slot.state == SG_RESOURCESTATE_ALLOC) {
			_sg_dealloc_attachments(atts);
			SOKOL_dbg_assert(atts->slot.state == SG_RESOURCESTATE_INITIAL);
		}
	}
}
...
{
	SOKOL_dbg_assert(_sg.valid);
	SOKOL_dbg_assert(!_sg.cur_pass.valid);
	SOKOL_dbg_assert(!_sg.cur_pass.in_pass);
	SOKOL_dbg_assert(pass);
	SOKOL_dbg_assert((pass->_start_canary == 0) && (pass->_end_canary == 0));
	...
	if(!pass_def.compute) {
		if(pass_def.attachments.id != SG_INVALID_ID) {
			// an offscreen pass
			SOKOL_dbg_assert(_sg.cur_pass.atts == 0);
			...
			// a swapchain pass
			SOKOL_dbg_assert(pass_def.swapchain.width > 0);
			SOKOL_dbg_assert(pass_def.swapchain.height > 0);
			SOKOL_dbg_assert(pass_def.swapchain.color_format > SG_PIXELFORMAT_NONE);
			SOKOL_dbg_assert(pass_def.swapchain.sample_count > 0);
...
SOKOL_API_IMPL void
sg_apply_pipeline(sg_pipeline pip_id)
{
	SOKOL_dbg_assert(_sg.valid);
...
{
	SOKOL_dbg_assert(_sg.valid);
	SOKOL_dbg_assert(bindings);
	SOKOL_dbg_assert((bindings->_start_canary == 0) && (bindings->_end_canary == 0));
	...
	if(!_sg.cur_pass.is_compute) {
		for(size_t i = 0; i < SG_MAX_VERTEXBUFFER_BINDSLOTS; i++) {
			if(bnd.pip->cmn.vertex_buffer_layout_active[i]) {
				SOKOL_dbg_assert(bindings->vertex_buffers[i].id != SG_INVALID_ID);
	...
	for(int i = 0; i < SG_MAX_IMAGE_BINDSLOTS; i++) {
		if(shd->cmn.images[i].stage != SG_SHADERSTAGE_NONE) {
			SOKOL_dbg_assert(bindings->images[i].id != SG_INVALID_ID);
	...
	for(size_t i = 0; i < SG_MAX_SAMPLER_BINDSLOTS; i++) {
		if(shd->cmn.samplers[i].stage != SG_SHADERSTAGE_NONE) {
			SOKOL_dbg_assert(bindings->samplers[i].id != SG_INVALID_ID);
	...
	for(size_t i = 0; i < SG_MAX_STORAGEBUFFER_BINDSLOTS; i++) {
		if(shd->cmn.storage_buffers[i].stage != SG_SHADERSTAGE_NONE) {
			SOKOL_dbg_assert(bindings->storage_buffers[i].id != SG_INVALID_ID);
...
sg_update_buffer(sg_buffer buf_id, const sg_range *data)
{
	...
	if((data->size > 0) && buf && (buf->slot.state == SG_RESOURCESTATE_VALID)) {
		if(_sg_validate_update_buffer(buf, data)) {
			SOKOL_dbg_assert(data->size <= (size_t)buf->cmn.size);
			// only one update allowed per buffer and frame
			SOKOL_dbg_assert(buf->cmn.update_frame_index != _sg.frame_index);
			// update and append on same buffer in same frame not allowed
			SOKOL_dbg_assert(buf->cmn.append_frame_index != _sg.frame_index);
...
{
	SOKOL_dbg_assert(_sg.valid);
	SOKOL_dbg_assert(data && data->ptr);
	...
	if(buf) {
		...
		SOKOL_dbg_assert(_sg_multiple_u64((uint64_t)start_pos, 4));
...
SOKOL_API_IMPL bool
sg_query_buffer_overflow(sg_buffer buf_id)
{
	SOKOL_dbg_assert(_sg.valid);
	_sg_buffer_t *buf = _sg_lookup_buffer(&_sg.pools, buf_id.id);
...
SOKOL_API_IMPL bool
sg_query_buffer_will_overflow(sg_buffer buf_id, size_t size)
{
	SOKOL_dbg_assert(_sg.valid);
	_sg_buffer_t *buf = _sg_lookup_buffer(&_sg.pools, buf_id.id);
	bool result       = false;
...
SOKOL_API_IMPL void
sg_update_image(sg_image img_id, const sg_image_data *data)
{
	SOKOL_dbg_assert(_sg.valid);
	...
	if(img && img->slot.state == SG_RESOURCESTATE_VALID) {
		if(_sg_validate_update_image(img, data)) {
			SOKOL_dbg_assert(img->cmn.upd_frame_index != _sg.frame_index);
...
SOKOL_API_IMPL void
sg_push_debug_group(const char *name)
{
	SOKOL_dbg_assert(_sg.valid);
	SOKOL_dbg_assert(name);
...
SOKOL_API_IMPL bool
sg_add_commit_listener(sg_commit_listener listener)
{
	SOKOL_dbg_assert(_sg.valid);
...
SOKOL_API_IMPL bool
sg_remove_commit_listener(sg_commit_listener listener)
{
	SOKOL_dbg_assert(_sg.valid);
...
SOKOL_API_IMPL bool
sg_frame_stats_enabled(void)
{
	return _sg.stats_enabled;
}

SOKOL_API_IMPL sg_buffer_info
sg_query_buffer_info(sg_buffer buf_id)
{
	SOKOL_dbg_assert(_sg.valid);
	sg_buffer_info info;
...
SOKOL_API_IMPL sg_image_info
sg_query_image_info(sg_image img_id)
{
	SOKOL_dbg_assert(_sg.valid);
	sg_image_info info;
	_sg_clear(&info, sizeof(info));
...
SOKOL_API_IMPL sg_sampler_info
sg_query_sampler_info(sg_sampler smp_id)
{
	SOKOL_dbg_assert(_sg.valid);
...
SOKOL_API_IMPL sg_shader_info
sg_query_shader_info(sg_shader shd_id)
{
	SOKOL_dbg_assert(_sg.valid);
	sg_shader_info info;
	_sg_clear(&info, sizeof(info));
...
SOKOL_API_IMPL sg_pipeline_info
sg_query_pipeline_info(sg_pipeline pip_id)
{
	SOKOL_dbg_assert(_sg.valid);
...
sg_query_buffer_desc(sg_buffer buf_id)
{
	SOKOL_dbg_assert(_sg.valid);
	sg_buffer_desc desc;
...
SOKOL_API_IMPL sg_buffer_type
sg_query_buffer_type(sg_buffer buf_id)
{
	SOKOL_dbg_assert(_sg.valid);
	const _sg_buffer_t *buf = _sg_lookup_buffer(&_sg.pools, buf_id.id);
	if(buf) {
...
SOKOL_API_IMPL sg_usage
sg_query_buffer_usage(sg_buffer buf_id)
{
	SOKOL_dbg_assert(_sg.valid);
	const _sg_buffer_t *buf = _sg_lookup_buffer(&_sg.pools, buf_id.id);
	if(buf) {
...
sg_query_image_desc(sg_image img_id)
{
	SOKOL_dbg_assert(_sg.valid);
	sg_image_desc desc;
	_sg_clear(&desc, sizeof(desc));
...
SOKOL_API_IMPL sg_image_type
sg_query_image_type(sg_image img_id)
{
	SOKOL_dbg_assert(_sg.valid);
	const _sg_image_t *img = _sg_lookup_image(&_sg.pools, img_id.id);
	if(img) {
...
SOKOL_API_IMPL int
sg_query_image_width(sg_image img_id)
{
	SOKOL_dbg_assert(_sg.valid);
	const _sg_image_t *img = _sg_lookup_image(&_sg.pools, img_id.id);
	if(img) {
...
SOKOL_API_IMPL int
sg_query_image_height(sg_image img_id)
{
	SOKOL_dbg_assert(_sg.valid);
	const _sg_image_t *img = _sg_lookup_image(&_sg.pools, img_id.id);
	if(img) {
...
SOKOL_API_IMPL int
sg_query_image_num_slices(sg_image img_id)
{
	SOKOL_dbg_assert(_sg.valid);
	const _sg_image_t *img = _sg_lookup_image(&_sg.pools, img_id.id);
	if(img) {
...
SOKOL_API_IMPL int
sg_query_image_num_mipmaps(sg_image img_id)
{
	SOKOL_dbg_assert(_sg.valid);
	const _sg_image_t *img = _sg_lookup_image(&_sg.pools, img_id.id);
	if(img) {
...
sg_query_image_pixelformat(sg_image img_id)
{
	SOKOL_dbg_assert(_sg.valid);
	const _sg_image_t *img = _sg_lookup_image(&_sg.pools, img_id.id);
	if(img) {
...
SOKOL_API_IMPL sg_usage
sg_query_image_usage(sg_image img_id)
{
	SOKOL_dbg_assert(_sg.valid);
	const _sg_image_t *img = _sg_lookup_image(&_sg.pools, img_id.id);
	if(img) {
...
SOKOL_API_IMPL int
sg_query_image_sample_count(sg_image img_id)
{
	SOKOL_dbg_assert(_sg.valid);
	SOKOL_dbg_assert(_sg.valid);
	const _sg_image_t *img = _sg_lookup_image(&_sg.pools, img_id.id);
...
sg_query_sampler_desc(sg_sampler smp_id)
{
	SOKOL_dbg_assert(_sg.valid);
...
sg_query_shader_desc(sg_shader shd_id)
{
	SOKOL_dbg_assert(_sg.valid);
	sg_shader_desc desc;
	_sg_clear(&desc, sizeof(desc));
...
sg_query_pipeline_desc(sg_pipeline pip_id)
{
	SOKOL_dbg_assert(_sg.valid);
...
sg_query_buffer_defaults(const sg_buffer_desc *desc)
{
	SOKOL_dbg_assert(_sg.valid && desc);
...
SOKOL_API_IMPL const void *
sg_d3d11_device(void)
{
#if defined(SOKOL_D3D11)
...
sg_d3d11_query_image_info(sg_image img_id)
{
	SOKOL_dbg_assert(_sg.valid);
...
SOKOL_API_IMPL const void *
sg_mtl_render_command_encoder(void)
{
#if defined(SOKOL_METAL)
	if(nil != _sg.mtl.render_cmd_encoder) {
...
SOKOL_API_IMPL const void *
sg_mtl_compute_command_encoder(void)
{
#if defined(SOKOL_METAL)
	if(nil != _sg.mtl.compute_cmd_encoder) {
...
SOKOL_API_IMPL sg_mtl_buffer_info
sg_mtl_query_buffer_info(sg_buffer buf_id)
{
	SOKOL_dbg_assert(_sg.valid);
...
SOKOL_API_IMPL sg_mtl_image_info
sg_mtl_query_image_info(sg_image img_id)
{
	SOKOL_dbg_assert(_sg.valid);
...
SOKOL_API_IMPL sg_mtl_shader_info
sg_mtl_query_shader_info(sg_shader shd_id)
{
	SOKOL_dbg_assert(_sg.valid);
...
SOKOL_API_IMPL const void *
sg_wgpu_render_pass_encoder(void)
{
#if defined(SOKOL_WGPU)
...
SOKOL_API_IMPL const void *
sg_wgpu_compute_pass_encoder(void)
{
#if defined(SOKOL_WGPU)
...
SOKOL_API_IMPL sg_wgpu_buffer_info
sg_wgpu_query_buffer_info(sg_buffer buf_id)
{
	SOKOL_dbg_assert(_sg.valid);
...
SOKOL_API_IMPL sg_wgpu_image_info
sg_wgpu_query_image_info(sg_image img_id)
{
	SOKOL_dbg_assert(_sg.valid);
...
SOKOL_API_IMPL sg_wgpu_shader_info
sg_wgpu_query_shader_info(sg_shader shd_id)
{
	SOKOL_dbg_assert(_sg.valid);
...
SOKOL_API_IMPL sg_gl_buffer_info
sg_gl_query_buffer_info(sg_buffer buf_id)
{
	SOKOL_dbg_assert(_sg.valid);
...
SOKOL_API_IMPL sg_gl_image_info
sg_gl_query_image_info(sg_image img_id)
{
	SOKOL_dbg_assert(_sg.valid);
...
SOKOL_API_IMPL sg_gl_sampler_info
sg_gl_query_sampler_info(sg_sampler smp_id)
{
	SOKOL_dbg_assert(_sg.valid);
...
sg_gl_query_shader_info(sg_shader shd_id)
{
	SOKOL_dbg_assert(_sg.valid);
