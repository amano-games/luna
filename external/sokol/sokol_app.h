...
_SOKOL_PRIVATE int
_sapp_ring_idx(int i)
{
	return i % _SAPP_RING_NUM_SLOTS;
}

_SOKOL_PRIVATE void
_sapp_ring_init(_sapp_ring_t *ring)
{
	ring->head = 0;
	ring->tail = 0;
}
...
_SOKOL_PRIVATE bool
_sapp_ring_full(_sapp_ring_t *ring)
{
	return _sapp_ring_idx(ring->head + 1) == ring->tail;
}

_SOKOL_PRIVATE bool
_sapp_ring_empty(_sapp_ring_t *ring)
{
	return ring->head == ring->tail;
}

_SOKOL_PRIVATE int
_sapp_ring_count(_sapp_ring_t *ring)
{
	int count;
	...
	SOKOL_dbg_assert((count >= 0) && (count < _SAPP_RING_NUM_SLOTS));
	return count;
}

_SOKOL_PRIVATE void
_sapp_ring_enqueue(_sapp_ring_t *ring, double val)
{
	SOKOL_dbg_assert(!_sapp_ring_full(ring));
	ring->buf[ring->head] = val;
	ring->head            = _sapp_ring_idx(ring->head + 1);
}

_SOKOL_PRIVATE double
_sapp_ring_dequeue(_sapp_ring_t *ring)
{
	SOKOL_dbg_assert(!_sapp_ring_empty(ring));
...
_sapp_int64_muldiv(int64_t value, int64_t numer, int64_t denom)
{
...
_SOKOL_PRIVATE void
_sapp_timestamp_init(_sapp_timestamp_t *ts)
{
#if defined(_SAPP_APPLE)
...
_SOKOL_PRIVATE double
_sapp_timestamp_now(_sapp_timestamp_t *ts)
{
#if defined(_SAPP_APPLE)
...
_SOKOL_PRIVATE void
_sapp_timing_reset(_sapp_timing_t *t)
{
	t->last        = 0.0;
	t->accum       = 0.0;
	t->spike_count = 0;
...
_SOKOL_PRIVATE void
_sapp_timing_init(_sapp_timing_t *t)
{
...
_SOKOL_PRIVATE void
_sapp_timing_put(_sapp_timing_t *t, double dur)
{
	...
	t->accum += dur;
	t->num += 1;
	SOKOL_dbg_assert(t->num > 0);
	t->avg         = t->accum / t->num;
	t->spike_count = 0;
}

_SOKOL_PRIVATE void
_sapp_timing_discontinuity(_sapp_timing_t *t)
{
	t->last = 0.0;
}

_SOKOL_PRIVATE void
_sapp_timing_measure(_sapp_timing_t *t)
{
...
_SOKOL_PRIVATE void
_sapp_timing_external(_sapp_timing_t *t, double now)
{
	if(t->last > 0.0) {
		double dur = now - t->last;
		_sapp_timing_put(t, dur);
...
_SOKOL_PRIVATE double
_sapp_timing_get_avg(_sapp_timing_t *t)
{
	return t->avg;
}
...
static void
_sapp_log(sapp_log_item log_item, uint32_t log_level, const char *msg, uint32_t line_nr)
{
...
// >>memory
_SOKOL_PRIVATE void
_sapp_clear(void *ptr, size_t size)
{
...
_SOKOL_PRIVATE void *
_sapp_malloc_clear(size_t size)
{
...
_SOKOL_PRIVATE bool
_sapp_call_event(const sapp_event *e)
{
	if(!_sapp.cleanup_called) {
...
_SOKOL_PRIVATE char *
_sapp_dropped_file_path_ptr(int index)
{
	SOKOL_dbg_assert(_sapp.drop.buffer);
	SOKOL_dbg_assert((index >= 0) && (index <= _sapp.drop.max_files));
	int offset = index * _sapp.drop.max_path_length;
	SOKOL_dbg_assert(offset < _sapp.drop.buf_size);
...
*/
_SOKOL_PRIVATE bool
_sapp_strcpy(const char *src, char *dst, int max_len)
{
	SOKOL_dbg_assert(src && dst && (max_len > 0));
...
_SOKOL_PRIVATE sapp_desc
_sapp_desc_defaults(const sapp_desc *desc)
{
...
_SOKOL_PRIVATE void
_sapp_init_state(const sapp_desc *desc)
{
	SOKOL_dbg_assert(desc);
	SOKOL_dbg_assert(desc->width >= 0);
	SOKOL_dbg_assert(desc->height >= 0);
	SOKOL_dbg_assert(desc->sample_count >= 0);
	SOKOL_dbg_assert(desc->swap_interval >= 0);
	SOKOL_dbg_assert(desc->clipboard_size >= 0);
	SOKOL_dbg_assert(desc->max_dropped_files >= 0);
	SOKOL_dbg_assert(desc->max_dropped_file_path_length >= 0);
...
_SOKOL_PRIVATE void
_sapp_discard_state(void)
{
	if(_sapp.clipboard.enabled) {
		SOKOL_dbg_assert(_sapp.clipboard.buffer);
...
_SOKOL_PRIVATE void
_sapp_init_event(sapp_event_type type)
{
...
_SOKOL_PRIVATE bool
_sapp_image_validate(const sapp_image_desc *desc)
{
	SOKOL_dbg_assert(desc->width > 0);
	SOKOL_dbg_assert(desc->height > 0);
	SOKOL_dbg_assert(desc->pixels.ptr != 0);
	SOKOL_dbg_assert(desc->pixels.size > 0);
...
_SOKOL_PRIVATE int
_sapp_image_bestmatch(const sapp_image_desc image_descs[], int num_images, int width, int height)
{
...
_SOKOL_PRIVATE int
_sapp_icon_num_images(const sapp_icon_desc *desc)
{
	int index = 0;
...
_SOKOL_PRIVATE bool
_sapp_validate_icon_desc(const sapp_icon_desc *desc, int num_images)
{
	SOKOL_dbg_assert(num_images <= SAPP_MAX_ICONIMAGES);
	for(int i = 0; i < num_images; i++) {
...
_SOKOL_PRIVATE void
_sapp_setup_default_icon(void)
{
	SOKOL_dbg_assert(0 == _sapp.default_icon_pixels);
...
_SOKOL_PRIVATE void
_sapp_macos_init_keytable(void)
{
...
_SOKOL_PRIVATE void
_sapp_macos_discard_state(void)
{
...
_SOKOL_PRIVATE void
_sapp_macos_run(const sapp_desc *desc)
{
...
_SOKOL_PRIVATE void
_sapp_macos_mouse_event(sapp_event_type type, sapp_mousebutton btn, uint32_t mod)
{
...
_SOKOL_PRIVATE void
_sapp_macos_key_event(sapp_event_type type, sapp_keycode key, bool repeat, uint32_t mod)
{
	if(_sapp_events_enabled()) {
		_sapp_init_event(type);
...
_SOKOL_PRIVATE void
_sapp_macos_app_event(sapp_event_type type)
{
	if(_sapp_events_enabled()) {
		_sapp_init_event(type);
		_sapp_call_event(&_sapp.event);
...
*/
_SOKOL_PRIVATE void
_sapp_macos_update_dimensions(void)
{
...
_SOKOL_PRIVATE void
_sapp_macos_toggle_fullscreen(void)
{
...
_SOKOL_PRIVATE void
_sapp_macos_set_clipboard_string(const char *str)
{
	@autoreleasepool {
...
_sapp_macos_get_clipboard_string(void)
{
	SOKOL_dbg_assert(_sapp.clipboard.buffer);
	@autoreleasepool {
...
_SOKOL_PRIVATE void
_sapp_macos_update_window_title(void)
{
	[_sapp.macos.window setTitle:[NSString stringWithUTF8String:_sapp.window_title]];
}

_SOKOL_PRIVATE void
_sapp_macos_mouse_update_from_nspoint(NSPoint mouse_pos, bool clear_dxdy)
{
...
_SOKOL_PRIVATE void
_sapp_macos_mouse_update_from_nsevent(NSEvent *event, bool clear_dxdy)
{
	_sapp_macos_mouse_update_from_nspoint(event.locationInWindow, clear_dxdy);
}

_SOKOL_PRIVATE void
_sapp_macos_show_mouse(bool visible)
{
...
_SOKOL_PRIVATE void
_sapp_macos_lock_mouse(bool lock)
{
...
_SOKOL_PRIVATE void
_sapp_macos_update_cursor(sapp_mouse_cursor cursor, bool shown)
{
	...
	// update cursor type
	SOKOL_dbg_assert((cursor >= 0) && (cursor < _SAPP_MOUSECURSOR_NUM));
...
_SOKOL_PRIVATE void
_sapp_macos_set_icon(const sapp_icon_desc *icon_desc, int num_images)
{
...
- (void)applicationDidFinishLaunching:(NSNotification *)aNotification
{
	...
	SOKOL_dbg_assert(glpixelformat_obj != nil);
...
_SOKOL_PRIVATE void
_sapp_macos_poll_input_events(void)
{
	/*
...
_SOKOL_PRIVATE void
_sapp_ios_run(const sapp_desc *desc)
{
	_sapp_init_state(desc);
	static int argc     = 1;
...
_SOKOL_PRIVATE void
_sapp_ios_app_event(sapp_event_type type)
{
	if(_sapp_events_enabled()) {
		_sapp_init_event(type);
		_sapp_call_event(&_sapp.event);
...
_SOKOL_PRIVATE void
_sapp_ios_touch_event(sapp_event_type type, NSSet<UITouch *> *touches, UIEvent *event)
{
	if(_sapp_events_enabled()) {
		_sapp_init_event(type);
...
_SOKOL_PRIVATE void
_sapp_ios_update_dimensions(void)
{
...
_SOKOL_PRIVATE void
_sapp_ios_show_keyboard(bool shown)
{
...
_sapp_emsc_onpaste(const char *str)
{
	if(_sapp.clipboard.enabled) {
...
_sapp_html5_get_ask_leave_site(void)
{
	return _sapp.html5_ask_leave_site ? 1 : 0;
}

EMSCRIPTEN_KEEPALIVE void
_sapp_emsc_begin_drop(int num)
{
	if(!_sapp.drop.enabled) {
		return;
	}
...
EMSCRIPTEN_KEEPALIVE void
_sapp_emsc_drop(int i, const char *name)
{
	...
	SOKOL_dbg_assert(_sapp.drop.num_files <= _sapp.drop.max_files);
...
EMSCRIPTEN_KEEPALIVE void
_sapp_emsc_end_drop(int x, int y, int mods)
{
	if(!_sapp.drop.enabled) {
		return;
	}
...
EMSCRIPTEN_KEEPALIVE void
_sapp_emsc_invoke_fetch_cb(int index, int success, int error_code, _sapp_html5_fetch_callback callback, uint32_t fetched_size, void *buf_ptr, uint32_t buf_size, void *user_data)
{
...
_SOKOL_PRIVATE void
_sapp_emsc_set_clipboard_string(const char *str)
{
...
_SOKOL_PRIVATE EM_BOOL
_sapp_emsc_pointerlockerror_cb(int emsc_type, const void *reserved, void *user_data)
{
	_SOKOL_UNUSED(emsc_type);
...
_SOKOL_PRIVATE void
_sapp_emsc_lock_mouse(bool lock)
{
	if(lock) {
...
*/
_SOKOL_PRIVATE void
_sapp_emsc_update_mouse_lock_state(void)
{
...
_SOKOL_PRIVATE void
_sapp_emsc_update_cursor(sapp_mouse_cursor cursor, bool shown)
{
	SOKOL_dbg_assert((cursor >= 0) && (cursor < _SAPP_MOUSECURSOR_NUM));
...
_SOKOL_PRIVATE void
_sapp_emsc_set_icon(const sapp_icon_desc *icon_desc, int num_images)
{
	SOKOL_dbg_assert((num_images > 0) && (num_images <= SAPP_MAX_ICONIMAGES));
...
_SOKOL_PRIVATE uint32_t
_sapp_emsc_mouse_button_mods(uint16_t buttons)
{
	uint32_t m = 0;
...
_SOKOL_PRIVATE EM_BOOL
_sapp_emsc_size_changed(int event_type, const EmscriptenUiEvent *ui_event, void *user_data)
{
    ...
    */
	...
	SOKOL_dbg_assert((_sapp.framebuffer_width > 0) && (_sapp.framebuffer_height > 0));
...
_SOKOL_PRIVATE EM_BOOL
_sapp_emsc_mouse_cb(int emsc_type, const EmscriptenMouseEvent *emsc_event, void *user_data)
{
	_SOKOL_UNUSED(user_data);
...
_SOKOL_PRIVATE EM_BOOL
_sapp_emsc_wheel_cb(int emsc_type, const EmscriptenWheelEvent *emsc_event, void *user_data)
{
	_SOKOL_UNUSED(emsc_type);
	_SOKOL_UNUSED(user_data);
...
_SOKOL_PRIVATE sapp_keycode
_sapp_emsc_translate_key(const char *str)
{
	int i = 0;
	const char *keystr;
...
_SOKOL_PRIVATE bool
_sapp_emsc_is_char_key(sapp_keycode key_code)
{
	return key_code < SAPP_KEYCODE_WORLD_1;
}

_SOKOL_PRIVATE EM_BOOL
_sapp_emsc_key_cb(int emsc_type, const EmscriptenKeyboardEvent *emsc_event, void *user_data)
{
	_SOKOL_UNUSED(user_data);
...
_SOKOL_PRIVATE EM_BOOL
_sapp_emsc_touch_cb(int emsc_type, const EmscriptenTouchEvent *emsc_event, void *user_data)
{
	_SOKOL_UNUSED(user_data);
...
_SOKOL_PRIVATE EM_BOOL
_sapp_emsc_focus_cb(int emsc_type, const EmscriptenFocusEvent *emsc_event, void *user_data)
{
	_SOKOL_UNUSED(emsc_type);
...
_SOKOL_PRIVATE EM_BOOL
_sapp_emsc_blur_cb(int emsc_type, const EmscriptenFocusEvent *emsc_event, void *user_data)
{
	_SOKOL_UNUSED(emsc_type);
	_SOKOL_UNUSED(emsc_event);
	_SOKOL_UNUSED(user_data);
...
#if defined(SOKOL_GLES3)
_SOKOL_PRIVATE EM_BOOL
_sapp_emsc_webgl_context_cb(int emsc_type, const void *reserved, void *user_data)
{
	_SOKOL_UNUSED(reserved);
	_SOKOL_UNUSED(user_data);
	sapp_event_type type;
...
_SOKOL_PRIVATE void
_sapp_emsc_wgpu_create_swapchain(void)
{
	SOKOL_dbg_assert(_sapp.wgpu.instance);
	SOKOL_dbg_assert(_sapp.wgpu.device);
	SOKOL_dbg_assert(0 == _sapp.wgpu.surface);
	SOKOL_dbg_assert(0 == _sapp.wgpu.swapchain);
	SOKOL_dbg_assert(0 == _sapp.wgpu.msaa_tex);
	SOKOL_dbg_assert(0 == _sapp.wgpu.msaa_view);
	SOKOL_dbg_assert(0 == _sapp.wgpu.depth_stencil_tex);
	SOKOL_dbg_assert(0 == _sapp.wgpu.depth_stencil_view);
	SOKOL_dbg_assert(0 == _sapp.wgpu.swapchain_view);
...
_SOKOL_PRIVATE void
_sapp_emsc_wgpu_discard_swapchain(void)
{
...
_SOKOL_PRIVATE void
_sapp_emsc_wgpu_size_changed(void)
{
...
_sapp_emsc_wgpu_request_device_cb(WGPURequestDeviceStatus status, WGPUDevice device, const char *msg, void *userdata)
{
	...
	SOKOL_dbg_assert(!_sapp.wgpu.async_init_done);
...
_SOKOL_PRIVATE void
_sapp_emsc_wgpu_request_adapter_cb(WGPURequestAdapterStatus status, WGPUAdapter adapter, const char *msg, void *userdata)
{
	_SOKOL_UNUSED(msg);
	_SOKOL_UNUSED(userdata);
...
#define _SAPP_WGPU_MAX_REQUESTED_FEATURES (8)
	...
	if(wgpuAdapterHasFeature(adapter, WGPUFeatureName_TextureCompressionBC)) {
		SOKOL_dbg_assert(cur_feature_index < _SAPP_WGPU_MAX_REQUESTED_FEATURES);
...
_SOKOL_PRIVATE void
_sapp_emsc_wgpu_init(void)
{
	SOKOL_dbg_assert(0 == _sapp.wgpu.instance);
	SOKOL_dbg_assert(!_sapp.wgpu.async_init_done);
...
_SOKOL_PRIVATE void
_sapp_emsc_register_eventhandlers(void)
{
...
_SOKOL_PRIVATE void
_sapp_emsc_unregister_eventhandlers(void)
{
...
_SOKOL_PRIVATE EM_BOOL
_sapp_emsc_frame_animation_loop(double time, void *userData)
{
	_SOKOL_UNUSED(userData);
...
_SOKOL_PRIVATE void
_sapp_emsc_frame_main_loop(void)
{
...
_SOKOL_PRIVATE void
_sapp_emsc_run(const sapp_desc *desc)
{
	_sapp_init_state(desc);
...
_SOKOL_PRIVATE void
_sapp_gl_init_fbconfig(_sapp_gl_fbconfig *fbconfig)
{
...
_SOKOL_PRIVATE void
_sapp_gl_init_fbselect(_sapp_gl_fbselect *fbselect)
{
...
#if defined(_SAPP_WIN32)
_SOKOL_PRIVATE bool
_sapp_win32_utf8_to_wide(const char *src, wchar_t *dst, int dst_num_bytes)
{
	SOKOL_dbg_assert(src && dst && (dst_num_bytes > 1));
...
_SOKOL_PRIVATE void
_sapp_win32_app_event(sapp_event_type type)
{
	if(_sapp_events_enabled()) {
		_sapp_init_event(type);
		_sapp_call_event(&_sapp.event);
...
_SOKOL_PRIVATE void
_sapp_win32_init_keytable(void)
{
	/* same as GLFW */
...
_sapp_dxgi_GetBuffer(IDXGISwapChain *self, UINT Buffer, REFIID riid, void **ppSurface)
{
#if defined(__cplusplus)
...
static inline HRESULT
_sapp_d3d11_CreateRenderTargetView(ID3D11Device *self, ID3D11Resource *pResource, const D3D11_RENDER_TARGET_VIEW_DESC *pDesc, ID3D11RenderTargetView **ppRTView)
{
#if defined(__cplusplus)
...
static inline HRESULT
_sapp_d3d11_CreateTexture2D(ID3D11Device *self, const D3D11_TEXTURE2D_DESC *pDesc, const D3D11_SUBRESOURCE_DATA *pInitialData, ID3D11Texture2D **ppTexture2D)
{
#if defined(__cplusplus)
...
static inline HRESULT
_sapp_d3d11_CreateDepthStencilView(ID3D11Device *self, ID3D11Resource *pResource, const D3D11_DEPTH_STENCIL_VIEW_DESC *pDesc, ID3D11DepthStencilView **ppDepthStencilView)
{
#if defined(__cplusplus)
...
static inline HRESULT
_sapp_dxgi_ResizeBuffers(IDXGISwapChain *self, UINT BufferCount, UINT Width, UINT Height, DXGI_FORMAT NewFormat, UINT SwapChainFlags)
{
#if defined(__cplusplus)
...
static inline HRESULT
_sapp_dxgi_Present(IDXGISwapChain *self, UINT SyncInterval, UINT Flags)
{
#if defined(__cplusplus)
...
static inline HRESULT
_sapp_dxgi_GetFrameStatistics(IDXGISwapChain *self, DXGI_FRAME_STATISTICS *pStats)
{
#if defined(__cplusplus)
...
static inline HRESULT
_sapp_dxgi_SetMaximumFrameLatency(IDXGIDevice1 *self, UINT MaxLatency)
{
#if defined(__cplusplus)
...
static inline HRESULT
_sapp_dxgi_GetAdapter(IDXGIDevice1 *self, IDXGIAdapter **pAdapter)
{
#if defined(__cplusplus)
...
static inline HRESULT
_sapp_dxgi_GetParent(IDXGIObject *self, REFIID riid, void **ppParent)
{
#if defined(__cplusplus)
...
static inline HRESULT
_sapp_dxgi_MakeWindowAssociation(IDXGIFactory *self, HWND WindowHandle, UINT Flags)
{
#if defined(__cplusplus)
...
_SOKOL_PRIVATE void
_sapp_d3d11_create_device_and_swapchain(void)
{
...
#endif
	SOKOL_dbg_assert(SUCCEEDED(hr) && _sapp.d3d11.swap_chain && _sapp.d3d11.device && _sapp.d3d11.device_context);
...
_SOKOL_PRIVATE void
_sapp_d3d11_destroy_device_and_swapchain(void)
{
...
_SOKOL_PRIVATE void
_sapp_d3d11_create_default_render_target(void)
{
	SOKOL_dbg_assert(0 == _sapp.d3d11.rt);
	SOKOL_dbg_assert(0 == _sapp.d3d11.rtv);
	SOKOL_dbg_assert(0 == _sapp.d3d11.msaa_rt);
	SOKOL_dbg_assert(0 == _sapp.d3d11.msaa_rtv);
	SOKOL_dbg_assert(0 == _sapp.d3d11.ds);
	SOKOL_dbg_assert(0 == _sapp.d3d11.dsv);
	...
	SOKOL_dbg_assert(SUCCEEDED(hr) && _sapp.d3d11.ds);
	hr = _sapp_d3d11_CreateDepthStencilView(_sapp.d3d11.device, (ID3D11Resource *)_sapp.d3d11.ds, NULL, &_sapp.d3d11.dsv);
	SOKOL_dbg_assert(SUCCEEDED(hr) && _sapp.d3d11.dsv);
}

_SOKOL_PRIVATE void
_sapp_d3d11_destroy_default_render_target(void)
{
...
_SOKOL_PRIVATE void
_sapp_d3d11_resize_default_render_target(void)
{
...
_SOKOL_PRIVATE void
_sapp_d3d11_present(bool do_not_wait)
{
	UINT flags = 0;
...
_sapp_wgl_init(void)
{
	...
	SOKOL_dbg_assert(_sapp.wgl.msg_hwnd);
...
_SOKOL_PRIVATE void
_sapp_wgl_shutdown(void)
{
	SOKOL_dbg_assert(_sapp.wgl.opengl32 && _sapp.wgl.msg_hwnd);
...
_SOKOL_PRIVATE bool
_sapp_wgl_has_ext(const char *ext, const char *extensions)
{
	SOKOL_dbg_assert(ext && extensions);
...
_SOKOL_PRIVATE bool
_sapp_wgl_ext_supported(const char *ext)
{
	SOKOL_dbg_assert(ext);
...
_SOKOL_PRIVATE void
_sapp_wgl_load_extensions(void)
{
	SOKOL_dbg_assert(_sapp.wgl.msg_dc);
...
_SOKOL_PRIVATE int
_sapp_wgl_attrib(int pixel_format, int attrib)
{
	SOKOL_dbg_assert(_sapp.wgl.arb_pixel_format);
	int value = 0;
...
_SOKOL_PRIVATE void
_sapp_wgl_attribiv(int pixel_format, int num_attribs, const int *attribs, int *results)
{
	SOKOL_dbg_assert(_sapp.wgl.arb_pixel_format);
...
_SOKOL_PRIVATE int
_sapp_wgl_find_pixel_format(void)
{
	SOKOL_dbg_assert(_sapp.win32.dc);
	SOKOL_dbg_assert(_sapp.wgl.arb_pixel_format);
...
#define _sapp_wgl_num_query_tags (12)
	...
	for(int i = 0; i < native_count; i++) {
		...
		if(query_results[result_support_opengl_index] == 0 || query_results[result_draw_to_window_index] == 0 || query_results[result_pixel_type_index] != WGL_TYPE_RGBA_ARB || query_results[result_acceleration_index] == WGL_NO_ACCELERATION_ARB) {
			continue;
		}
...
_sapp_wgl_create_context(void)
{
	...
	const int attrs[] = {
		WGL_CONTEXT_MAJOR_VERSION_ARB, _sapp.desc.gl_major_version, WGL_CONTEXT_MINOR_VERSION_ARB, _sapp.desc.gl_minor_version,
#if defined(SOKOL_DEBUG)
...
_SOKOL_PRIVATE void
_sapp_wgl_destroy_context(void)
{
	SOKOL_dbg_assert(_sapp.wgl.gl_ctx);
	_sapp.wgl.DeleteContext(_sapp.wgl.gl_ctx);
	_sapp.wgl.gl_ctx = 0;
}

_SOKOL_PRIVATE void
_sapp_wgl_swap_buffers(void)
{
	SOKOL_dbg_assert(_sapp.win32.dc);
	/* FIXME: DwmIsCompositionEnabled? (see GLFW) */
...
_SOKOL_PRIVATE bool
_sapp_win32_wide_to_utf8(const wchar_t *src, char *dst, int dst_num_bytes)
{
	SOKOL_dbg_assert(src && dst && (dst_num_bytes > 1));
	_sapp_clear(dst, (size_t)dst_num_bytes);
...
_SOKOL_PRIVATE bool
_sapp_win32_update_dimensions(void)
{
	RECT rect;
...
_SOKOL_PRIVATE void
_sapp_win32_set_fullscreen(bool fullscreen, UINT swp_flags)
{
...
_SOKOL_PRIVATE void
_sapp_win32_toggle_fullscreen(void)
{
	_sapp_win32_set_fullscreen(!_sapp.fullscreen, SWP_SHOWWINDOW);
}

_SOKOL_PRIVATE void
_sapp_win32_init_cursor(sapp_mouse_cursor cursor)
{
	SOKOL_dbg_assert((cursor >= 0) && (cursor < _SAPP_MOUSECURSOR_NUM));
	...
	SOKOL_dbg_assert(0 != _sapp.win32.cursors[cursor]);
}

_SOKOL_PRIVATE void
_sapp_win32_init_cursors(void)
{
...
_SOKOL_PRIVATE bool
_sapp_win32_cursor_in_content_area(void)
{
	POINT pos;
	if(!GetCursorPos(&pos)) {
		return false;
...
_SOKOL_PRIVATE void
_sapp_win32_update_cursor(sapp_mouse_cursor cursor, bool shown, bool skip_area_test)
{
...
_SOKOL_PRIVATE void
_sapp_win32_capture_mouse(uint8_t btn_mask)
{
...
_SOKOL_PRIVATE void
_sapp_win32_release_mouse(uint8_t btn_mask)
{
	if(0 != _sapp.win32.mouse.capture_mask) {
		_sapp.win32.mouse.capture_mask &= ~btn_mask;
		if(0 == _sapp.win32.mouse.capture_mask) {
...
_SOKOL_PRIVATE bool
_sapp_win32_is_foreground_window(void)
{
	return _sapp.win32.hwnd == GetForegroundWindow();
}

_SOKOL_PRIVATE void
_sapp_win32_lock_mouse(bool lock)
{
	_sapp.win32.mouse.requested_lock = lock;
}

_SOKOL_PRIVATE void
_sapp_win32_free_raw_input_data(void)
{
...
_SOKOL_PRIVATE void
_sapp_win32_alloc_raw_input_data(size_t size)
{
	SOKOL_dbg_assert(!_sapp.win32.raw_input_data.ptr);
	SOKOL_dbg_assert(size > 0);
...
_SOKOL_PRIVATE void *
_sapp_win32_ensure_raw_input_data(size_t required_size)
{
	...
	SOKOL_dbg_assert((((uintptr_t)_sapp.win32.raw_input_data.ptr) & 7) == 0);
	return _sapp.win32.raw_input_data.ptr;
}

_SOKOL_PRIVATE void
_sapp_win32_do_lock_mouse(void)
{
...
_SOKOL_PRIVATE void
_sapp_win32_do_unlock_mouse(void)
{
	_sapp.mouse.locked = false;
...
_SOKOL_PRIVATE void
_sapp_win32_update_mouse_lock(void)
{
...
_SOKOL_PRIVATE bool
_sapp_win32_update_monitor(void)
{
...
_SOKOL_PRIVATE void
_sapp_win32_mouse_update(LPARAM lParam)
{
	if(!_sapp.mouse.locked) {
...
_SOKOL_PRIVATE void
_sapp_win32_mouse_event(sapp_event_type type, sapp_mousebutton btn)
{
	if(_sapp_events_enabled()) {
		_sapp_init_event(type);
...
_SOKOL_PRIVATE void
_sapp_win32_scroll_event(float x, float y)
{
	if(_sapp_events_enabled()) {
		_sapp_init_event(SAPP_EVENTTYPE_MOUSE_SCROLL);
		_sapp.event.modifiers = _sapp_win32_mods();
...
_SOKOL_PRIVATE void
_sapp_win32_key_event(sapp_event_type type, int vk, bool repeat)
{
...
_SOKOL_PRIVATE void
_sapp_win32_char_event(uint32_t c, bool repeat)
{
...
_SOKOL_PRIVATE void
_sapp_win32_dpi_changed(HWND hWnd, LPRECT proposed_win_rect)
{
    ...
    */
	SOKOL_dbg_assert(_sapp.desc.high_dpi);
	...
	if(fn_getdpiforwindow) {
		...
		SetWindowPos(hWnd, 0, proposed_win_rect->left, proposed_win_rect->top, proposed_win_rect->right - proposed_win_rect->left, proposed_win_rect->bottom - proposed_win_rect->top, SWP_NOZORDER | SWP_NOACTIVATE);
	}
	FreeLibrary(user32);
}

_SOKOL_PRIVATE void
_sapp_win32_files_dropped(HDROP hdrop)
{
	if(!_sapp.drop.enabled) {
		return;
	}
...
_SOKOL_PRIVATE void
_sapp_win32_timing_measure(void)
{
#if defined(SOKOL_D3D11)
...
_SOKOL_PRIVATE LRESULT CALLBACK
...
_SOKOL_PRIVATE void
_sapp_win32_create_window(void)
{
	WNDCLASSW wndclassw;
...
_SOKOL_PRIVATE void
_sapp_win32_destroy_window(void)
{
...
_SOKOL_PRIVATE void
_sapp_win32_destroy_icons(void)
{
...
_SOKOL_PRIVATE void
_sapp_win32_restore_console(void)
{
	if(_sapp.desc.win32_console_utf8) {
...
_SOKOL_PRIVATE bool
_sapp_win32_set_clipboard_string(const char *str)
{
	SOKOL_dbg_assert(str);
	SOKOL_dbg_assert(_sapp.win32.hwnd);
	SOKOL_dbg_assert(_sapp.clipboard.enabled && (_sapp.clipboard.buf_size > 0));
...
_SOKOL_PRIVATE const char *
_sapp_win32_get_clipboard_string(void)
{
	SOKOL_dbg_assert(_sapp.clipboard.enabled && _sapp.clipboard.buffer);
	SOKOL_dbg_assert(_sapp.win32.hwnd);
	if(!OpenClipboard(_sapp.win32.hwnd)) {
...
_SOKOL_PRIVATE void
_sapp_win32_update_window_title(void)
{
...
_SOKOL_PRIVATE void
_sapp_win32_set_icon(const sapp_icon_desc *icon_desc, int num_images)
{
	SOKOL_dbg_assert((num_images > 0) && (num_images <= SAPP_MAX_ICONIMAGES));
...
*/
_SOKOL_PRIVATE bool
_sapp_win32_is_win10_or_greater(void)
{
...
_SOKOL_PRIVATE void
_sapp_win32_run(const sapp_desc *desc)
{
	_sapp_init_state(desc);
...
/* android loop thread */
_SOKOL_PRIVATE bool
_sapp_android_init_egl(void)
{
	SOKOL_dbg_assert(_sapp.android.display == EGL_NO_DISPLAY);
	SOKOL_dbg_assert(_sapp.android.context == EGL_NO_CONTEXT);
	...
	SOKOL_dbg_assert(cfg_count > 0);
	SOKOL_dbg_assert(cfg_count <= 32);
...
_SOKOL_PRIVATE void
_sapp_android_cleanup_egl(void)
{
...
_SOKOL_PRIVATE bool
_sapp_android_init_egl_surface(ANativeWindow *window)
{
	SOKOL_dbg_assert(_sapp.android.display != EGL_NO_DISPLAY);
	SOKOL_dbg_assert(_sapp.android.context != EGL_NO_CONTEXT);
	SOKOL_dbg_assert(_sapp.android.surface == EGL_NO_SURFACE);
	SOKOL_dbg_assert(window);
...
_SOKOL_PRIVATE void
_sapp_android_cleanup_egl_surface(void)
{
	if(_sapp.android.display == EGL_NO_DISPLAY) {
		return;
	}
...
_SOKOL_PRIVATE void
_sapp_android_app_event(sapp_event_type type)
{
	if(_sapp_events_enabled()) {
		_sapp_init_event(type);
		_sapp_call_event(&_sapp.event);
...
_SOKOL_PRIVATE void
_sapp_android_update_dimensions(ANativeWindow *window, bool force_update)
{
	SOKOL_dbg_assert(_sapp.android.display != EGL_NO_DISPLAY);
	SOKOL_dbg_assert(_sapp.android.context != EGL_NO_CONTEXT);
	SOKOL_dbg_assert(_sapp.android.surface != EGL_NO_SURFACE);
	...
	SOKOL_dbg_assert(win_w >= 0 && win_h >= 0);
	...
	if(win_changed || force_update) {
		if(!_sapp.desc.high_dpi) {
			...
			SOKOL_dbg_assert(egl_result == EGL_TRUE);
	...
	SOKOL_dbg_assert(egl_result_w == EGL_TRUE);
	_SOKOL_UNUSED(egl_result_w);
	SOKOL_dbg_assert(egl_result_h == EGL_TRUE);
...
_SOKOL_PRIVATE bool
_sapp_android_touch_event(const AInputEvent *e)
{
...
_SOKOL_PRIVATE bool
_sapp_android_key_event(const AInputEvent *e)
{
...
_SOKOL_PRIVATE int
_sapp_android_input_cb(int fd, int events, void *data)
{
	_SOKOL_UNUSED(fd);
	_SOKOL_UNUSED(data);
	...
	SOKOL_dbg_assert(_sapp.android.current.input);
...
_SOKOL_PRIVATE int
_sapp_android_main_cb(int fd, int events, void *data)
{
	_SOKOL_UNUSED(data);
	if((events & ALOOPER_EVENT_INPUT) == 0) {
	...
	case _SOKOL_ANDROID_MSG_CREATE: {
		...
		SOKOL_dbg_assert(!_sapp.valid);
		bool result = _sapp_android_init_egl();
		SOKOL_dbg_assert(result);
		_SOKOL_UNUSED(result);
		_sapp.valid               = true;
...
_SOKOL_PRIVATE bool
_sapp_android_should_update(void)
{
...
_SOKOL_PRIVATE void
_sapp_android_show_keyboard(bool shown)
{
	SOKOL_dbg_assert(_sapp.valid);
...
_SOKOL_PRIVATE void
_sapp_android_msg(_sapp_android_msg_t msg)
{
...
_SOKOL_PRIVATE void
_sapp_android_on_start(ANativeActivity *activity)
{
...
_SOKOL_PRIVATE void
_sapp_android_on_resume(ANativeActivity *activity)
{
	_SOKOL_UNUSED(activity);
...
_SOKOL_PRIVATE void *
_sapp_android_on_save_instance_state(ANativeActivity *activity, size_t *out_size)
{
	_SOKOL_UNUSED(activity);
...
_SOKOL_PRIVATE void
_sapp_android_on_window_focus_changed(ANativeActivity *activity, int has_focus)
{
	_SOKOL_UNUSED(activity);
...
_SOKOL_PRIVATE void
_sapp_android_on_pause(ANativeActivity *activity)
{
	_SOKOL_UNUSED(activity);
...
_SOKOL_PRIVATE void
_sapp_android_on_stop(ANativeActivity *activity)
{
	_SOKOL_UNUSED(activity);
...
_SOKOL_PRIVATE void
_sapp_android_msg_set_native_window(ANativeWindow *window)
{
	pthread_mutex_lock(&_sapp.android.pt.mutex);
...
_SOKOL_PRIVATE void
_sapp_android_on_native_window_created(ANativeActivity *activity, ANativeWindow *window)
{
	_SOKOL_UNUSED(activity);
...
_SOKOL_PRIVATE void
_sapp_android_on_native_window_destroyed(ANativeActivity *activity, ANativeWindow *window)
{
	_SOKOL_UNUSED(activity);
...
_SOKOL_PRIVATE void
_sapp_android_msg_set_input_queue(AInputQueue *input)
{
	pthread_mutex_lock(&_sapp.android.pt.mutex);
...
_SOKOL_PRIVATE void
_sapp_android_on_input_queue_created(ANativeActivity *activity, AInputQueue *queue)
{
	_SOKOL_UNUSED(activity);
...
_SOKOL_PRIVATE void
_sapp_android_on_input_queue_destroyed(ANativeActivity *activity, AInputQueue *queue)
{
	_SOKOL_UNUSED(activity);
	_SOKOL_UNUSED(queue);
...
_SOKOL_PRIVATE void
_sapp_android_on_config_changed(ANativeActivity *activity)
{
	_SOKOL_UNUSED(activity);
...
_SOKOL_PRIVATE void
_sapp_android_on_low_memory(ANativeActivity *activity)
{
	_SOKOL_UNUSED(activity);
...
_SOKOL_PRIVATE void
_sapp_android_on_destroy(ANativeActivity *activity)
{
	/*
...
_SOKOL_PRIVATE int
_sapp_x11_error_handler(Display *display, XErrorEvent *event)
{
...
_SOKOL_PRIVATE void
_sapp_x11_grab_error_handler(void)
{
...
_SOKOL_PRIVATE void
_sapp_x11_release_error_handler(void)
{
...
_SOKOL_PRIVATE void
_sapp_x11_init_extensions(void)
{
...
//
_SOKOL_PRIVATE sapp_keycode
_sapp_x11_translate_keysyms(const KeySym *keysyms, int width)
{
	if(width > 1) {
		switch(keysyms[1]) {
...
_SOKOL_PRIVATE void
_sapp_x11_query_system_dpi(void)
{
	/* from GLFW:
...
_SOKOL_PRIVATE bool
_sapp_glx_has_ext(const char *ext, const char *extensions)
{
	SOKOL_dbg_assert(ext);
	const char *start = extensions;
	while(true) {
...
_SOKOL_PRIVATE bool
_sapp_glx_extsupported(const char *ext, const char *extensions)
{
	if(extensions) {
...
_SOKOL_PRIVATE void *
_sapp_glx_getprocaddr(const char *procname)
{
...
_SOKOL_PRIVATE int
_sapp_glx_attrib(GLXFBConfig fbconfig, int attrib)
{
	int value;
...
_SOKOL_PRIVATE GLXFBConfig
_sapp_glx_choosefbconfig(void)
{
...
_SOKOL_PRIVATE void
_sapp_glx_choose_visual(Visual **visual, int *depth)
{
...
_sapp_glx_create_context(void)
{
	...
	const int attribs[] = {
		GLX_CONTEXT_MAJOR_VERSION_ARB, _sapp.desc.gl_major_version, GLX_CONTEXT_MINOR_VERSION_ARB, _sapp.desc.gl_minor_version, GLX_CONTEXT_PROFILE_MASK_ARB, GLX_CONTEXT_CORE_PROFILE_BIT_ARB, GLX_CONTEXT_FLAGS_ARB, GLX_CONTEXT_FORWARD_COMPATIBLE_BIT_ARB, 0, 0};
...
_SOKOL_PRIVATE void
_sapp_glx_destroy_context(void)
{
	if(_sapp.glx.window) {
...
_SOKOL_PRIVATE void
_sapp_glx_swap_buffers(void)
{
	_sapp.glx.SwapBuffers(_sapp.x11.display, _sapp.glx.window);
}

_SOKOL_PRIVATE void
_sapp_glx_swapinterval(int interval)
{
...
_SOKOL_PRIVATE void
_sapp_x11_send_event(Atom type, int a, int b, int c, int d, int e)
{
	XEvent event;
...
_SOKOL_PRIVATE void
_sapp_x11_query_window_size(void)
{
...
_SOKOL_PRIVATE void
_sapp_x11_set_fullscreen(bool enable)
{
...
_SOKOL_PRIVATE void
_sapp_x11_create_hidden_cursor(void)
{
	SOKOL_dbg_assert(0 == _sapp.x11.hidden_cursor);
	const int w       = 16;
	const int h       = 16;
	XcursorImage *img = XcursorImageCreate(w, h);
	SOKOL_dbg_assert(img && (img->width == 16) && (img->height == 16) && img->pixels);
	img->xhot              = 0;
	img->yhot              = 0;
...
_SOKOL_PRIVATE void
_sapp_x11_create_standard_cursor(sapp_mouse_cursor cursor, const char *name, const char *theme, int size, uint32_t fallback_native)
{
	SOKOL_dbg_assert((cursor >= 0) && (cursor < _SAPP_MOUSECURSOR_NUM));
	SOKOL_dbg_assert(_sapp.x11.display);
	if(theme) {
...
_SOKOL_PRIVATE void
_sapp_x11_destroy_cursors(void)
{
	SOKOL_dbg_assert(_sapp.x11.display);
...
_SOKOL_PRIVATE void
_sapp_x11_toggle_fullscreen(void)
{
	_sapp.fullscreen = !_sapp.fullscreen;
...
_SOKOL_PRIVATE void
_sapp_x11_update_cursor(sapp_mouse_cursor cursor, bool shown)
{
	SOKOL_dbg_assert((cursor >= 0) && (cursor < _SAPP_MOUSECURSOR_NUM));
	if(shown) {
...
_SOKOL_PRIVATE void
_sapp_x11_lock_mouse(bool lock)
{
	if(lock == _sapp.mouse.locked) {
		return;
	}
...
_SOKOL_PRIVATE void
_sapp_x11_set_clipboard_string(const char *str)
{
	SOKOL_dbg_assert(_sapp.clipboard.enabled && _sapp.clipboard.buffer);
...
_SOKOL_PRIVATE const char *
_sapp_x11_get_clipboard_string(void)
{
	SOKOL_dbg_assert(_sapp.clipboard.enabled && _sapp.clipboard.buffer);
...
_SOKOL_PRIVATE void
_sapp_x11_update_window_title(void)
{
	Xutf8SetWMProperties(_sapp.x11.display,
		...
		NULL,
		NULL);
	XChangeProperty(_sapp.x11.display, _sapp.x11.window, _sapp.x11.NET_WM_NAME, _sapp.x11.UTF8_STRING, 8, PropModeReplace, (unsigned char *)_sapp.window_title, strlen(_sapp.window_title));
	XChangeProperty(_sapp.x11.display, _sapp.x11.window, _sapp.x11.NET_WM_ICON_NAME, _sapp.x11.UTF8_STRING, 8, PropModeReplace, (unsigned char *)_sapp.window_title, strlen(_sapp.window_title));
	XFlush(_sapp.x11.display);
}

_SOKOL_PRIVATE void
_sapp_x11_set_icon(const sapp_icon_desc *icon_desc, int num_images)
{
	SOKOL_dbg_assert((num_images > 0) && (num_images <= SAPP_MAX_ICONIMAGES));
	int long_count = 0;
	for(int i = 0; i < num_images; i++) {
	...
	XChangeProperty(_sapp.x11.display, _sapp.x11.window, _sapp.x11.NET_WM_ICON, XA_CARDINAL, 32, PropModeReplace, (unsigned char *)icon_data, long_count);
	_sapp_free(icon_data);
	XFlush(_sapp.x11.display);
}

_SOKOL_PRIVATE void
_sapp_x11_create_window(Visual *visual, int depth)
{
...
_SOKOL_PRIVATE int
_sapp_x11_get_window_state(void)
{
...
_SOKOL_PRIVATE uint32_t
_sapp_x11_key_modifier_bit(sapp_keycode key)
{
	switch(key) {
...
_SOKOL_PRIVATE uint32_t
_sapp_x11_button_modifier_bit(sapp_mousebutton btn)
{
	switch(btn) {
...
_SOKOL_PRIVATE uint32_t
_sapp_x11_mods(uint32_t x11_mods)
{
	uint32_t mods = 0;
...
_SOKOL_PRIVATE void
_sapp_x11_app_event(sapp_event_type type)
{
	if(_sapp_events_enabled()) {
		_sapp_init_event(type);
		_sapp_call_event(&_sapp.event);
...
_SOKOL_PRIVATE void
_sapp_x11_mouse_update(int x, int y, bool clear_dxdy)
{
	if(!_sapp.mouse.locked) {
...
_SOKOL_PRIVATE void
_sapp_x11_mouse_event(sapp_event_type type, sapp_mousebutton btn, uint32_t mods)
{
	if(_sapp_events_enabled()) {
		_sapp_init_event(type);
		_sapp.event.mouse_button = btn;
...
_SOKOL_PRIVATE void
_sapp_x11_scroll_event(float x, float y, uint32_t mods)
{
	if(_sapp_events_enabled()) {
		_sapp_init_event(SAPP_EVENTTYPE_MOUSE_SCROLL);
		_sapp.event.modifiers = mods;
...
_SOKOL_PRIVATE void
_sapp_x11_key_event(sapp_event_type type, sapp_keycode key, bool repeat, uint32_t mods)
{
	if(_sapp_events_enabled()) {
		_sapp_init_event(type);
		_sapp.event.key_code   = key;
...
_SOKOL_PRIVATE void
_sapp_x11_char_event(uint32_t chr, bool repeat, uint32_t mods)
{
	if(_sapp_events_enabled()) {
		_sapp_init_event(SAPP_EVENTTYPE_CHAR);
...
_SOKOL_PRIVATE sapp_keycode
_sapp_x11_translate_key(int scancode)
{
...
_SOKOL_PRIVATE int32_t
_sapp_x11_keysym_to_unicode(KeySym keysym)
{
	int min = 0;
...
_SOKOL_PRIVATE bool
_sapp_x11_keypress_repeat(int keycode)
{
	bool repeat = false;
...
_SOKOL_PRIVATE void
_sapp_x11_keyrelease_repeat(int keycode)
{
	if((keycode >= 0) && (keycode < _SAPP_X11_MAX_X11_KEYCODES)) {
...
_SOKOL_PRIVATE bool
_sapp_x11_parse_dropped_files_list(const char *src)
{
...
_SOKOL_PRIVATE void
_sapp_x11_on_genericevent(XEvent *event)
{
...
_SOKOL_PRIVATE void
_sapp_x11_on_focusin(XEvent *event)
{
...
_SOKOL_PRIVATE void
_sapp_x11_on_focusout(XEvent *event)
{
...
_SOKOL_PRIVATE void
_sapp_x11_on_keypress(XEvent *event)
{
...
_SOKOL_PRIVATE void
_sapp_x11_on_keyrelease(XEvent *event)
{
	int keycode            = (int)event->xkey.keycode;
	const sapp_keycode key = _sapp_x11_translate_key(keycode);
...
_SOKOL_PRIVATE void
_sapp_x11_on_buttonpress(XEvent *event)
{
...
_SOKOL_PRIVATE void
_sapp_x11_on_buttonrelease(XEvent *event)
{
	_sapp_x11_mouse_update(event->xbutton.x, event->xbutton.y, false);
	const sapp_mousebutton btn = _sapp_x11_translate_button(event);
	if(btn != SAPP_MOUSEBUTTON_INVALID) {
...
_SOKOL_PRIVATE void
_sapp_x11_on_enternotify(XEvent *event)
{
...
_SOKOL_PRIVATE void
_sapp_x11_on_leavenotify(XEvent *event)
{
	if(0 == _sapp.x11.mouse_buttons) {
		_sapp_x11_mouse_update(event->xcrossing.x, event->xcrossing.y, true);
...
_SOKOL_PRIVATE void
_sapp_x11_on_motionnotify(XEvent *event)
{
	if(!_sapp.mouse.locked) {
...
_SOKOL_PRIVATE void
_sapp_x11_on_configurenotify(XEvent *event)
{
...
_SOKOL_PRIVATE void
_sapp_x11_on_propertynotify(XEvent *event)
{
...
_SOKOL_PRIVATE void
_sapp_x11_on_selectionnotify(XEvent *event)
{
...
_SOKOL_PRIVATE void
_sapp_x11_on_clientmessage(XEvent *event)
{
...
_SOKOL_PRIVATE void
_sapp_x11_on_selectionrequest(XEvent *event)
{
...
_SOKOL_PRIVATE void
_sapp_x11_process_event(XEvent *event)
{
	switch(event->type) {
	case GenericEvent:
...
_SOKOL_PRIVATE void
_sapp_linux_run(const sapp_desc *desc)
{
...
SOKOL_API_IMPL sapp_desc
sapp_query_desc(void)
{
	return _sapp.desc;
}

SOKOL_API_IMPL uint64_t
sapp_frame_count(void)
{
	return _sapp.frame_count;
}

SOKOL_API_IMPL double
sapp_frame_duration(void)
{
...
SOKOL_API_IMPL int
sapp_depth_format(void)
{
	return _SAPP_PIXELFORMAT_DEPTH_STENCIL;
}

SOKOL_API_IMPL int
sapp_sample_count(void)
{
...
SOKOL_API_IMPL bool
sapp_keyboard_shown(void)
{
	return _sapp.onscreen_keyboard_shown;
}

SOKOL_API_IMPL bool
sapp_is_fullscreen(void)
{
	return _sapp.fullscreen;
}

SOKOL_API_IMPL void
sapp_toggle_fullscreen(void)
{
#if defined(_SAPP_MACOS)
...
SOKOL_API_IMPL bool
sapp_mouse_shown(void)
{
	return _sapp.mouse.shown;
}

SOKOL_API_IMPL void
sapp_lock_mouse(bool lock)
{
#if defined(_SAPP_MACOS)
...
SOKOL_API_IMPL sapp_mouse_cursor
sapp_get_mouse_cursor(void)
{
	return _sapp.mouse.current_cursor;
}

SOKOL_API_IMPL void
sapp_request_quit(void)
{
	_sapp.quit_requested = true;
}

SOKOL_API_IMPL void
sapp_cancel_quit(void)
{
...
SOKOL_API_IMPL void
sapp_set_clipboard_string(const char *str)
{
	if(!_sapp.clipboard.enabled) {
		return;
	}
...
SOKOL_API_IMPL const char *
sapp_get_clipboard_string(void)
{
	if(!_sapp.clipboard.enabled) {
		return "";
	}
...
SOKOL_API_IMPL void
sapp_set_window_title(const char *title)
{
	SOKOL_dbg_assert(title);
...
SOKOL_API_IMPL void
sapp_set_icon(const sapp_icon_desc *desc)
{
	SOKOL_dbg_assert(desc);
...
SOKOL_API_IMPL int
sapp_get_num_dropped_files(void)
{
	SOKOL_dbg_assert(_sapp.drop.enabled);
	return _sapp.drop.num_files;
}

SOKOL_API_IMPL const char *
sapp_get_dropped_file_path(int index)
{
	SOKOL_dbg_assert(_sapp.drop.enabled);
	SOKOL_dbg_assert((index >= 0) && (index < _sapp.drop.num_files));
	SOKOL_dbg_assert(_sapp.drop.buffer);
	if(!_sapp.drop.enabled) {
		return "";
	}
...
SOKOL_API_IMPL uint32_t
sapp_html5_get_dropped_file_size(int index)
{
	SOKOL_dbg_assert(_sapp.drop.enabled);
	SOKOL_dbg_assert((index >= 0) && (index < _sapp.drop.num_files));
#if defined(_SAPP_EMSCRIPTEN)
...
{
	SOKOL_dbg_assert(_sapp.drop.enabled);
	SOKOL_dbg_assert(request);
	SOKOL_dbg_assert(request->callback);
	SOKOL_dbg_assert(request->buffer.ptr);
	SOKOL_dbg_assert(request->buffer.size > 0);
#if defined(_SAPP_EMSCRIPTEN)
...
SOKOL_API_IMPL const void *
sapp_metal_get_current_drawable(void)
{
	SOKOL_dbg_assert(_sapp.valid);
#if defined(SOKOL_METAL)
#if defined(_SAPP_MACOS)
...
SOKOL_API_IMPL const void *
sapp_metal_get_depth_stencil_texture(void)
{
	SOKOL_dbg_assert(_sapp.valid);
#if defined(SOKOL_METAL)
#if defined(_SAPP_MACOS)
...
SOKOL_API_IMPL const void *
sapp_metal_get_msaa_color_texture(void)
{
	SOKOL_dbg_assert(_sapp.valid);
#if defined(SOKOL_METAL)
#if defined(_SAPP_MACOS)
...
SOKOL_API_IMPL const void *
sapp_d3d11_get_device_context(void)
{
	SOKOL_dbg_assert(_sapp.valid);
#if defined(SOKOL_D3D11)
...
SOKOL_API_IMPL const void *
sapp_d3d11_get_swap_chain(void)
{
	SOKOL_dbg_assert(_sapp.valid);
#if defined(SOKOL_D3D11)
...
SOKOL_API_IMPL const void *
sapp_d3d11_get_render_view(void)
{
	SOKOL_dbg_assert(_sapp.valid);
#if defined(SOKOL_D3D11)
	if(_sapp.sample_count > 1) {
		SOKOL_dbg_assert(_sapp.d3d11.msaa_rtv);
		return _sapp.d3d11.msaa_rtv;
	} else {
		SOKOL_dbg_assert(_sapp.d3d11.rtv);
...
SOKOL_API_IMPL const void *
sapp_d3d11_get_resolve_view(void)
{
	SOKOL_dbg_assert(_sapp.valid);
#if defined(SOKOL_D3D11)
	if(_sapp.sample_count > 1) {
...
SOKOL_API_IMPL const void *
sapp_wgpu_get_render_view(void)
{
	SOKOL_dbg_assert(_sapp.valid);
#if defined(_SAPP_EMSCRIPTEN) && defined(SOKOL_WGPU)
	if(_sapp.sample_count > 1) {
		SOKOL_dbg_assert(_sapp.wgpu.msaa_view);
		return (const void *)_sapp.wgpu.msaa_view;
	} else {
		SOKOL_dbg_assert(_sapp.wgpu.swapchain_view);
...
SOKOL_API_IMPL const void *
sapp_wgpu_get_resolve_view(void)
{
	SOKOL_dbg_assert(_sapp.valid);
#if defined(_SAPP_EMSCRIPTEN) && defined(SOKOL_WGPU)
	if(_sapp.sample_count > 1) {
...
SOKOL_API_IMPL const void *
sapp_wgpu_get_depth_stencil_view(void)
{
	SOKOL_dbg_assert(_sapp.valid);
#if defined(_SAPP_EMSCRIPTEN) && defined(SOKOL_WGPU)
...
SOKOL_API_IMPL int
sapp_gl_get_major_version(void)
{
	SOKOL_dbg_assert(_sapp.valid);
#if defined(_SAPP_ANY_GL)
...
SOKOL_API_IMPL int
sapp_gl_get_minor_version(void)
{
	SOKOL_dbg_assert(_sapp.valid);
#if defined(_SAPP_ANY_GL)
...
SOKOL_API_IMPL const void *
sapp_android_get_native_activity(void)
{
