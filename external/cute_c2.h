...
C2_INLINE void
c2SinCos(float radians, float *s, float *c)
{
	*c = c2Cos(radians);
	*s = c2Sin(radians);
}
...
c2Minv(c2v a, c2v b)
{
	return c2V(c2Min(a.x, b.x), c2Min(a.y, b.y));
}
C2_INLINE c2v
c2Maxv(c2v a, c2v b)
{
	return c2V(c2Max(a.x, b.x), c2Max(a.y, b.y));
}
C2_INLINE c2v
c2Clampv(c2v a, c2v lo, c2v hi)
{
...
c2Absv(c2v a)
{
	return c2V(c2Abs(a.x), c2Abs(a.y));
}
C2_INLINE float
c2Hmin(c2v a)
{
	return c2Min(a.x, a.y);
}
C2_INLINE float
c2Hmax(c2v a)
{
	return c2Max(a.x, a.y);
}
C2_INLINE float
c2Len(c2v a)
{
	return c2Sqrt(c2Dot(a, a));
}
C2_INLINE c2v
c2Norm(c2v a)
{
...
C2_INLINE c2v
c2Lerp(c2v a, c2v b, float t)
{
	return c2Add(a, c2Mulvs(c2Sub(b, a), t));
}
C2_INLINE int
c2Parallel(c2v a, c2v b, float kTol)
{
...
C2_INLINE c2x
c2Transform(c2v p, float radians)
{
	c2x x;
	x.r = c2Rot(radians);
	x.p = p;
...
C2_INLINE c2v
c2Intersect(c2v a, c2v b, float da, float db)
{
...
int
c2Collided(const void *A, const c2x *ax, C2_TYPE typeA, const void *B, const c2x *bx, C2_TYPE typeB)
{
	switch(typeA) {
	case C2_TYPE_CIRCLE:
		switch(typeB) {
...
void
c2Collide(const void *A, const c2x *ax, C2_TYPE typeA, const void *B, const c2x *bx, C2_TYPE typeB, c2Manifold *m)
{
	m->count = 0;
...
int
c2CastRay(c2Ray A, const void *B, const c2x *bx, C2_TYPE typeB, c2Raycast *out)
{
	switch(typeB) {
...
static C2_INLINE int
c2Support(const c2v *verts, int count, c2v d)
{
	int imax   = 0;
...
static C2_INLINE void
c2Witness(c2Simplex *s, c2v *a, c2v *b)
{
	float den = 1.0f / s->div;
	switch(s->count) {
	case 1:
...
static C2_INLINE float
c2GJKSimplexMetric(c2Simplex *s)
{
	switch(s->count) {
...
float
c2GJK(const void *A, C2_TYPE typeA, const c2x *ax_ptr, const void *B, C2_TYPE typeB, const c2x *bx_ptr, c2v *outA, c2v *outB, int use_radius, int *iterations, c2GJKCache *cache)
{
	c2x ax;
	c2x bx;
	if(!ax_ptr)
...
c2TOIResult
c2TOI(const void *A, C2_TYPE typeA, const c2x *ax_ptr, c2v vA, const void *B, C2_TYPE typeB, const c2x *bx_ptr, c2v vB, int use_radius)
{
	float t = 0;
	c2x ax;
	c2x bx;
...
void
c2Norms(c2v *verts, c2v *norms, int count)
{
...
c2Poly
c2Dual(c2Poly poly, float skin_factor)
{
	c2Poly dual;
...
c2Poly
c2InflatePoly(c2Poly poly, float skin_factor)
{
...
void
c2Inflate(void *shape, C2_TYPE type, float skin_factor)
{
	switch(type) {
	case C2_TYPE_CIRCLE: {
...
int
c2CircletoCircle(c2Circle A, c2Circle B)
{
	c2v c    = c2Sub(B.p, A.p);
	float d2 = c2Dot(c, c);
	float r2 = A.r + B.r;
...
int
c2CircletoAABB(c2Circle A, c2AABB B)
{
...
int
c2AABBtoAABB(c2AABB A, c2AABB B)
{
...
int
c2CircleToPoint(c2Circle A, c2v B)
{
	c2v n    = c2Sub(A.p, B);
	float d2 = c2Dot(n, n);
	return d2 < A.r * A.r;
...
int
c2CircletoCapsule(c2Circle A, c2Capsule B)
{
	c2v n    = c2Sub(B.b, B.a);
...
int
c2AABBtoCapsule(c2AABB A, c2Capsule B)
{
...
int
c2CapsuletoCapsule(c2Capsule A, c2Capsule B)
{
...
int
c2CircletoPoly(c2Circle A, const c2Poly *B, const c2x *bx)
{
...
int
c2AABBtoPoly(c2AABB A, const c2Poly *B, const c2x *bx)
{
...
int
c2CapsuletoPoly(c2Capsule A, const c2Poly *B, const c2x *bx)
{
...
int
c2PolytoPoly(const c2Poly *A, const c2x *ax, const c2Poly *B, const c2x *bx)
{
...
int
c2RaytoCircle(c2Ray A, c2Circle B, c2Raycast *out)
{
	c2v p      = B.p;
	c2v m      = c2Sub(A.p, p);
...
static inline float
c2SignedDistPointToPlane_OneDimensional(float p, float n, float d)
{
	return p * n - d * n;
}

static inline float
c2RayToPlane_OneDimensional(float da, float db)
{
	if(da < 0)
...
int
c2RaytoAABB(c2Ray A, c2AABB B, c2Raycast *out)
{
	c2v p0 = A.p;
...
int
c2RaytoCapsule(c2Ray A, c2Capsule B, c2Raycast *out)
{
	c2m M;
...
int
c2RaytoPoly(c2Ray A, const c2Poly *B, const c2x *bx_ptr, c2Raycast *out)
{
...
static C2_INLINE c2h
c2PlaneAt(const c2Poly *p, const int i)
{
	c2h h;
	h.n = p->norms[i];
...
void
c2CircletoPolyManifold(c2Circle A, const c2Poly *B, const c2x *bx_tr, c2Manifold *m)
{
	m->count = 0;
	c2v a, b;
...
void
c2AABBtoPolyManifold(c2AABB A, const c2Poly *B, const c2x *bx, c2Manifold *m)
{
	m->count = 0;
	c2Poly p;
	c2BBVerts(p.verts, &A);
...
static int
c2SidePlanes(c2v *seg, c2v ra, c2v rb, c2h *h)
{
...
static int
c2SidePlanesFromPoly(c2v *seg, c2x x, const c2Poly *p, int e, c2h *h)
{
...
static void
c2KeepDeep(c2v *seg, c2h h, c2Manifold *m)
{
	int cp = 0;
	for(int i = 0; i < 2; ++i) {
		c2v p   = seg[i];
...
static C2_INLINE c2v
c2CapsuleSupport(c2Capsule A, c2v dir)
{
...
static void
c2AntinormalFace(c2Capsule cap, const c2Poly *p, c2x x, int *face_out, c2v *n_out)
{
	float sep = -FLT_MAX;
	int index = ~0;
	c2v n     = c2V(0, 0);
...
static void
c2Incident(c2v *incident, const c2Poly *ip, c2x ix, c2v rn_in_incident_space)
{
	int index     = ~0;
...
void
c2CapsuletoPolyManifold(c2Capsule A, const c2Poly *B, const c2x *bx_ptr, c2Manifold *m)
{
	m->count = 0;
	c2v a, b;
...
static float
c2CheckFaces(const c2Poly *A, c2x ax, const c2Poly *B, c2x bx, int *face_index)
{
...
void
c2PolytoPolyManifold(const c2Poly *A, const c2x *ax_ptr, const c2Poly *B, const c2x *bx_ptr, c2Manifold *m)
{
	m->count = 0;
