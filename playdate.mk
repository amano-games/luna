...
# TARGET_PLAYDATE / TARGET_SIMULATOR are Playdate SDK flags; they also select OS_PLAYDATE.
...
build_pd:
	...
	$(MAKE) -f $(ROOT_DIR)/playdate.mk $(DEVICE_READY) assets \
		DESTDIR=$(DESTDIR) PREFIX=$(PREFIX) GAME_NAME=$(GAME_NAME) \
		PLATFORM_DIR=$(PLATFORM_DIR) BUILD_DEBUG=$(BUILD_DEBUG) CDEFS="$(CDEFS)"
	$(MAKE) -f $(ROOT_DIR)/playdate.mk $(OBJS) \
		DESTDIR=$(DESTDIR) PREFIX=$(PREFIX) GAME_NAME=$(GAME_NAME) \
		PLATFORM_DIR=$(PLATFORM_DIR) BUILD_DEBUG=$(BUILD_DEBUG) CDEFS="$(CDEFS)"
...
publish_release:
	...
	$(MAKE) -f $(ROOT_DIR)/playdate.mk $(PUBLISH_OBJS) BUILD_DEBUG=0 DESTDIR=$(DESTDIR) PREFIX=$(PREFIX) GAME_NAME=$(GAME_NAME) PLATFORM_DIR=$(PLATFORM_DIR) CDEFS="$(CDEFS)"
